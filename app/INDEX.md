...
| URL | Page component |
|-----|----------------|
| `/builder` | `BuilderPage.tsx` — segment `loading.tsx` is house leftover, not group Loading. Builder first paint is house leftover (static page; Blank workout / saved, not RouteLoading). Write side of the official training catalog (templates). Show all includes the offline program shell (template only until Start opens Train). Not a shop. |
