...
| `cron/wind-down` | GET | `Authorization: Bearer CRON_SECRET` | — | **Hourly.** Evening (19–22 local) note after a session that ran hot. Push only; own `last_wind_down_at` marker so it never suppresses a comeback. `?dryRun=1` reports `localHour` per candidate |
