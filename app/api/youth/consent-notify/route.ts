 ...
 * Auth: session | Rate: 3/min/IP + 3/hour/recipient | Schema: youthConsentNotifySchema
 * 404 when the youth/school surface is parked (P2-3).
...
export const POST = withApiLogging('youth/consent-notify', async(request: NextRequest) => {
  ...
  const recipientLimited = await rateLimitAsync(`youth-consent-notify:to:${emailKey}`, 3, 60 * 60_000);
  if (!recipientLimited.ok) {
    return NextResponse.json(
      { error: 'Too many consent requests. Try again shortly.' },
      {
        status: 429,
        headers: { 'Retry-After': String(recipientLimited.retryAfterSec ?? 3600) },
      }
    );
  }
