...
**Mascot day (prefer when Kalligator kit is thin):**

1. `mascot-kalligator-idle` — turnaround / default  
2. `mascot-kalligator-invite` — empty + social invite  
3. `mascot-kalligator-celebrate` — Victory  
4. `social-kalligator-loop` — 4–6s Lite motion (optional)  
...
2. **Name:** `Kalligator` (exact — you will `@Kalligator` in later prompts).  
3. Paste **Character creation prompt** below (appearance lock).  
4. Optional but strong: upload `/brand/mascot/kalligator-idle.webp` (or inbox frame) as a reference ingredient — plain paper background, subject only.  
...
8. Save. Later scenes: `@Kalligator` + a short action prompt from § Scene prompts.

### Character creation prompt (paste into New Character)

```
Create a reusable brand mascot character named Kalligator for Mission Winning.
...
1. Mid grey cream belly (two short strokes forming a V / rank mark).
...
- No second character in frame. No busy environment behind Kalligator for the reference sheet.
```

### Character info (personality field)

```
Kalligator is Mission Winning’s mission-briefing companion. Speaks and moves with calm competence:
...
Dry wit allowed; guilt, streak harassment, and paywall shame are forbidden. Kalligator is not the
AI Coach — Kalligator is a silent/visual companion for invites, empty states, and victory moments.
```

### Secondary angle / body lock (when Flow asks)

```
Same Kalligator character: cartoon crocodile/alligator, identical proportions and colors
(paper #f3f2f2, vermillion red #ec3013, cream belly #6f6b69). Full-body / profile
...
changes — Kalligator has no clothing; belly + spines are the “uniform.”
```

### Scene prompts (after character is saved — use `@Kalligator`)

**Idle / turnaround still**

```
@Kalligator centered on solid paper #f3f2f2, neutral briefing pose, cream belly visible,
...
```
@Kalligator on paper #f3f2f2, calm invite: slight forward energy / open arms or fist beckon, friendly but not
desperate, cream belly + the one red accents. No text, no logos, no guilt expression.
```

**Celebrate (Victory)**

```
@Kalligator on paper #f3f2f2, quiet victory: subtle lift, cream belly gleams, vermillion red rim light.
...
```
@Kalligator on paper field, subtle nod and cream belly flash, loopable micro-motion,
clinical calm. Flat geometric mascot style. No text, no logos, no photoreal.
```

**Export naming:** `mascot-kalligator-idle-frame.png` · `mascot-kalligator-invite-frame.png` · `mascot-kalligator-celebrate-frame.png` → `npm run media:optimize-inbox`

---

## Mascot queue — Kalligator (short prompts if character already exists)

If `@Kalligator` is saved, prefer the Scene prompts above. Legacy one-shot prompts (no Characters tab):

### Prompt: mascot-kalligator-idle (`promptId: flow-mascot-kalligator-idle`)
...
No text, no logos, no photoreal scales, no purple, no cute crying face.
Readable silhouette for app empty states.
```

**Export:** `media/inbox/mascot-kalligator-idle-frame.png` → `public/brand/mascot/kalligator-idle.webp`

### Prompt: mascot-kalligator-invite (`promptId: flow-mascot-kalligator-invite`)
...
friendly attention, the one red accent #ec3013, cream belly #6f6b69. Paper canvas
#f3f2f2. Not desperate, not guilt-tripping. No text, no logos, no photoreal.
```

**Export:** `media/inbox/mascot-kalligator-invite-frame.png` → `public/brand/mascot/kalligator-invite.webp`

### Prompt: mascot-kalligator-celebrate (`promptId: flow-mascot-kalligator-celebrate`)
...
**Export:** `media/inbox/mascot-kalligator-celebrate-frame.png` → `public/brand/mascot/kalligator-celebrate.webp`

### Prompt: social-kalligator-loop (`promptId: flow-social-kalligator-loop`)

**Model:** Veo 3.1 Lite · 4–6s · 1:1 or 9:16

```
Mission Winning brand mascot Kalligator. Geometric crocodile/alligator, flat paper/vermillion red/mid grey,
subtle nod and cream belly flash on paper field. Loopable micro-motion,
clinical calm. No text, no logos, no photoreal.
```

**Export:** `media/inbox/social-kalligator-loop-raw.mp4` (+ optional `social-kalligator-loop-frame.png`)
