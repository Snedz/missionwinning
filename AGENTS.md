...
- **Hello-world flow:** `/active` is the free Train logger. From a fresh state it routes through a short onboarding (Begin → Continue), lands on Today with one Start, and you log a set by entering weight/reps and clicking **Log set**. Account is offered after the first workout, never before.
