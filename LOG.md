...
Mutants: consent equality flipped → stale record passes → red; utm persisted pre-consent → attribution red; drop Sentry from subprocessor body → legalContent red; retention constant 90→60 → prose guard red; llm_usage removed from deletion registry → completeness red; token column added to export list → redaction red; font-semibold beside display-section → check-display-type red; css walk reverted → designSystem fixture red; font-medium reintroduced in page-components → designWeights red.
