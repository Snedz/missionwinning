...
## 2026-09-18 — Fuel help FAQ honesty (`.1111`)

Program 67 kits (`resume-kit`,
`invoice-lite`, `kdp-color`)
are not on this disk. This
ship is the on-disk Fuel
product: athlete help now
names the four facts the
logger already enforces.
**Edit-before-log** — low /
rough drafts keep Log meal
disabled until a touch.
**Vision vs heuristic** —
`MEAL_VISION_*` or filename
color, not a food lab.
**Restock does not order.**
**Not clinical.** Cross-links:
fuel guide ↔ faq ↔ Train /
Coach. Guard:
`fuelHelpFaqHonesty.test.ts`
discovers `docs/help/*.md`.
No paymentUrl. No checkout.
No invented NL tokens. No
ClearShot. No tip-promote.

**Mutants killed:** dropping
the Fuel FAQ heading; dropping
edit-before-log; dropping
restock-does-not-order;
inventing `paymentUrl`.
