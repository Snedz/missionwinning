...
## 2026-08-16 — `/harness` pastes the harvest (`.871`)

Empty queue plus a living `idea:next` pick used to print and wait for
`--paste`. That is why IL-H-15 sat off the list. `/harness` is now the
paste. Generate stays legal until two scout-harvests in a row survive
zero. Then C5. Not AU2.

**Ship:** `harvestGate.ts` · `idea:paste` · route `harvestAction`
paste | generate. Next `/harness` pastes IL-H-15.

Label `.871` (onto master `.870`).

Excellence-Override: harness harvest paste (tooling; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-856-for-871.md](docs/archive/log/LOG-rotate-856-for-871.md).

## 2026-08-16 — Harness follows the Horizon W path (`.870`)

Empty GRAPH_LOOP plus an empty harvest used to stop. That is the right
rule for letters and the wrong rule for the product: Horizon W is still
NOW and still unscored.

**Ship:** `route` `path` (recipe 15) · `criticalPath.ts` names the first
unproven W-instrument. Current master prints `C5` founder phone, not AU2.

Label `.870` (onto master `.869`).

Excellence-Override: harness critical path (tooling; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-855-for-870.md](docs/archive/log/LOG-rotate-855-for-870.md).

## 2026-08-16 — Process is `/harness` (`.869`)

`/graph` named the outer loop after the wrong thing. The field word is
agent harness. Graph already means Neo4j ingest and the idea graph.

**Ship:** command `/harness` · `npm run harness` / `harness:done` · hop
files in `docs/harness/`. `/graph` and `npm run graph` stay as aliases.
`GRAPH_LOOP.md`, `ideaGraph`, and `graph:ingest` are unchanged.

Label `.869` (onto master `.868`).

Excellence-Override: process rename (tooling; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-854-for-869.md](docs/archive/log/LOG-rotate-854-for-869.md).

## 2026-08-16 — A hop is done when a command says so (`.868`)

The agent saying "done" was the default closer. Mid-hop state lived in
`~/.grok` on one machine. A fresh spawn after compaction could not see
the ticket, the one-sentence done, or the accept command.

**Ship:** `docs/graph/HOP.md` · `PINNED.md` · `npm run graph:done`.
Does not write the queue. Empty template stays on master.

Label `.868` (onto master `.867`).

Excellence-Override: graph hop contract (tooling; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-853-for-868.md](docs/archive/log/LOG-rotate-853-for-868.md).

## 2026-08-16 — W1 Activation lands I-Day on Today (`.867`)

ORCHESTRATION still sent I-Day to `/active`. Product lands `/log` since `.839`.

**Ship:** W1 row + `orchestrationW1Landing.test.ts`. No new GRAPH_LOOP letter.

Label `.867` (onto master `.866`).

Excellence-Override: W1 activation landing (docs + pin; RESULT unscored)
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-851-for-866.md](docs/archive/log/LOG-rotate-851-for-866.md).

## 2026-08-16 — Returning dose follows missed sessions (`.865`)

H-07: the same fourteen-day gap is not the same return. Stored miss
markers set the dose. No plan keeps the calendar short-session scale.
The quiet line still names no gap.

**Ship:** `doseScaleForMissedSessions` · `computeReentry` reads the plan.

Label `.865` (onto master `.864`).

Excellence-Override: re-entry dose (Today/Coach start; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-850-for-865.md](docs/archive/log/LOG-rotate-850-for-865.md).

## 2026-08-16 — H-07 is on the queue (`.864`)

Next `idea:next`. Now AT is `IL-H-07` — first session back follows missed
sessions, not days. Do not invent AT2.

**Ship:** GRAPH_LOOP AT · LEDGER harvest-8.

Label `.864` (onto master `.863`).

Excellence-Override: queue paste, no product surface (RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-849-for-864.md](docs/archive/log/LOG-rotate-849-for-864.md).

## 2026-08-16 — One tap off the cold path (`.863`)

H-05: the walk was already four taps against a budget of five. The unused
slack is gone. The first empty row proposes the athlete's last stored set,
not a suggestion they did not produce. Cold history stays blank.

**Ship:** `TAP_BUDGET` 5→4 · `resolveSetInput` last-set before suggestion.

Label `.863` (onto master `.862`).

Excellence-Override: first-set tap ratchet (logger; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-848-for-863.md](docs/archive/log/LOG-rotate-848-for-863.md).

## 2026-08-16 — H-05 is on the queue (`.862`)

Next `idea:next`. Now AS is `IL-H-05` — one tap off the cold path.
Do not invent AS2.

**Ship:** GRAPH_LOOP AS · LEDGER harvest-7.

Label `.862` (onto master `.861`).

Excellence-Override: queue paste, no product surface (RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-847-for-862.md](docs/archive/log/LOG-rotate-847-for-862.md).

## 2026-08-16 — Coach week arrives as a visible diff (`.861`)

H-02: the adapt banner asserted that the week changed. The proposal is the
session-count change `adaptPlan` already made, plus the stored set
`logCitation` already quotes. Same Today slot. TAP_BUDGET holds.

**Ship:** `weekDiff.ts` · `CoachAdaptBanner` · `todayBlockBudget` instrument.

Label `.861` (onto master `.860`).

Excellence-Override: Coach week visible diff (Today/Coach banner; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-846-for-861.md](docs/archive/log/LOG-rotate-846-for-861.md).

## 2026-08-16 — H-02 is on the queue (`.860`)

Next `idea:next`. Now AR is `IL-H-02` — Coach week as a visible diff.
Do not invent AR2.

**Ship:** GRAPH_LOOP AR · LEDGER harvest-6.

Label `.860` (onto master `.859`).

Excellence-Override: queue paste, no product surface (RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-845-for-860.md](docs/archive/log/LOG-rotate-845-for-860.md).

## 2026-08-16 — Adapt banner needs a material week (`.859`)

H-08: a weekly plan generates a weekly diff, and a consent surface that
arrives regardless of content stops being read. Array order is not a
change. Dropping a session's exercises still is.

**Ship:** `sessionsMateriallyEqual` in `adapt.ts`.

Label `.859` (onto master `.858`).

Excellence-Override: coach adapt predicate (no athlete chrome; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-844-for-859.md](docs/archive/log/LOG-rotate-844-for-859.md).
