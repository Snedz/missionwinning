...
## 2026-08-13 — Visibility + Under the Hood weights (`.729`)

Account → **Visibility** (`/account/transparency`): N limits apply, each check
named with the exact reason (logger open · PRIVATE_MODE · region · Coach skip ·
score hidden · Bundle notify-only). Account → **Under the Hood**
(`/account/under-the-hood`): dark tabular BOOSTS / PENALTIES. BOOSTS = live
`rewards/catalog.ts` plus Club v1 planned (session, coach-plan). PENALTIES =
report / mute / block / hide as visibility filters — they do not debit points.
Download JSON + text includes weights and labels on this athlete. Help: replies
from people you trained with beat likes.
