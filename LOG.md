...
Latest first. Older entries: [docs/archive/log/LOG-rotate-1004-for-1019.md](docs/archive/log/LOG-rotate-1004-for-1019.md) (`.1004`). Prior rotate: [docs/archive/log/LOG-rotate-1003-for-1018.md](docs/archive/log/LOG-rotate-1003-for-1018.md) (`.1003`). Prior rotate: [docs/archive/log/LOG-rotate-1002-for-1017.md](docs/archive/log/LOG-rotate-1002-for-1017.md) (`.1002`). Prior rotate: [docs/archive/log/LOG-rotate-1001-for-1016.md](docs/archive/log/LOG-rotate-1001-for-1016.md) (`.1001`). Prior rotate: [docs/archive/log/LOG-rotate-1000-for-1015.md](docs/archive/log/LOG-rotate-1000-for-1015.md) (`.1000`). Prior rotate: [docs/archive/log/LOG-rotate-999-for-1014.md](docs/archive/log/LOG-rotate-999-for-1014.md) (`.999`). Prior rotate: [docs/archive/log/LOG-rotate-998-for-1013.md](docs/archive/log/LOG-rotate-998-for-1013.md) (`.998`). Prior rotate: [docs/archive/log/LOG-rotate-997-for-1012.md](docs/archive/log/LOG-rotate-997-for-1012.md) (`.997`). Prior rotate: [docs/archive/log/LOG-rotate-996-for-1011.md](docs/archive/log/LOG-rotate-996-for-1011.md) (`.996`). Prior rotate: [docs/archive/log/LOG-rotate-995-for-1010.md](docs/archive/log/LOG-rotate-995-for-1010.md) (`.995`). Prior rotate: [docs/archive/log/LOG-rotate-994-for-1009.md](docs/archive/log/LOG-rotate-994-for-1009.md) (`.994`). Prior rotate: [docs/archive/log/LOG-rotate-993-for-1008.md](docs/archive/log/LOG-rotate-993-for-1008.md) (`.993`). Prior rotate: [docs/archive/log/LOG-rotate-992-for-1007.md](docs/archive/log/LOG-rotate-992-for-1007.md) (`.992`). Prior rotate: [docs/archive/log/LOG-rotate-991-for-1006.md](docs/archive/log/LOG-rotate-991-for-1006.md) (`.991`). Prior rotate: [docs/archive/log/LOG-rotate-989-for-1005.md](docs/archive/log/LOG-rotate-989-for-1005.md) (`.989`).
...
## 2026-08-26 — Never-trained is not overdue (`.1019`)

Anatomy painted `daysSince >= 7`
as overdue. Never-trained is
`days = 99`, so Chest they never
logged read as a missed session.

**Ship:** anatomy overdue only if
days ≥ 7 and days is not the 99
never-trained sentinel. Window
session count is not the trained
flag (14-day empty still overdue
if they trained that group).
Never-trained stays idle — not
red, not “· overdue”. Score keeps
the 99 sentinel. Grid heatmap idle
stays. Guest. First set ungated.
Today still one Start. Month they
own `.1018` stays. Empty-load
`.1017` stays. `/private` stays the
tight lock. No `PRIVATE_MODE` flip.

Label `2026.07-unified.1019` (from
master `.1018` / `b376d4ecd`).
Stamp stays `.1019`. Live www
stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1004-for-1019.md](docs/archive/log/LOG-rotate-1004-for-1019.md).


## 2026-08-26 — Month they own (`.1018`)

The History month grid already marks
trained days. A tap could not open
that day's live rows. Session count
is a fact already on the log. A fire
count is not.

**Ship:** History Show-all month —
tap a live day, see that day's live
rows, open the row they already have.
Tombs stay unmarked and out of the
day list. Start-from fold never
erases the month. Search stays a
query. Charts stay the full live
diary. Empty month / empty day
invents nothing. History only. Guest.
First set ungated. Today still one
Start. Empty-load cite `.1017`
stays. `/private` stays the tight
lock. No `PRIVATE_MODE` flip.

Label `2026.07-unified.1018` (from
master `.1017` / `8a606a12`). Stamp
stays `.1018`. Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1003-for-1018.md](docs/archive/log/LOG-rotate-1003-for-1018.md).


## 2026-08-26 — Last cite is BW, not 0, on empty load (`.1017`)

Next already prints `8 × BW` on empty
load. Last / Prev still interpolated
`8 × 0`.

**Ship:** weight-type empty load Last /
Prev / after-complete print `8 × BW`.
Loaded stays `5 × 100`. Custom stays
type weight. Display only — store still
`weight: 0`. Guest. First set ungated.
Today still one Start. Session file
`.1016` stays. `/private` stays the
tight lock. No `PRIVATE_MODE` flip.

Label `2026.07-unified.1017` (from
master `.1016` / `fa96397d5`). Stamp
stays `.1017`. Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1002-for-1017.md](docs/archive/log/LOG-rotate-1002-for-1017.md).


## 2026-08-26 — This session as a file they own (`.1016`)

Export this diary writes the whole
live diary. Tuesday's receipt is
still trapped in History.

**Ship:** History detail saves THAT
session as a local CSV (JSON if the
same rows). Reuses `decideExportDiary`
columns. The file re-imports via
`.1013`. Empty / missing / tomb
invents nothing — Save disabled.
Honest logged columns only. No
public URL, Feed, share, email, or
clipboard permalink. History only.
Guest. First set ungated. Today
still one Start. Assisted cite
`.1015` stays. `/private` stays the
tight lock. No `PRIVATE_MODE` flip.

Label `2026.07-unified.1016` (from
master `.1015` / `4046afd9`). Stamp
stays `.1016`. Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1001-for-1016.md](docs/archive/log/LOG-rotate-1001-for-1016.md).


## 2026-08-26 — Next cite is BW, not 0 kg, on assisted 0 (`.1015`)

BW cite `.1009` closed vest-0.
Assisted 0 still printed `8 × 0 kg`.

**Ship:** live Next, Last ghost, and
after-complete cite unassisted help as
`8 × BW`. Help still `8 × −20 kg`.
Missing invents nothing. Display only —
store still `weight: 0`. Reuses
`formatSetRowLine` / `formatSetRowPrev`.
Custom prefix stays weight. Guest.
First set ungated. Today still one
Start. Duration cite `.1014` stays.
`/private` stays the tight lock. No
`PRIVATE_MODE` flip.

Label `2026.07-unified.1015` (from
master `.1014` / `f4b852279`). Stamp
stays `.1015`. Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1000-for-1015.md](docs/archive/log/LOG-rotate-1000-for-1015.md).


## 2026-08-26 — Next cite is 0:45, not mute (`.1014`)

Prev already says 0:45 for a plank.
Next / Last / after-complete still
dropped the hold, so the cite stayed
mute.

**Ship:** live Next, Last ghost, and
after-complete cite a logged hold as
`0:45`. Missing invents nothing. Copy
last hold — do not add a second. Guest.
First set ungated. Today still one
Start. Import `.1013` stays. `/private`
stays the tight lock. No `PRIVATE_MODE`
flip.

Label `2026.07-unified.1014` (from
master `.1013` / `3a28b75fa`). Stamp
stays `.1014`. Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-999-for-1014.md](docs/archive/log/LOG-rotate-999-for-1014.md).
