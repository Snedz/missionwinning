...
## 2026-09-14 — Dependabot leftovers batch (`.1067`)

Cursor chore so raw Dependabot PRs are not squash-merged
(Actions + Vercel Hobby). One lockfile.

Take: `stripe` 22.5.0 → 22.6.2 (#921) and
`@playwright/test` + `playwright` 1.62.1 → 1.63.0
(#923, closed unmerged). Pin Stripe `apiVersion`
to `2026-08-26.dahlia` in `stripeServer.ts` only.
No new checkout / portal / webhook code.

Already on master before this paper: #919
react-i18next 17.0.13, #922 next 16.3.4.
Skip #920 eslint 10 major. No Tailwind 4.

Paper only. `[skip vercel]`. No tip-promote.
Live www stays `.697`. PRIVATE_MODE stays.
