...
## 2026-08-15 — Victory honor next sits in the overlay footer (`.840`)

Radix Dialog `max-h-[90dvh] overflow-y-auto` put Session locked's NEXT
in the scroll body at `z-50` — the same layer as the tab bar. At 390×844
...
## 2026-08-15 — Empty `/active` Start is honest empty (`.839`)

Equipment on the device made the Train dock say Start Just Go and seed
template loads. Logger-depth empty-start never reached a blank set row.

**Ship:** dock Start workout → `startEmptyWorkout`. I-Day preview stays on
Welcome. Playwright `/log` `/active` waits use hydration, not networkidle.
