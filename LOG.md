...
line was a calendar gap and fired with no plan.

**Ship:** a planned miss on Today/Coach offers Do it
now, Skip, or Slide. Skip removes the session — it
does not stamp Missed or invent a fail identity.
No streak theater. Train set row untouched.

Label `.945` (past master `.944`).

Rotated LOG oldest → [docs/archive/log/LOG-rotate-925-for-945.md](docs/archive/log/LOG-rotate-925-for-945.md).

## 2026-08-24 — E-Victory receipt: vs-last same shape (`.944`)

Victory after Finish already existed. Session
totals compared last time by workout name, so a
renamed week stayed quiet and a same-named cardio
day invented a vs-last.

**Ship:** session vs-last matches sorted unique
lift ids. Empty history stays empty — no fake PR.
One prior same-shape session fills the receipt.
Sheet reused. Today untouched. No feed.

Label `.944` (stacked on `.943`).

Rotated LOG oldest → [docs/archive/log/LOG-rotate-924-for-944.md](docs/archive/log/LOG-rotate-924-for-944.md).

## 2026-08-24 — Strong CSV export: round-trip (`.943`)

Export already lived on Account as session/set
buttons. Empty history toasted and refused a file.

**Ship:** Strong session CSV downloads on click.
Empty history is header-only. One imported fixture
round-trips without invented sets. Skipped import
rows stay skipped. No account. Offline.

Label `.943` (past master `.942`).

Rotated LOG oldest → [docs/archive/log/LOG-rotate-923-for-943.md](docs/archive/log/LOG-rotate-923-for-943.md).

## 2026-08-24 — Gated www is the four-scene field (`.942`)

`.933` locked the words. Cold visitors still got a
signup sheet. `CinematicWww` already ported N1 and
was dead to the live door.

**Ship:** `/private` mounts SET → ANYWHERE → WEEK →
DOOR. Door slot is Get notified + Enter with code.
Brand pack stays master’s: Log a set. Offline.
`/today` and `/train` 308 before the gate. `/notify`
is the Super Bundle form. Cookie `/` stays `.696`.
No `PRIVATE_MODE` flip.

Label `.942` (past master `.941` after #780; do not
keep `.935`).

Rotated LOG oldest → [docs/archive/log/LOG-rotate-922-for-942.md](docs/archive/log/LOG-rotate-922-for-942.md).
...
## 2026-08-24 — Strong CSV import: preview + confirm (`.940`)

The `.180` parser already turned an English Strong-app
export into local history. Drop wrote immediately. A
bad row was counted in the parser and then hidden.

**Ship:** pick shows a dry-run preview (workouts, sets,
skipped rows). Confirm writes. Cancel does not. Empty
file and header-only stay errors. A second file still
adds. No account. Offline.

Label `.940` (past master `.939` after #777 + Dependabot;
do not steal `.935` #778 or `.938` #780).

Rotated LOG oldest → [docs/archive/log/LOG-rotate-920-for-940.md](docs/archive/log/LOG-rotate-920-for-940.md).
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-919-for-939.md](docs/archive/log/LOG-rotate-919-for-939.md).

## 2026-08-24 — F-008 gated www honesty (`.938`)

`#774` took `.934` on master. `#776` still owns the
door strings. This PR is leftover F-008 honesty:
MarketingNav / Welcome while gated, locale inherit,
help copy.

**Ship:** door stays Free / Log a set. Offline. /
No account. No wearable. Not Free beta. No
`PRIVATE_MODE` flip.

Label `.938` (past `.934`; skips `#778` `.935` ·
`#777` `.936` · `#779` `.937`).

Rotated LOG oldest → [docs/archive/log/LOG-rotate-918-for-938.md](docs/archive/log/LOG-rotate-918-for-938.md).

## 2026-08-24 — Coach why-this-session (`.934`)

Reserved `.699` concern (F-012 / F-002 session why-line).
This is PR #774 (minted as `.932` on `.930`). Rebased
onto `.933` brand so the door stays Free / Log a set.
Offline.

#478's engine was already on master via the merge-all that
closed it as “superseded by #544 / `.766`”. `.766` is
`logCitation`. The gap was silent empty: whyKey stories
could say “your logs” with zero workouts.

**Ship:** `session-empty` on a known empty history. Boss
`PlanSessionCard` cites inputs · rule · effect when logs
exist, or one quiet honest line when they do not. Coach
stays skippable. Why stays free. No E-Adjacency.

Label `.934` (stacked on `.933`). This is the `.699` concern.
Door copy stays `.933`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-917-for-934.md](docs/archive/log/LOG-rotate-917-for-934.md).

## 2026-08-24 — Brand copy lock (`.933`)

The name is Mission Winning. The company line was
still Train Anywhere. Win Daily. on the door, the
cinematic kicker, and SEO chrome. Tests asserted the
retired line.

**Ship:** public line Log a set. Offline. Support
No account. No wearable. Door Free · Enter with
code · Get notified. Alpha 0.1.0 stays off `/private`.
Honesty tests ban invite-only / Free beta / Win
Daily-as-tagline on the door.

Label `.933` (stacked on `.930`; `.931` honesty and
`.932` why-this-session are in-flight). This PR is
the source of the new door strings.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-916-for-933.md](docs/archive/log/LOG-rotate-916-for-933.md).

## 2026-08-24 — One set-count for a completed log (`.930`)

Today highlights and Victory receipt each had a private
`setCount`. Same domain type, two bodies.

**Ship:** `countCompletedLogSets` is the one helper.
Both readers import it.

Label `.930` (stacked on `.929`).

Rotated LOG oldest → [docs/archive/log/LOG-rotate-915-for-930.md](docs/archive/log/LOG-rotate-915-for-930.md).

## 2026-08-24 — Guards match the product (`.929`)

The cheap merge bar was red on master: LearnVs
`never`, a Journal grep from before `.926`, a mic the
Talk path does not have, and harness reading SMOOTHER
out of a `ready-for-founder` sentence.

**Ship:** `otherLearnVsPages` takes a string. Journal
empty greps the shipped debrief. Composer has no mic.
`ready-for-founder` first means role is null.

Label `.929` (stacked on `.928`).

Rotated LOG oldest → [docs/archive/log/LOG-rotate-914-for-929.md](docs/archive/log/LOG-rotate-914-for-929.md).
