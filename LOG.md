...
## 2026-08-13 — Multicultural form stills; representation ≠ territory (`.736`)

Founder correction: multicultural stills stay; regional targeting follows the
geo-block. Cast is diverse (race, religion, body, modest dress, kippah, hijab
as a person in a **served** country). Country availability is
`src/lib/legal/supportedRegions.ts` — not the pictures.

**Ship:** `docs/MEDIA.md` representation contract (`representation ≠ service
territory`). Catalog 006 `squats` plus `push-ups` / `pull-ups` 4-still cast
sets; `hash(exerciseId) % n` picker in `formCast.ts`. Captions stay movement-only.
Cast packs are still-only (do not pair a new poster with the old `athlete-a`
loop). Learn heroes and landing `GrayscalePhoto` placeholders left (no people
default). Remaining raster packs stay athlete-a until recast. No blocked-country
launch copy in alt / filenames / www heroes. Free logger ungated. No
`PRIVATE_MODE` flip.

Label `.736` (onto master `.697`). Excellence-Override below.

Excellence-Override: multicultural form stills / representation≠territory (RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-669-for-736.md](docs/archive/log/LOG-rotate-669-for-736.md).
