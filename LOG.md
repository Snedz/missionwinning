...
## 2026-08-13 — Five-event field test + published-table score (`.711`)

Free logger session: 3RM deadlift, hand-release push-up, sprint-drag-carry,
plank, 2-mile run. Public name **Five-event field test** / Field test. Reach
from Train empty (`?fieldTest=1`) and Victory “Run again” — not a tab, not
Today Quick options, not Bundle.

Optional 0–100 points per official event from the published ACFT Scoring Scales
(23 March 2022). No interpolation. Garage SDC is **unscored** and drops the
0–500 total. vs-last is this field test vs the last field test only. No
leaderboard, no MW-user percentile, no recruiter copy.

Label `.711` (past master `.697`; left `.698`–`.710`, including plate math #503 `.708`).

Excellence-Override: free five-event field test (logger)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-669-for-711.md](docs/archive/log/LOG-rotate-669-for-711.md).
