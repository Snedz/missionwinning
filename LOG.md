...
Latest first. Older entries: [docs/archive/log/LOG-rotate-970-for-992.md](docs/archive/log/LOG-rotate-970-for-992.md) (`.970`). Prior rotate: [docs/archive/log/LOG-rotate-967-for-991.md](docs/archive/log/LOG-rotate-967-for-991.md) (`.967`).
...
## 2026-08-25 — Cues on the movement (`.973`)

The live set already had a Form guide
sheet behind Info. Setup lived one tap
away. Week-4 dies when they do not know
the setup and abandon the set.

**Ship:** opening the live exercise
shows a short written cue list (setup
first, then execute, cap 3). Optional
still only from media we already have.
Hide never blocks Log set. Full guide
stays behind Info. Not a clip feed.
Not on Today. Guest. First set ungated.
Today still one Start. Resume `.963`
kept. Honesty `.971` / tags `.970` /
RPE `.967` / Fuel `.965` stay.
`/private` stays the tight lock. No
`PRIVATE_MODE` flip. Skips `.972`.

Label `.973` (past master `.971`). Live
www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-952-for-973.md](docs/archive/log/LOG-rotate-952-for-973.md).

## 2026-08-25 — Thin-history honesty (`.971`)

Cite, Wednesday, and the quiet week strip
looked like a progression product after
one or two sessions. Week-1 Strong
migrants have a notebook, not a dataset.

**Ship:** Wednesday does not invent a next
day from thin logs. If there is not
enough diary, it says so and does not
overwrite a saved notebook. Week strip
empty days stay empty — no streak, no
“on track”, no consistency score from
1–2 sessions. Empty cite has no Start
(notebook `.960` wins). Set-row empty
copy and resume / finish-partial stay.
Tags `.970` stay. RPE `.967` stays.
Fuel stays off Today. Guest. First set
ungated. Today still one Start.
`/private` stays the tight lock. No
`PRIVATE_MODE` flip. Skips `.972`.

Label `.971` (past master `.970`; title may
stay `.964`). Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-951-for-971.md](docs/archive/log/LOG-rotate-951-for-971.md).
