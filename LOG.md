...
## 2026-08-25 — Export this diary (`.1011`)

Import + backfill dumped years.
Search finds a row. Missing was a
file they can take.

**Ship:** History overflow saves
**their** live diary as CSV. JSON is
the same rows. Empty invents nothing
— Save stays off. Tombs stay out.
Start-from does not shrink the file.
Guest. First set ungated. Today still
one Start. Library skip `.1010` stays.
`/private` stays the tight lock. No
`PRIVATE_MODE` flip.

Label `2026.07-unified.1011` (from
master `.1010` / `4d128292d`). Stamp
