...
- Account export/delete take the id from `getUser` only; delete body `.strict()`
  so a client-chosen `userId` does not parse.
- Guest push cannot overwrite a row already linked to another account.
- Analytics strips pregnancy / PT / injury flag keys before PostHog.
- Logger/journal notes stay React text children; HTML reports use shared
  `escapeHtml`. `sanitizeNextPath` no longer accepts `..` or open class prefixes.

Tests assert secure behavior (unauthenticated mint fails, blocked ISO denied).
No PoCs. `PRIVATE_MODE` unchanged. Excellence-Override: blue-team P0 harden.
