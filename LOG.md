...
## 2026-08-05 — Train extract: activeSetDial peel (`.504`)

Framework review backlog #1: peel getSetInput / Apply-targets wiring out of ActiveWorkoutPage into `activeSetDial.ts` (prescribed history null, `setDialTemplateDefaults`, `planExerciseTargetPatches`). Page 686→651. Pins + mutants for dial order and `.206` hardcoded defaults.

Mutants: page calls `resolveActiveSetDial` again → pin red; prescribed passes lastSets → dial test red; `set?.reps ?? 10` inline → pin red.
