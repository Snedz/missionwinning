...
Latest first. Older entries: [docs/archive/log/LOG-rotate-980-for-999.md](docs/archive/log/LOG-rotate-980-for-999.md) (`.980`). Prior rotate: [docs/archive/log/LOG-rotate-978-for-998.md](docs/archive/log/LOG-rotate-978-for-998.md) (`.978`).
...
## 2026-08-25 — Private session notes (`.983`)

The live logger already jotted a field
note in Show all, then siphoned it into
the journal and left the completed log
without it. Close receipt kept sets and
vs-last. Missing: one optional note
stored **with the session**.

**Ship:** optional notes on the live
session and the close receipt.
Strong-style: add notes if you have
more to record. Empty invents nothing.
Stored locally with the session. Cloud
upsert and desk→gym snapshot stay
without it. Not a Feed. No public URL.
No comments. No likes. Guest. First set
ungated. Today still one Start. Resume
`.963` kept. 1RM `.981` / Supersets
`.980` / Learn `.978` / week strip
`.977` / Track `.976` / Move `.974` /
cues `.973` / honesty `.971` / tags
`.970` / RPE `.967` / Fuel `.965` stay.
`/private` stays the tight lock. No
`PRIVATE_MODE` flip.

Label `.983` (concern `.982`; past
master `.981` / `81b23935`). Live www
stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-959-for-983.md](docs/archive/log/LOG-rotate-959-for-983.md).

## 2026-08-25 — 1RM percent (`.981`)

Notebook programs already talk
percent. The live set row only took
kg × reps. Educational Epley is a
formula estimate, not a tested max.

**Ship:** optional percent field on
the live set row / next-set cite.
Type 80% and get a load when a
known 1-rep max exists, or show
percent of that max next to the
target. Empty invents nothing. No
invented max from one set. No
autoreg-as-identity. Guest. First
set ungated. Today still one Start.
Resume `.963` kept. Supersets `.980`
/ Learn `.978` / week strip `.977`
/ Track `.976` / Move `.974` / cues
`.973` / honesty `.971` / tags `.970`
/ RPE `.967` / Fuel `.965` stay.
`/private` stays the tight lock. No
`PRIVATE_MODE` flip.

Label `.981` (past master `.980` /
`17039cb3`). Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-958-for-981.md](docs/archive/log/LOG-rotate-958-for-981.md).
