...
## 2026-09-14 — Feature flags console (`.1067`)

Founder-only staged rollouts. Closed catalog in code
(`example_staged`, default 0%). Postgres overrides plus
percent / allowlist / kill. Kill beats allowlist. Raising
percent is monotonic; lowering can drop people. Console
`/account/flags` from Account owner tools. Athlete
`GET /api/flags` is a boolean map for this subject only.
Missing table is 503 `flags_unavailable`, never an empty
list. Train never imports the domain. `PRIVATE_MODE` and
surface parking stay deploy-time.
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-1048-for-1067.md](docs/archive/log/LOG-rotate-1048-for-1067.md) (`.1048`).
