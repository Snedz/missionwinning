...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-864-for-879.md](docs/archive/log/LOG-rotate-864-for-879.md).

## 2026-08-16 — Victory said 0 kg for push-ups (`.878`)

`calculateVolume` is load×reps. A bodyweight working set is 0, so the first
Just Go chest session printed **0 kg** next to one real set. That is a
receipt that says the athlete did nothing.

**Ship:** `victoryVolumeStat` — load volume when anything was loaded, working
reps (warmup excluded) when volume is 0. Victory cell + share card use it.
`0 kg` on a session with countable reps goes red.

Label `.878` (onto `.877`).

No `PRIVATE_MODE` flip.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-863-for-878.md](docs/archive/log/LOG-rotate-863-for-878.md).

## 2026-08-16 — Victory Next sat under the fold (`.877`)
...
Label `.877` (onto master `.876`). Measured after log: button top 708, in view,
above the tab bar.

No `PRIVATE_MODE` flip.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-862-for-877.md](docs/archive/log/LOG-rotate-862-for-877.md).
