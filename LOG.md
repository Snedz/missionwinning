...
## 2026-08-13 — Pregnancy + miscarriage safety v1 (counsel-hold, symptom line only) (`.752`)
...
never a logger gate). Quiet Account control under More settings only. Flag-off
deletes the key. **The flag changes exactly one product behavior:** which
stop-symptoms line appears on the hard-session warning sheet (bleeding /
cramping / dizzy when on; #519 line when off). Coach prescriptions and
max-effort CTAs are unchanged in this PR (follow-on). Combined Terms
educational paragraph. Copy-guard forbids “safe for pregnancy” / “prevents
miscarriage” product claims. Free logger ungated. Cause-talk refusal copy is
draft / not-for-prod — not wired into Coach.

Label `.752` (past master `.751`; `.746` was this branch’s first mint while master was still `.697`). Excellence-Override
below.
