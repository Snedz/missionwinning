...
`leaves an explicit next alone` (it asks `/active?next=/coach` about a route that
`.769` had just stopped redirecting) and the sibling test pinning hard rule 2 at
the proxy. So `.769` shipped with `npm run test:routes` red. Both are restored
here. The lesson is not "resolve conflicts more carefully" — it is that
`test:routes` is a separate command from `npm test` and I ran only the latter
after the re-land; the full gate runs both, and I skipped it because the ship
looked like docs.

**Still terrible, next:** `/active` — the free logger, and now where the gate's
CTA lands — server-renders 113 characters, of which the content area is the word
*"Loading Workout…"*. Same defect class as `.765`'s "Checking sign-in…", on the
wedge. `/learn` paints 102.

Label `.770` (onto master `.769`). Excellence-Override: www kaizen first-paint.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-753-for-770.md](docs/archive/log/LOG-rotate-753-for-770.md).
