...
Latest first. Older entries: [docs/archive/log/LOG-rotate-976-for-996.md](docs/archive/log/LOG-rotate-976-for-996.md) (`.976`). Prior rotate: [docs/archive/log/LOG-rotate-974-for-995.md](docs/archive/log/LOG-rotate-974-for-995.md) (`.974`).
...
## 2026-08-25 — Supersets (`.980`)

A named-app set log treats a superset as a
group, not a tag. Pair-of-two already lived
on the logger. After tags, RPE, and cues,
leftover in-set grammar: pair movements so
rest + vs-last do not treat A2 as a new
exercise. Circuits they already run stay
on the notebook.

**Ship:** two or more exercises group as
one superset in the live log. Rest after
the last peer in the round, on the first
peer's rest. vs-last stays per movement.
After-complete cite is quiet mid-round.
Save / Start / Repeat last / Finish-partial
keep the group when they had one. Empty
invents nothing. Optional. Guest. First
set ungated. Today still one Start. Resume
`.963` kept. Week strip `.977` / Track
`.976` / Move `.974` / cues `.973` /
honesty `.971` / tags `.970` / RPE `.967`
/ Fuel `.965` stay. Learn `.978` stays.
`/private` stays the tight lock. No
`PRIVATE_MODE` flip.

Label `.980` (past master `.978` / `1a06fbd7`;
concern `.979`). Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-957-for-980.md](docs/archive/log/LOG-rotate-957-for-980.md).

## 2026-08-25 — Quiet Learn (`.978`)

Learn first paint was a ten-path
catalog. First success already lived
in Strength Basics `sb-0` — log a
set, then Coach from those logs.

**Ship:** `/learn` first paint is
that existing intro. Other paths,
guidebook, and sample fold under
Show more. Cue list can open Learn
when they want more than a rack
card. Empty invents nothing. Not on
Today. Guest. First set ungated.
Today still one Start. Resume `.963`
kept. Quiet Track `.976` / Move
`.974` / cues `.973` / honesty
`.971` / tags `.970` / RPE `.967` /
Fuel `.965` stay. Week strip `.977`
stays. `/private` stays the tight
lock. No `PRIVATE_MODE` flip.

Label `.978` (past master `.977` /
`79791d8b`). Live www stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-956-for-978.md](docs/archive/log/LOG-rotate-956-for-978.md).

## 2026-08-25 — Week strip quiet row (`.977`)

Rest days on the quiet week strip were empty
on purpose. Fuel restock, Quiet Move, and
Quiet Track already lived on their own
surfaces. The rest-day hole had no quiet tap.

**Ship:** one tap on an empty week-strip day
can log one optional Fuel restock, easy walk,
or scale/tape. Empty invents nothing. Train
Done stays Train. Quiet rows do not score
thin history. Today still one Start. No
second home. No six-pillar dock. No streak X.
Guest. First set ungated. Resume `.963` kept.
Quiet Track `.976` / Quiet Move `.974` / cues
`.973` / honesty `.971` / tags `.970` / RPE
`.967` / Fuel `.965` stay. `/private` stays
the tight lock. No `PRIVATE_MODE` flip.

Label `.977` (past master `.976`). Live www
stays `.696`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-955-for-977.md](docs/archive/log/LOG-rotate-955-for-977.md).
