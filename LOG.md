...
## 2026-08-14 — In-app copy is Alpha, not Open beta (`.804`)

The stamp is Mission Winning Alpha 0.1.0. Profile, Today, Coach,
Fuel, Move, Learn still said “open beta.” `isFreeBeta()` is mute-pay,
not the name.

**Ship:** defaultValues + EN locale floors → Alpha. Honesty test.
Flag unchanged.

Label `.804` (onto master `.803`).

Excellence-Override: Alpha copy leftover (surface; RESULT unscored)

## 2026-08-14 — Planned rest bridges a streak (`.803`)

A rest day was a broken consecutive-day streak. Weekly goal is the
boss. Rest is part of the plan.

**Ship:** `plannedRest.ts` walks across rest without counting it.
Today Mission progress has a ghost Rest today. No second red.

Label `.803` (onto master `.802`).

Excellence-Override: planned rest (surface; RESULT unscored)

## 2026-08-14 — Programs is not a second store (`.802`)

FREE_BETA already hid $497 and UnlockButton. The page still needed
to say these cards are education outlines, not a checkout, and to
point at Mission Coach.

**Ship:** Coach pointer + not-a-store line. Coming soon stays on the
paid foot only.

Label `.802` (onto master `.801`).

Excellence-Override: Programs merch honesty (surface; RESULT unscored)

## 2026-08-14 — In-app Help FAQ (`.801`)

`docs/help/faq.md` existed. The app had no Help route. C4 is `/help`
plus a footer link. School/PFT answers stay out (parked track).

**Ship:** `helpFaq.ts` + `HelpPage` + `/help`. Logger never gated.

Label `.801` (onto master `.800`).

Excellence-Override: in-app Help/FAQ (surface; RESULT unscored)

## 2026-08-14 — Track does not sell a live strap (`.800`)

Wearables are parked. Track still labeled a disclosure “Body, wearables
& trends,” which reads as a live strap. GPS and a typed walk are the
live Track.

**Ship:** disclosure is Body & trends. Quiet no-strap line. Card still
null when the flag is off.

Label `.800` (onto master `.799`).

Excellence-Override: Track strap honesty (surface; RESULT unscored)

## 2026-08-14 — Astro `/compare` index; Next keeps the stories (`.799`)

www needed a compare index. The three vs-pages stay on Next
(`/guide/mission-winning-vs-*`). This page only points at them.

**Ship:** `compare.astro` + `compareContent.ts`. No fourth competitor.
No `_redirects`. No host steal.

Label `.799` (onto master `.798`).

Excellence-Override: commissioned www compare index (surface; RESULT unscored)

## 2026-08-14 — Astro `/vision` only; Next keeps the host (`.798`)

`sites/www` had index, start, week, about. No vision page. C1 is that
page, additive. Next keeps `www.missionwinning.com` and Next `/vision`.

**Ship:** `vision.astro` + `visionContent.ts`. Same English the Next
vision page already paints. No `_redirects`. No national-fitness track.

Label `.798` (onto master `.797`).

Excellence-Override: commissioned www vision page (surface; RESULT unscored)

## 2026-08-14 — Today Coach week shows dose + adapt (`.797`)

Coach already painted this week's dose and the adapt banner. Today's
week strip was days only — so the week "felt earned from logs" only
after opening `/coach`. G6 is that evidence on Today too.

**Ship:** `TodayCoachWeekStrip` mounts `summarizeWeekDose` + compact
`CoachAdaptBanner`. No second red. Lean first-paint stays off Coach.

Label `.797` (onto master `.796`).

Excellence-Override: Coach week dose+adapt on Today (surface; RESULT unscored)
...
## 2026-08-14 — Astro `/about` only; Next keeps the host (`.795`)

`sites/www` had index, start, week. No about page. G3 is that page,
additive. Next keeps `www.missionwinning.com`, the ~250 SEO URLs, and
Next `/about`. No `_redirects`. No national-fitness track.

**Ship:** `about.astro` + `aboutContent.ts`. Same English the Next about
already paints. Terminal CTA. Link home. No `PRIVATE_MODE` flip.

Label `.795` (onto master `.794`).

Excellence-Override: commissioned www about page (surface; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-781-for-795.md](docs/archive/log/LOG-rotate-781-for-795.md).
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-780-for-794.md](docs/archive/log/LOG-rotate-780-for-794.md).

## 2026-08-14 — Landing Reveal paints; photo slots are real (`.793`)

After Done, `/` is the `.696` landing. Below-fold bands (When you miss /
Where you train / The free core) lived in the DOM at opacity 0 until
IntersectionObserver fired. Full-page capture never intersects, so the
founder saw blank paper. The three stills were already in `public/photo/`
and already passed as `GrayscalePhoto` `base` — the empty captions were
the hidden band, not missing files.

**Ship:** `.reveal` paints at opacity 1. No restyle. No third red on `/`.
No `PRIVATE_MODE` flip.

Label `.793` (onto master `.792`).

Excellence-Override: landing Reveal was blank paper (surface; RESULT unscored)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-779-for-793.md](docs/archive/log/LOG-rotate-779-for-793.md).

## 2026-08-14 — www gate, Alpha 0.1.0 (`.792`)

Localhost painted Log a set as the red and sent `/welcome`, which
skips the door. Live www is waitlist-first. Invite-only / beta
language is the old stamp.

**Ship:** drop the `/welcome` poster. Notify me is the one red. Eyebrow
and waitlist copy are Alpha 0.1.0, not invite-only. After Done still
the `.696` landing. Logger `/active` stays public. No `PRIVATE_MODE`
flip.

Label `.792` (onto master `.791`).

Excellence-Override: www gate, Alpha 0.1.0

Rotated LOG oldest → [docs/archive/log/LOG-rotate-778-for-792.md](docs/archive/log/LOG-rotate-778-for-792.md).

## 2026-08-14 — Homepage after Done is the `.696` landing (`.791`)

Preview sent Done to I-Day. The page the founder wants is production
from 2026-08-12 (`8398ae3b`, dpl `4fi6bsdkh`): MarketingNav, Log a set
/ Your week rewrites itself, squat demo. Not cinematic www.

**Ship:** restore that LandingPage. `/` unlocks from the signed cookie
only — `hasServerPrivateAccess` is true whenever the gate is off, so
Preview would have skipped the teaser. Done on Preview mints the
cookie. Production still checks codes. No `PRIVATE_MODE` flip.

Label `.791` (onto master `.790`).

Excellence-Override: restore .696 homepage after Done

Rotated LOG oldest → [docs/archive/log/LOG-rotate-777-for-791.md](docs/archive/log/LOG-rotate-777-for-791.md).

## 2026-08-14 — Preview gate is one door, and it opens (`.790`)

The teaser printed Get notified and the no-spam line twice (parent +
form). Done 401'd when Preview lacked the codes list; a 200 still
assigned `/`, which remounts the same teaser.

**Ship:** title/foot once. Ungated walk-open skips the session probe
and sends Done / any typed code to `/welcome`. Gated www still
validates. Logger never gated. No `PRIVATE_MODE` flip.

Label `.790` (onto master `.789`).

Excellence-Override: Preview gate copy + walk-open unlock

Rotated LOG oldest → [docs/archive/log/LOG-rotate-776-for-790.md](docs/archive/log/LOG-rotate-776-for-790.md).
