...
## 2026-08-13 — Mission Server messenger (MSN rooms) (`.752`)

Native Mission Server messenger on `/server` (More → You). Steal MSN’s product
(rooms + presence + simple window + local persist) — not Discord.com, not a
feed, not DMs. Continues #518’s local Garage store; does not restart it.

**Ship:** buddy list of rooms, ink chat window, Available / Away / Offline on
this device, display name (or Athlete) + optional Mission ID `00`–`99`. Messages
survive reload via `mw_mission_server`. Realtime is off unless flagged; fail
open to local. Isolation test: chat is not imported from Today or Train.
Free logger untouched. Preview will not deploy (`[skip vercel]`).

Label `.752` (onto master `.751`). Prompt asked `.748`; that label already
shipped as exercise notes. Do not steal `.697`–`.730`.
...
## 2026-08-13 — Learn vs Strong / Hevy / Fitbod citation pages (`.751`)

Original Learn explanation pages LLMs can cite: **Mission Winning vs Strong**,
**vs Hevy**, **vs Fitbod** at `/guide/mission-winning-vs-*`. Diataxis explanation;
free HTML; no paywall; not magazine chapters; not Growth’s docs-only #486.

Honest table: offline, account-lite, Coach-from-logs, no wearable, CSV import of
Strong/Hevy **live**, those export dialects **planned** (not on this build).
Founder intel: Strong free = 3 templates (not locked logs); Hevy social is free;
Fitbod 7-day trial (not a 3-workout lock). Logger never for sale.

Label `.751` (onto master `.750`). Originally reserved `.716`; landed as `.751` past master `.750`.
Excellence-Override below.

Excellence-Override: Learn vs-pages AEO

Rotated LOG oldest → [docs/archive/log/LOG-rotate-692-for-751.md](docs/archive/log/LOG-rotate-692-for-751.md).
## 2026-08-13 — Session history list (`.750`)

Free session history excellence: deepen the one true list on `/history`
Sessions. Not a feed, not the Athlete Page, not Victory vs-last.

**Ship:** `sessionHistoryList` projects date · title/muscles · set count from
device `workoutHistory` (tombstones omitted). Empty waits on persist hydrate
and stays an invitation (Open Today). Tap opens the completed log read-only
(existing details dialog). **Again** stays a separate control. Calendar and
`/history/[date]` unchanged. Cloud merge never gates the list. No Top 8,
comments, or streak-as-shame.

Label `.750` (onto master `.749`). Originally reserved `.720`; landed as `.750` past master `.749`.
Excellence-Override below.

Excellence-Override: session history list

Rotated LOG oldest → [docs/archive/log/LOG-rotate-691-for-750.md](docs/archive/log/LOG-rotate-691-for-750.md).
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-690-for-749.md](docs/archive/log/LOG-rotate-690-for-749.md).
## 2026-08-13 — Exercise notes on the set log (`.748`)

Strong buries the cue. The free logger already stored a per-exercise `note` and
could read last time’s line (`lastNotesFor`) — but the field sat behind overflow
and never prefilled. The athlete retyped “belt on 3” or never saw it.

**Ship:** appearance-time seed (`startWorkout` / add / swap) from the last
persisted cue; `''` is sticky this session and is not treated as unset. Always-
visible one-line field **after** the set rows so load / reps / RPE keep first
paint. No AI notes, no feed, no Top 8. Overflow Note focuses the field.

Label `.748` (onto master `.747`). Originally reserved `.718`; landed as `.748` past master `.747`.
Excellence-Override below.

Excellence-Override: exercise notes on the log

Rotated LOG oldest → [docs/archive/log/LOG-rotate-689-for-748.md](docs/archive/log/LOG-rotate-689-for-748.md).
## 2026-08-13 — Repeat last session from the log (`.747`)

Strong/Hevy migrants live on copying the last completed session. History already
had “Train this again” (`templateFromCompletedLog`); Today and Train empty Start
seeded Just Go instead. One control, not a template marketplace.

**Ship:** `repeatLastSessionTemplate` wraps the existing mapper (newest-first,
skip tombstones). Train empty Start copies that log or opens a blank logger.
Today primary: resume → live Coach (honesty unchanged) → Repeat last → Just Go.
Prefills compose with `getLastPerformanceForSet` / `resolveSetInput` (F-013/#489
untouched). No rest auto-start, no Coach on this control, no account/network,
no guilt copy when history is empty. Set-log table is first paint when exercises
are copied.

Label `.747` (onto master `.746`). Originally reserved `.717`; landed as `.747` past master `.746`.
Excellence-Override below.

Excellence-Override: repeat last session
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-684-for-746.md](docs/archive/log/LOG-rotate-684-for-746.md).
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-669-for-715.md](docs/archive/log/LOG-rotate-669-for-715.md).
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-679-for-744.md](docs/archive/log/LOG-rotate-679-for-744.md).
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-669-for-743.md](docs/archive/log/LOG-rotate-669-for-743.md).

## 2026-08-13 — Public GitHub pass: Beta 0.1 + mission (stars) (`.714`)

Prepare the product tree to go public on GitHub. Founder still owns visibility.
Agents did not flip `PRIVATE_MODE` or `FREE_BETA`. Public name is **0.1 (beta)** /
**Mission Winning 0.1 (beta)** / **Beta 0.1**. Internal `APP_BUILD_LABEL` is
`2026.07-unified.714` (`GET /api/health` still returns that, not `0.1 (beta)`).

**Mission (exact):** The mission is advancement of civilization and propagation of
consciousness to the stars. README first paint, About section 01, vision.md north-star
row. Nested: stars/civilization is the north star; L1 Health (Train + Coach) is how we
serve it today. Dual-repo honesty: this tree is the inspectable free core; war room,
secrets, EIN, and personal email stay out.

Public stamp independent of #488 (same `APP_PUBLIC_*` strings). CONTRIBUTING clone URL
is the real GitHub URL. Founder Settings one-pager: [docs/PUBLIC_GITHUB.md](docs/PUBLIC_GITHUB.md).

Label `.714` (skip occupied `.698`–`.713`). Excellence-Override below.
