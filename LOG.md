...
## 2026-08-13 — F-017 account-lite: log first set, defer signup (`.700`)

MatrAIx F-017: forced account / too-much-setup before trying. A cold invited
phone can log the first set with no account. Account is offered after the first
workout — never forced, never mid-set. PRIVATE_MODE / FREE_BETA / Stripe
untouched. Absorbs Day-1 F-020 (no device-link), F-028 (no plan wall), F-030
(fail-open already `.697`), F-033 (defer OS permission).
...
Excellence-Override: F-017 account-lite (log first set, defer signup)
