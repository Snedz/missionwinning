...
## 2026-08-27 — Signed-in desk names the three loops (`.1056`)

The lean Today hid WEEK in Show all.
The rail was a 13-item Mission /
Pillars / Toolkit list. Neither
taught LOG → WEEK → GARAGE.

**Ship:** visual signed-in desk.
Rail groups are Log (Today · Train ·
History) · Week (Coach) · Catalog
(Library · Builder) · You. Pillars
and `/server` stay in More. Fuel
stays reachable on I-Day; Move /
Mind / Track / Learn wait on first
workout. Today first-paints a Week
door to `/coach` (not the week
strip, not a second Start). Library
+ Builder share catalog state tabs.
History joins the social isolation
walk. `HomeTodayDashboard` stays
parked. One Start. Start order
unchanged. Dock still Summary +
Search; Train joins while live.
No Message tab. No costume. No
`/private` / www / LandingPage
diff. Guest. First set ungated.

Label `2026.07-unified.1056` (from
master `.1055` / `e44351c3`). Visual
preview hop — do not merge, do not
