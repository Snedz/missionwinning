...
## 2026-08-31 — PathShell, not House (`.1075`)

Signed-in chrome is PathShell
word nav (Today / Coach; Train
while live; History after
Finish). House floor unmounted.
Finish keeps the open session
clientId. Coach does not
auto-mint a week on empty
storage. Coach and History are
not I-Day walls. Log set stays
filled. `generateWeek` stays on
`/coach`. Park leftover PR #889.
Continue #890. `[skip vercel]`.
No `PRIVATE_MODE` flip. No
promote. Live www stays `.696`.

Label `2026.07-unified.1075`
(from this branch `.1074` /
`543823f4`). Stamp `.1075`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1060-for-1075.md](docs/archive/log/LOG-rotate-1060-for-1075.md).

## 2026-08-31 — Legal is the policy, not a legal-index tour (`.1074`)

Privacy / cookies / terms /
refunds / DMCA / usage / regions
/ service-terms / accessibility
first paint is jump chips +
sections. Legal footer is off
those pages. Legal copy
unchanged. Log set stays filled.
`generateWeek` stays on `/coach`.
Help stays the FAQ. Park leftover
PR #889. Continue #890. `[skip
vercel]`. No `PRIVATE_MODE` flip.
No promote. Live www stays `.696`.

Label `2026.07-unified.1074`
(from this branch `.1073` /
`81726238`). Stamp `.1074`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1059-for-1074.md](docs/archive/log/LOG-rotate-1059-for-1074.md).

## 2026-08-31 — Help is the FAQ, not a legal-index tour (`.1073`)

`/help` first paint is the FAQ.
Legal footer is off the page.
HELP_FAQ stays the object. Log
set stays filled. `generateWeek`
stays on `/coach`. Fuel stays
the log. Feedback stays the form.
Explore stays the board. Park
leftover PR #889. Continue #890.
`[skip vercel]`. No `PRIVATE_MODE`
flip. No promote. Live www stays
`.696`.

Label `2026.07-unified.1073`
(from this branch `.1072` /
`11deecbb`). Stamp `.1073`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1058-for-1073.md](docs/archive/log/LOG-rotate-1058-for-1073.md).

## 2026-08-31 — Explore is the board, not a GPS tour (`.1072`)

`/explore` first paint is the
board, pin list, and add a place.
GPS nearby, session tag, and maps
are off the page. Add a place
stays the writer. Log set stays
filled. `generateWeek` stays on
`/coach`. Fuel stays the log.
Feedback stays the form. Garage
stays quiet title. Park leftover
PR #889. Continue #890. `[skip
vercel]`. No `PRIVATE_MODE` flip.
No promote. Live www stays `.696`.

Label `2026.07-unified.1072`
(from this branch `.1071` /
`b10307dc`). Stamp `.1072`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1057-for-1072.md](docs/archive/log/LOG-rotate-1057-for-1072.md).

## 2026-08-31 — Garage is quiet title, not a messenger tour (`.1071`)

`/server` first paint is quiet
title. Rooms, buddy list, chat,
and presence are off the page.
Engine stays on disk. Log set
stays filled. `generateWeek`
stays on `/coach`. Fuel stays
the log. Feedback stays the form.
Coach never reads Garage. Park
leftover PR #889. Continue #890.
`[skip vercel]`. No `PRIVATE_MODE`
flip. No promote. Live www stays
`.696`.

Label `2026.07-unified.1071`
(from this branch `.1070` /
`7ea67a72`). Stamp `.1071`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1056-for-1071.md](docs/archive/log/LOG-rotate-1056-for-1071.md).

## 2026-08-31 — Feedback is the form, not a sign-in tour (`.1070`)

`/feedback` first paint is the form.
Sign-in and legal footer are off
the page. composeFeedbackNote
stays the writer. Log set stays
filled. `generateWeek` stays on
`/coach`. Fuel stays the log.
Learn stays the intro. Track
stays the scale/tape log. Park
leftover PR #889. Continue #890.
`[skip vercel]`. No `PRIVATE_MODE`
flip. No promote. Live www stays
`.696`.
...
(from this branch `.1069` /
`1909ca4f`). Stamp `.1070`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1055-for-1070.md](docs/archive/log/LOG-rotate-1055-for-1070.md).

## 2026-08-31 — Learn is the intro, not a paths tour (`.1069`)

`/learn` first paint is the intro.
Other paths, guidebook, sample
workout, and premium are off the
page. QuietLearnIntroCard stays
the intro. Log set stays filled.
`generateWeek` stays on `/coach`.
Fuel stays the log. Move stays
the quiet log. Mind stays the
check-in. Track stays the
scale/tape log. Park leftover PR
#889. Continue #890. `[skip
vercel]`. No `PRIVATE_MODE` flip.
No promote. Live www stays `.696`.

Label `2026.07-unified.1069`
(from this branch `.1068` /
`17e4fd6b`). Stamp `.1069`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1054-for-1069.md](docs/archive/log/LOG-rotate-1054-for-1069.md).

## 2026-08-31 — Track is the scale/tape log, not a walks tour (`.1068`)

`/track` first paint is the
scale/tape log. Walks, GPS,
import, wearables, and trends
are off the page. BodyMetricsCard
stays the writer. Log set stays
filled. `generateWeek` stays on
`/coach`. Fuel stays the log.
Move stays the quiet log. Mind
stays the check-in. Park leftover
PR #889. Continue #890. `[skip
vercel]`. No `PRIVATE_MODE` flip.
No promote. Live www stays `.696`.

Label `2026.07-unified.1068`
(from this branch `.1067` /
`7a3e4cca`). Stamp `.1068`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1053-for-1068.md](docs/archive/log/LOG-rotate-1053-for-1068.md).

## 2026-08-31 — Mind is the check-in, not a sessions tour (`.1067`)

`/mind` first paint is the check-in.
Breathe, Show all, sessions,
premium, and recent wins are off
the page. DailyCheckIn stays the
writer. Log set stays filled.
`generateWeek` stays on `/coach`.
Fuel stays the log. Move stays
the quiet log. You stays title +
Account. Account stays sign-in /
return / prefs. Park leftover PR
#889. Continue #890. `[skip
vercel]`. No `PRIVATE_MODE` flip.
No promote. Live www stays `.696`.

Label `2026.07-unified.1067`
(from this branch `.1066` /
`d8a65d44`). Stamp `.1067`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1052-for-1067.md](docs/archive/log/LOG-rotate-1052-for-1067.md).

## 2026-08-31 — Account is sign-in / return / prefs (`.1066`)

`/account` first paint is sign-in,
return, and prefs. Explore, help,
gym kit, referral, feedback, and
Super Bundle are off first paint.
Import stays via `#import`. Log
set stays filled. `generateWeek`
stays on `/coach`. Fuel stays the
log. Move stays the quiet log. You
stays title + Account. Park leftover
PR #889. Continue #890. `[skip
vercel]`. No `PRIVATE_MODE` flip.
No promote. Live www stays `.696`.

Label `2026.07-unified.1066`
(from this branch `.1065` /
`d611e326`). Stamp `.1066`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1050-for-1066.md](docs/archive/log/LOG-rotate-1050-for-1066.md).

## 2026-08-31 — You is quiet title + Account door (`.1065`)

`/profile` first paint is title +
ghost Account. Identity kit,
career, table, rewards, share,
kits, and private note are off
the page. Log set stays filled.
`generateWeek` stays on `/coach`.
Fuel stays the log. Move stays
the quiet log. Park leftover PR
#889. Continue #890. `[skip
vercel]`. No `PRIVATE_MODE` flip.
No promote. Live www stays `.696`.

Label `2026.07-unified.1065`
(from this branch `.1064` /
`caac544d`). Stamp `.1065`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1048-for-1065.md](docs/archive/log/LOG-rotate-1048-for-1065.md).

## 2026-08-31 — Move is the quiet log, not a flow tour (`.1064`)

`/move` first paint is the quiet
log. Flow grid, collections,
premium preview, recent wins, and
Browse-flows empty are off the
page. QuietMoveLogCard stays the
writer. Log set stays filled.
`generateWeek` stays on `/coach`.
Fuel stays empty + Log meal, or
today's meals. Park leftover PR
#889. Continue #890. `[skip
vercel]`. No `PRIVATE_MODE` flip.
No promote. Live www stays `.696`.

Label `2026.07-unified.1064`
(from this branch `.1063` /
`96f39a3b`). Stamp `.1064`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1047-for-1064.md](docs/archive/log/LOG-rotate-1047-for-1064.md).

## 2026-08-30 — Fuel is the log, not a tour (`.1063`)

`/nutrition` first paint is empty +
Log meal, or today's meals.
Remaining, notepad, Show more,
recipes, restock, week, wizard,
adapt, weight, and past days are
off the page. FuelLogSheet stays
the meal writer. Log set stays
filled. `generateWeek` stays on
`/coach`. Park leftover PR #889.
Continue #890. `[skip vercel]`.
No `PRIVATE_MODE` flip. No
...
Label `2026.07-unified.1063`
(from this branch `.1062` /
`8a2119ea`). Stamp `.1063`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1046-for-1063.md](docs/archive/log/LOG-rotate-1046-for-1063.md).

## 2026-08-30 — Coach is the week writer, not a tour (`.1062`)

`/coach` first paint is empty +
Generate, or this week's session.
Week strip, dose, live voice,
next-day cite, Show all, chat, and
Super Bundle taster are off the
page. `generateWeek` stays on
`useCoachPlan`. Log set stays
filled. Park leftover PR #889.
Continue #890. `[skip vercel]`.
No `PRIVATE_MODE` flip. No
promote. Live www stays `.696`.

Label `2026.07-unified.1062`
(from this branch `.1061` /
`9f07f336`). Stamp `.1062`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1045-for-1062.md](docs/archive/log/LOG-rotate-1045-for-1062.md).

## 2026-08-30 — Victory is a receipt, not a tour (`.1061`)

Victory first paint is title +
stats + one next. Show all, feel,
share, rewards, vs-last table,
jot, name, save-routine, and
start-again are off the overlay.
Name / notes / train again stay
on History. Finish on 0 sets
stays a no-op. Log set stays
filled. Generate stays on
`/coach`. Park leftover PR #889.
Continue #890. `[skip vercel]`.
No `PRIVATE_MODE` flip. No
promote. Live www stays `.696`.
...
(from this branch `.1060` /
`157a1c65`). Stamp `.1061`.

Rotated LOG oldest → [docs/archive/log/LOG-rotate-1044-for-1061.md](docs/archive/log/LOG-rotate-1044-for-1061.md).
