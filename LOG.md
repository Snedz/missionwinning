...
## 2026-08-13 — Super Bundle Fuel premium recipes (`.700`)

Super Bundle Fuel depth so $59 can beat a food-log subscription on cookable
plates — not a barcode catalog. Free logger stays free. No Fuel SKU, no trial,
no new tab.

**Ship:** +30 protein-first garage / travel / leftover recipes (110 → **140**).
`PREMIUM_RECIPE_COUNT` + `CONTENT_FLOORS.recipesPremium` + teasers (real names).
Fuel Coach + Today insight + Fuel shop blurbs say protein-first; bundle Fuel
strings interpolate `{{count}}` so they cannot repeat the old “20 recipes” lie.
Honesty test kills a stale floor, a missing frozen name, a sub-25g plate, a
phantom teaser, and a typed Fuel shop digit.

Label `.700` (onto master `.697`; `.698`/`.699` reserved). Excellence-Override below.
