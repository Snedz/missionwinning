...
## 2026-08-13 — Guidebook Ch3 + Periodization intro: weeks from logs (`.698`)

#479 already owns Strength Basics `sb-0` (offline log, then Coach). This ship
extends a different free chapter: Programming & Tuning (`programming-tuning` /
Ch3) plus its linked Learn path `periodization-design`.

Diataxis tutorial on Ch3-s1: log sets on this phone (offline, no wearable), then
Mission Coach builds the week from that diary. RPE and deload sections stop
sending practice to Builder templates. Periodization path opens on `pd-0` Weeks
From Your Logs. Original MW wording; originality logged. Learn stays ungated.
Heroes still parked. No chrome restyle. No PRIVATE_MODE flip.

Excellence-Override: Learn free intro wedge (content)
