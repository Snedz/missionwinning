...
## 2026-09-24 — Coach muscle balance map (`.1112`)

History already counted sessions
per major group, and Coach never
showed where those sessions
landed on a body. A rectangle
silhouette is not that picture.
`react-body-highlighter` (MIT,
Copyright (c) 2020 GV79) draws
anterior and posterior. The
polygons stay in the package.
`NOTICE.md` carries the copyright
and permission notice.

`muscleBalanceFromHistory` does
not count again. It reads
`buildMuscleHeatmap` session
counts for the same 14-day
window, drops tombs, and projects
the six major groups onto a
closed, disjoint slug list.
`Full Body` and `Cardio` paint
nothing. Forearm, adductors,
head, neck, knees, and soleus
are not major groups, so they
stay off. Frequency is sessions,
not sets: two presses in one
log are one Chest. The week
planner does not read this.

`/coach` mounts one strip when
the page is past first paint.
Token fills replace the
package's default blue. An
empty window is a blank body
and one sentence. The text list
is the accessible reading.

**Mutants killed:** tombstone
still a session; two lifts in
one session counting twice;
stored Back losing to the
bench-press catalog name;
`Full Body` inventing a region;
a slug claimed by two groups;
an unmapped vendor part painted;
the wrapper pasting polygons or
a hex ramp; Coach not mounting
the strip.
