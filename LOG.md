...
## 2026-08-27 — Left room rail on athlete shell (`.1056`)

Last night shipped IA docs
(`.1053`–`.1055`). Local `/log`
was still a bare desk: top bar,
one Start, no left rail. The
Sidebar already existed and was
hidden below `md`.

**Ship:** recut that Sidebar to
the five named rooms — Today ·
Train · Coach (AI) · History ·
Library — and show it on the
signed-in shell at every width.
You + More sit in the rail foot.
More keeps Builder / Messenger /
Account. Messenger is never a
rail href or a Message item.
Same wireframe tokens. No
costume. No `/private` restyle.
