...
## 2026-08-13 — Pregnancy + miscarriage safety (educational, counsel-hold) (`.746`)
...
do not promote until that review.
...
never a logger gate). Quiet Account control under More settings only. While
any hold flag is on, Coach `nextTargets` cannot emit a load jump (weight /
loadPct rise) — the gate is in code, not a prompt; why-line is only “This is
not medical advice; ask your clinician.” PFT Take full/mini, fitness-test
Continue, and program Load for closed max-effort session names stay hidden.
e1RM estimates remain; no CTA to test a max. Terms EN adds one clinician-owned
pregnancy sentence (not a legal-pack rewrite). Copy-guard forbids “safe for
pregnancy” / “prevents miscarriage” product claims. Free logger ungated.

Label `.746` (occupied `.698`–`.745`; onto master `.697`). Excellence-Override
below. Tweet source was readable; product bits used: stop always exists, output
gate in code, hard cap on max-effort CTAs.
