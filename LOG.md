...
## 2026-08-27 — Crew board + founder gate (`.1057`)

Signed-in More room at `/crew`.
Six seats with owns / stops / sign.
Vote locked until that charter is
signed. 0/6 → 6/6 is real state.
Founder gate holds send / delete /
publish / promote. No undo after a
signature. House chrome, not a
clinic or a lab. Never a rail icon.
Never Today. `/server` stays a
drawer. Isolation + `logSet` stay.
Zero diffs on `/private`, landing,
www. `[skip vercel]`. No
`PRIVATE_MODE` flip. No promote.
Live www stays `.696`.

Label `2026.07-unified.1057` (from
house `.1056` / `2b609256`). Visual
only. Do not merge.
