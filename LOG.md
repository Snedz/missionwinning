...
## 2026-09-14 — TodayDesk concept chip (`.1071`)

One concept-first `house-state` chip on
TodayDesk before Start
(`data-testid="today-concept-chip"`):
`Today: Legs · from last log` (or
Push / Pull / Recovery). Source is
CoachConcept spine — `plan.nextConceptId`
or derive from the next open
PlanSession (same as Victory `.1067`).
Guest and signed-in. Absent when
freestyle / no plan / week complete.
No Feed. No remake of Today. One Start
stays Start.
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-1054-for-1071.md](docs/archive/log/LOG-rotate-1054-for-1071.md) (`.1054`).
...
## 2026-09-14 — Font + i18n CI hygiene (`.1069`)

`#935` left two unit reds.
Library hidden-name row used
`font-medium` (Archivo ships
400/600/800 only). `ar` / `id`
/ `pt` / `zh` `activeLastPerformance`
kept `{reps,weight}` after `en`
moved to `{line}`.
...
Rotated LOG oldest →
[docs/archive/log/LOG-rotate-1052-for-1069.md](docs/archive/log/LOG-rotate-1052-for-1069.md)
(`.1052`).
...
## 2026-09-13 · `.1067` — CoachConcept + Victory concept-first cite (NCP apply)

Discrete `CoachConceptId` vocabulary. Finish → adapt stamps
`nextConceptId` on the next planned/swapped session. Victory
`data-testid="victory-next-env-cite"` shows concept-first muted
text when known (deepens `.1066`). No Feed. No invented loads.
No NCP weights.
...
## 2026-09-13 · `.1066` — Victory next-env cite (RSI surface)

After Finish, when Coach adapt marks this log done and a next planned
session remains, Victory shows one muted cite
(`data-testid="victory-next-env-cite"`): “Next session updated from this
workout”. Finish calls the existing `adaptPlan` path sync so the cite is
honest. No Feed. No invented loads.
...
Patched in sharp `0.35.4`.

Root app: `overrides.sharp` → `0.35.4` (lock was `0.35.3` via Next
optionalDependencies). Refresh `package-lock.json`.
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-1043-for-1061.md](docs/archive/log/LOG-rotate-1043-for-1061.md).

## 2026-09-13 — House black+yellow 4th skin density (`.1059`)

Signed-in house was a white/zinc
wireframe with black press. Founder
...
Label `2026.07-unified.1059` (from
`.1058` / `23d4b13c`; master paper
`.1057` / `f302f40f`). Stamp stays
`.1059`. Live www stays `.696`.
`[skip vercel]`.
...
You private note summary cite is house leftover.
You table Edit summary cite is house leftover.
You athlete card Edit summary cite is house leftover.
You identity Edit summary cite is house leftover.
...
You athlete card privacy cite is house leftover.
You kit saved cite is house leftover.
