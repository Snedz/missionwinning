...
sessions by lift shape. A second
session still had no calm chip,
and a Push did not compare to the
last Push when the lifts differed.

`decideResumeLast` shows **Resume
last** when History has a finished
log and the live session has no
logged set. The chip copies that
log through
`templateFromCompletedLog`.
It is not the open-session
resume path. Weights stay
editable. Logged work is not
replaced. Copy is "Session 2 —
lock the habit". The tone
constants reject streak and
shame language.

`topLiftDeltas` ranks this
session's top lifts against the
previous log of the same template
name. Up is `text-status-ok`.
Even or down is `text-status-warn`.
A first time on that name has no
status color and no invented zero.
Equal volume ties sort by
exercise id. The optional
one-line diary normalizes
locally and
`victoryDiaryBlocksSave` is false,
so Next stays open.

Distinct from shape receipt
(`.713`) and Repeat last (`.717`).
Year heatmap and workout CSV
import stay as they landed.
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-1099-for-1114.md](docs/archive/log/LOG-rotate-1099-for-1114.md) (`.1099`). `.1098` was already rotated by `.1113`.
