...
## 2026-08-13 — Super Bundle Move premium flows (`.709`)

Overnight Super Bundle: prescribed, train-anywhere, original Move flows. Not a
Pliability video library, not a mobility score, not a new tab. Catalog only —
S6 Victory wiring (#491) is untouched. Free S6 deep-link ids and name prefixes
frozen. Premium collision `morning-wake-up` renamed to `morning-wake-premium-11`.
Clock honesty: `sum(durationSec) >= durationMin * 60 − 60`. Equipment rows carry
a floor fallback. Eight original premium flows take the catalog 48 → **56**.
Floor `movePremium: 56`. Locked hint interpolates `CONTENT_FLOORS`.
`post-upper-flush` no longer tagged `post-legs`. I3b copy: 60 Mind + 56 Move.
Tests: `movePremD3_709.test.ts`. Mutants: collision id, clock shortfall,
Pliability copy, stolen S6 prefix. Retagged off `.699` (reserved #478).
Excellence-Override below.

Excellence-Override: Super Bundle Move premium flows (content)

Rotated LOG oldest → [docs/archive/log/LOG-rotate-669-for-709.md](docs/archive/log/LOG-rotate-669-for-709.md).
