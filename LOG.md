...
## 2026-08-15 — Victory Peak-End is the sheet alone (`.844`)

GNT-1 U1/U5 stills showed Session locked stacked on
"No session running": `completeActiveWorkout` clears the session
before the sheet opens, and the empty Active invite stayed mounted
under the honor overlay. Close was an 18px X.

**Ship:** `ActiveEmptyState` mounts `WorkoutVictorySheet` only while
victory is open; dismiss returns the invite. Victory close is 44px.
TAP_BUDGET stays 5. AL1 stays open.

Label `.844` (past origin/master `.843` and tip `.841`).

Excellence-Override: gauntlet GNT-1.smoother

## 2026-08-15 — U1 e2e empty-start REACH teaser match npm run dev (`.841`)

After `.840` `gotoHydrated`, the three U1 specs still missed live
`npm run dev` chrome: empty `/active` docks Just Go (not "Start
workout"), More has no `/assessments`, `/` is the waitlist teaser,
evening Day in Review has no Turn on without VAPID.

**Ship:** empty-start clicks the dock primary after dropping
`mw_equipment`. REACH discovers tabs + `moreSheetRowHrefs`. first-90
homepage/hero accept the teaser; 19:00 does not require Turn on.
TAP_BUDGET stays 5. AL1 stays open.
...
Rotated LOG oldest → [docs/archive/log/LOG-rotate-827-for-844.md](docs/archive/log/LOG-rotate-827-for-844.md).
