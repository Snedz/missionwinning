...
## 2026-09-24 — Offline program shell (`.1112`)

Local program templates, separate
from logged workouts. The tree is
Program → Week → Session →
exercise → suggested set:
`level`, `durationWeeks`,
`sessionsPerWeek`, `equipment[]`.
One full-body seed. Another
program can be saved on this
device through `safeStorage`.
Builder Show all lists them.
Start opens Train with those
suggested sets and does not
replace a session that is
already open.

`parseProgram` copies known
fields and refuses a billing
key anywhere in the payload
(`isPremium`, `paymentUrl`,
price, subscription, Stripe,
RevenueCat, license). No new
table, no premium gate on the
logger. Flat Builder cycles in
`src/data/programTemplates.ts`
stay as they are.

**Mutants killed:** billing key
accepted on the program or on
a nested week; `durationWeeks`
lying about `weeks.length`;
`sessionsPerWeek` lying about
the sessions in a week; a
template session emitted as a
history row (`startedAt`);
suggested-set order ignored;
the seed overwritten in device
storage.
