...
| File | Route |
|------|-------|
| `ActiveWorkoutPage.tsx` | `/active` | House compose canvas + sidecar (rest / skip / jot). Cold `/active` is `TrainComposeEmpty`. Live session is name + table + rest; elapsed pauses (`.1001`); plates / jot / HR in Show all |
...
| `LibraryPage.tsx` | `/library` | Official exercise catalog. House catalog states (muscle chips) + short first-paint list; posters / templates / merge in Show all. Duplicate names can be merged (`.1002`). A name can be hidden from Add / search (`.1004`). Hidden / Unhide first-paint is a house leftover only when a name is hidden — empty is honest. Search is house-field. |
...
| File | Route | Notes |
|------|-------|-------|
| `CoachPage.tsx` | `/coach` | Weekly AI plan — first paint is this week’s session; generate dock, next-day cite, week dose, and Show all fold chrome are house leftovers. Voice/LoadBand/LogCite/Manage internals stay. |
