...
| File | Route |
|------|-------|
| `ActiveWorkoutPage.tsx` | `/active` | House compose canvas + sidecar (rest / skip / jot). After hydrate, empty `/active` seeds Just Go so the set table is the product, not Restoring session. Live session is name + table + rest; elapsed pauses (`.1001`); plates / jot / HR in Show all |
