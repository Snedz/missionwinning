...
| `AccountPage.tsx` | `/account` — settings leftover; first paint is house leftover (static import, no `RouteLoading` / `useSearchParams`). Account first-paint sign-in block is house leftover. Sign-in / return / prefs first; Explore / more / help are house-card |
