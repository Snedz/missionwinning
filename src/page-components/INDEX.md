...
| `LibraryPage.tsx` | `/library` | Official exercise catalog. House catalog states (muscle chips) + short first-paint list; posters / templates / merge in Show all. Duplicate names can be merged (`.1002`). A name can be hidden from Add / search (`.1004`). Hidden / Unhide first-paint is a house leftover only when a name is hidden — empty is honest. Search is house-field. Library pick bar is house leftover. |
