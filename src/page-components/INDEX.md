...
| File | Route |
|------|-------|
| `ActiveWorkoutPage.tsx` | `/active` | House compose canvas + sidecar (rest / skip / jot). Route is a static import (no `RouteLoading`). First paint writes today's session in `useLayoutEffect` and paints a set table before hydrate. Persist merge keeps that compose. Live session is name + table + rest; elapsed pauses (`.1001`); plates / jot / HR in Show all |
