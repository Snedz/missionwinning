/** Shown on Profile so testers can confirm the deployed build. Bump when shipping UI to master. `.1072` Mission OS capability interfaces + in-memory fakes (MiniHost.mount happy-path); paper only; do not promote; live www stays `.697`; PRIVATE_MODE stays; [skip vercel]. */
