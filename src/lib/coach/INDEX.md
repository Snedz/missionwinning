...
8h. `trainerReply.ts` — free AI personal trainer for the open set (`.1115`). Gemini Flash when `GEMINI_API_KEY` is set, else `COACH_LLM_*`, else `sessionCoachLine`. No premium check. No checkout URL. Route: `POST /api/coach/trainer`.
