...
8e2. `nextDayFromLogs.ts` — stable next day (Wednesday) from the live diary, or the live plan when it owns the next calendar day (`.955`). Does not call `generateWeek`
...
| `nextDayFromLogs.test.ts` | Wednesday from logs: empty invents nothing; rotation stable; plan next-day wins; tombstones/0-rep out |
