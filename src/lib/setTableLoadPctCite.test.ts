/**
 * Set-table load-% cite is house leftover — house-lede, not text-muted.
 * Load-% cell stays house-num. next-cite leftover stays.
...
test('Set-table load-% cite is house leftover, not text-muted', () => {
  const table = read('src/components/workout/SetLogTable.tsx');
  const needle = 'data-testid="set-table-load-pct-cite"';
  const start = table.indexOf(needle);
  assert.ok(start >= 0, `missing ${needle}`);
  const cite = table.slice(Math.max(0, start - 180), start + 40);
  assert.match(cite, /house-lede/);
  assert.doesNotMatch(cite, /text-muted-foreground/);
});

test('load-% cell stays house-num (not this leftover)', () => {
  const table = read('src/components/workout/SetLogTable.tsx');
  const cell = sliceFromTestId(table, 'set-table-load-pct');
  assert.match(cell, /numberInput/);
  assert.doesNotMatch(cell, /house-lede/);
});

test('next-cite leftover stays (not this leftover)', () => {
  ...
  assert.ok(start >= 0, `missing ${needle}`);
  const nextLine = next.slice(Math.max(0, start - 180), start + 40);
...
test('house leftover rule paints set-table load-% cite with --house-muted', () => {
...
test('DESIGN names Set-table load-% cite is house leftover', () => {
