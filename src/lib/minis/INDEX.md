...
| billing | Injected `{ bundle, muted: true }`. Never checkout. Dual-mount snapshots stay isolated (`.1094`). |
| photos | `photos_stub` when scoped with no snapshot. Injected stub snapshot isolates per mount (`.1095`). Unscoped → `scope_denied` (same CapResult deny as billing). |
