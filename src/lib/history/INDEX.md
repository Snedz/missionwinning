...
> One concern: past-session facts for `/history` — month marks, the session list, find-a-past-session search (`.1008`), export this diary (`.1011`), import that file back (`.1013`), this session as a file they own (`.1016`), and the month they own (`.1017`).
...
6. `monthTheyOwn.ts` — tap a live day (`.1017`); tombs out; start-from never erases the month
...
| `monthTheyOwn.ts` | Month they own (`.1017`). Live-day facts + tap select. Tombs out. Start-from ignored. Empty invents nothing. Not a fire count. |
