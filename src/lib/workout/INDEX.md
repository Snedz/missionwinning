...
10a0. `resumeLastOffer.ts` — Train chip when history has a finished log and today has no logged set (`.1112`). Copies that log through `templateFromCompletedLog`. Not `sessionResume.ts` (open session, `.963`). Prefill keeps last weights editable. Tone is Session 2 — lock the habit. Logged work is not replaced.
