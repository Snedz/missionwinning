...
10c2. `sessionCoachLine.ts` — one sentence for the open set (`.1114`). Index, name, kind, and the dialed load. Blank name is silence. Empty barbell is not bodyweight. Tank line only on a work set after `activeCoachTipKind` is high. Not a model. Not a paywall. Ask under the line (`.1115`) posts to `/api/coach/trainer`; the rules sentence is the dark-seat answer.
