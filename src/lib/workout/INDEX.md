...
1d2. `openEmptyLoad.ts` — open set-row empty load is blank, not 0 (`.1048`). Store stays 0. Display only. Never write BW as a stored kilogram. Open cell keeps a local draft (`displayOpenLoadDraft`) so `0.` / `2.5` stay typeable.  
1e. `setRowType.ts` — open row speaks weight · bodyweight reps · duration · assisted (`.994`). Empty / custom stays weight×reps. Vest is extra only. Live Next/Last cites reuse this grammar (`.1009` BW, `.1014` hold `0:45`, `.1015` assisted 0 → BW not `0 kg`, `.1017` weight-type empty load → BW not `8 × 0`). Open empty load cell is blank, not 0 (`.1048`).  
...
3. `superset.ts` — group of two or more (`groupWithNext` / `unpair` / `pairMark` A1/A2/A3), advance after log, rest gate, rest identity on first peer (`.749` / `.980` / concern `.979`); History edit can pair / unpair a finished log (`.1047`)  
...
10a0. `resumeLastOffer.ts` — Train chip when history has a finished log and today has no logged set (`.1114`). Copies that log through `templateFromCompletedLog`. Not `sessionResume.ts` (open session, `.963`). Prefill keeps last weights editable. Tone is Session 2 — lock the habit. Logged work is not replaced.
10a0b. `victoryVsLastLifts.ts` — top lifts vs the previous session of the same template name (`.1114`). Up is green, even or down is amber, first time has no status color.
10a0c. `victoryDiaryLine.ts` — optional one-line Victory note (`.1114`). Empty is valid. Never blocks save.  
...
10a6. `editFinishedSession.ts` — edit the sets on a finished History log (`.997`). Confirm-gated. Empty invents nothing. Never wipes. Not Resume. `sameEvidence` includes loadPct (`.1044`) / tempo (`.1043`) / side (`.1042`) / RIR (`.1041`) / RPE (`.1040`) / kind (`.1039`). `draftsEqual` includes the lift note (`.1045`) and `supersetGroup` (`.1047`). `stripDraft` omits a blank group and runs `stripOrphanGroups`.  
...
10a6m. `patchFinishedSuperset.ts` — optional exercise group on a finished History session (`.1047`). Pair with next. Unpair then `stripOrphanGroups`. One lift / junk empty. Already sharing with next is noop. Does not rewrite sets / notes / duration / name. Save still `decideEditSave`.  
...
23. `tempo.ts` — optional ecc/pause/con parse + last-tempo recall (`.757`); History edit can correct a logged tempo (`.1043`)
...
| `setTableLogSetHousePress.test.ts` | Log set is house leftover press (`--house-press` / #ffb000), not poster red (`.1059`) |
| `logConsoleLogSetHousePress.test.ts` | LogConsole Log set is house leftover press (`--house-press` / #ffb000), not poster red (`.1059`) |
...
| `resumeLastOffer.test.ts` | Prefill weights stay editable; logged work hides the chip (`.1114`) |
| `resumeLastSurface.test.ts` | Train chip + Victory green/amber + diary does not gate Next (`.1114`) |
| `victoryVsLastLifts.test.ts` | Same-name volume delta; first time has no red class; diary never blocks save (`.1114`) |
...
| `copySessionDay.test.ts` | Finished log → another day as a new row; source stays; tomb / future / same-day invent nothing (`.1030`) |
| `copySessionDaySurface.test.ts` | History detail Copy; Today one Start; Move / Repeat / edit / backfill stay (`.1030`) |
...
| `assistedCite.test.ts` | Assisted 0 prints `8 × BW`, not `8 × 0 kg`; help stays minus (`.1015`) |
| `emptyLoadCite.test.ts` | Weight-type empty load Last/Prev/after-complete print `8 × BW`, not `8 × 0` (`.1017`) |
...
| `appendFinishedExercise.test.ts` | Finished-session add lift: empty / junk invent nothing; unknown noop; squat after bench 135×5 is 0/0; clone; duplicate allowed (`.1037`) |
...
| `patchFinishedSuperset.test.ts` | Finished-session group: empty / junk / one-lift invent nothing; last + next empty; pair two shares id; already paired noop; unpair clears and strips orphan; no sets/notes write; clone; no store; Save still decideEditSave (`.1047`) |
| `patchFinishedSupersetSurface.test.ts` | History edit superset control; Today one Start; session-note / lift-note / load-pct stay (`.1047`) |
