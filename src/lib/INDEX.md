...
| **First paint** | `firstPaintFloor.test.ts`, `unlockWaitlistHonesty.test.ts`, `adjustSessionHonesty.test.ts`, `coachChatPlaceholderHonesty.test.ts`, `talkRetryHonesty.test.ts` | No raw keys, no textless public fallback, no copy that changes after hydration |
