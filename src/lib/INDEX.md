...
| **Today primary CTA** | `todayPrimaryAction.ts`, `coach/loadCoachTodayOptional.ts`, [`today/accountLite.ts`](today/accountLite.ts) | Shared Just Go / journey primary for lean + dashboard. F-017: first set needs no account; offer after first workout. |
