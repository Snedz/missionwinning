...
| **Gated www honesty** | `gatedWwwHonesty.ts` | F-008 / old `.698` — EN Free / Enter with code / Get notified on the gated door. Public line: Log a set. Offline. Support: No account. No wearable. Build `.934` on `#776` / `.933` door strings. |
...
| **First paint** | `firstPaintFloor.test.ts`, `unlockWaitlistHonesty.test.ts`, `adjustSessionHonesty.test.ts`, `coachChatPlaceholderHonesty.test.ts`, `talkRetryHonesty.test.ts`, `parqDoorHonesty.test.ts`, `todayCoachPinHonesty.test.ts`, `trainEmptyStartHonesty.test.ts`, `journalEmptyHonesty.test.ts`, `libraryAddHonesty.test.ts` | No raw keys, no textless public fallback, no copy that changes after hydration |
