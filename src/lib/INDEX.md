...
| **Mission OS minis** | [`minis/`](minis/INDEX.md), [`mission-os/`](mission-os/INDEX.md), `minisIsolation.test.ts` | Function bus (`createMiniBus`, #935) + named doors + in-memory fakes (`MiniHost.mount`, `.1072`). Health stub `l1.health` at `mission://minis/health` (L1 first mini, identity + storage only, `.1074`). ClearShot stub `utility.clearshot` at `mission://minis/clearshot` (utility lane, photos + storage.write, `.1075`). Mount isolation on bus fakes (`.1076`). Last-segment deeplink `mission://minis/clearshot` → `utility.clearshot` mount (`.1077`). No Today/Train UI. Isolation: coach / store / HomePage / ActiveWorkout stay blind. |
...
| `mission-os/` | Named host interfaces + in-memory fakes (`MiniHost.mount`) — [mission-os/INDEX.md](mission-os/INDEX.md) |
