...
| **Mission OS minis** | [`minis/`](minis/INDEX.md), [`mission-os/`](mission-os/INDEX.md), `minisIsolation.test.ts` | Function bus (`createMiniBus`, #935) + named doors + in-memory fakes (`MiniHost.mount` / `MiniHost.unmount` / `MiniHost.listMounted`, `.1072` / `.1078` / `.1081`). Health stub `l1.health` at `mission://minis/health` (L1 first mini, identity + storage only, `.1074`). ClearShot stub `utility.clearshot` at `mission://minis/clearshot` (utility lane, photos + storage.write, `.1075`). Mount isolation on bus fakes (`.1076`). Last-segment deeplink `mission://minis/clearshot` → `utility.clearshot` mount (`.1077`). Unmount/remount leftover isolation on the in-memory host (`.1078`). Billing CapResult deny consistency on every fake method; test-only `test.billing` grant (`.1079`). Photos CapResult deny consistency — Health `scope_denied`, ClearShot `photos_stub` (`.1080`). `listMounted` inventory: mounted ids + declared scopes only; undeclared peek `scope_denied`; never-mounted `unknown_mini` (`.1081`). Identity CapResult deny consistency — test-only `test.noidentity` `scope_denied`; Health + ClearShot stub success (`.1082`). Storage CapResult deny consistency — unscoped `scope_denied` (no write); Health stub success; ClearShot write-only (`.1083`). Allow-path consistency — declared billing / photos / identity / storage return existing stub envelopes; test-only `test.granted` (`.1084`). Unknown-method consistency — declared door + unknown name is `unknown_method`; undeclared door stays `scope_denied` (`.1085`). Duplicate mount of a still-mounted id is `already_mounted` (`.1086`). Unknown id on `MiniHost.mount` is `unknown_mini` (`.1087`). Unmount of an id that is not currently mounted is `not_mounted` (`.1088`). Capability call for an id that is not currently mounted is `not_mounted` — does not throw, does not auto-mount (`.1089`). No Today/Train UI. Isolation: coach / store / HomePage / ActiveWorkout stay blind. |
...
| **Today primary CTA** | `todayPrimaryAction.ts`, `coach/loadCoachTodayOptional.ts`, `today/todayReturnCite.ts`, `today/todayConceptChip.ts`, `today/quietWeekGlance.ts`, `today/quietWeekRow.ts`, `today/quietWeekTrackTrend.ts` | Shared Just Go / journey primary for lean + dashboard; last/next cite on Start (`.954`); concept chip from Coach plan truth (`.1071`); quiet Mon–Sun diary glance (`.961`); thin flag when 1–2 sessions (`.971`); empty rest-day quiet Fuel / Move / Track row (`.977`); muted last-vs-this on a Track strip day (`.989`) |
| **Search catalog** | `searchCatalog.ts` | Filter More rooms by query; map Fuel/Coach/Train to Summary pin ids. Search is a field, not a shop. Official catalog = Library + Builder (`catalogLabeling.test.ts`). |
...
| `mission-os/` | Named host interfaces + in-memory fakes (`MiniHost.mount` / `unmount`) — [mission-os/INDEX.md](mission-os/INDEX.md) |
