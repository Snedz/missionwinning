 ...
 * Stop / not-care / clinician stay. Confirm stays outline. Portals stay mw-house.
 */
...
test('hard-session not-care / clinician stay text-muted (not this leftover)', () => {
  const sheet = read('src/components/workout/HardSessionWarningSheet.tsx');
  const notCareStart = sheet.indexOf('hardSessionNotCare');
  assert.ok(notCareStart >= 0, 'missing hardSessionNotCare');
  const notCare = sheet.slice(Math.max(0, notCareStart - 160), notCareStart + 80);
  assert.match(notCare, /text-sm text-muted-foreground leading-relaxed/);
