...
- **Tests to run:** `src/lib/mission-os/fakes.test.ts`, `src/lib/mission-os/host.test.ts`, `src/lib/mission-os/health.test.ts`, `src/lib/mission-os/clearshot.test.ts`, `src/lib/mission-os/mountIsolation.test.ts`, `src/lib/mission-os/deeplink.test.ts`, `src/lib/mission-os/billing.test.ts`, `src/lib/mission-os/photos.test.ts`, `src/lib/mission-os/listMounted.test.ts`, `src/lib/mission-os/identity.test.ts`, `src/lib/mission-os/storage.test.ts`, `src/lib/mission-os/allow.test.ts`, `src/lib/mission-os/call.test.ts`, `src/lib/mission-os/alreadyMounted.test.ts`, `src/lib/mission-os/unknownMini.test.ts`, `src/lib/mission-os/notMounted.test.ts`, `src/lib/mission-os/callNotMounted.test.ts`, `src/lib/mission-os/deeplinkUnknownMini.test.ts`, `src/lib/mission-os/deeplinkBadDeeplink.test.ts`, `src/lib/mission-os/dualMountStorage.test.ts`, `src/lib/mission-os/dualMountIdentity.test.ts`, `src/lib/mission-os/dualMountBilling.test.ts`, `src/lib/mission-os/dualMountPhotos.test.ts`, `src/lib/minisIsolation.test.ts`
...
| Door | Stub |
|------|------|
| identity | `createIdentityFake`. Closed method `read`. Unscoped → `scope_denied`. Scoped → injected `{ missionId, callSign }` (guests `null`). Per-mini `identities[id]` + fake-only `injectIdentitySnapshot` — A's read is not B's (`.1093`). Unknown name on a declared door → `unknown_method`. Never mint. Never auth UI. |
| billing | `createBillingFake` / `createBillingHold`. Closed methods `read` / `checkout` / `portal`. Unscoped → `scope_denied`. Scoped → muted read + `{ held: true }` stub. Per-mini `billings[id]` + fake-only `injectBillingSnapshot` — A's read is not B's (`.1094`). Unknown name on a declared door → `unknown_method`. Never Stripe. |
| photos | `createPhotosFake`. Closed methods `read` / `write`. Unscoped → `scope_denied`. No snapshot → `photos_stub`. Bound snapshot → isolated stub envelope. Per-mini `photoses[id]` + fake-only `injectPhotosSnapshot` — A's read is not B's (`.1095`). Unknown name on a declared door → `unknown_method`. Never camera. Never MediaStore. |
| storage | `createStorageFake`. Closed methods `get` / `set`. Unscoped → `scope_denied` (does not write). Health (read + write) stub success. ClearShot write stub success; read stays `scope_denied`. Unknown name on a declared door → `unknown_method` (does not write). Cap 32 keys / 4KB (mw-core). `MiniHost.unmount(id)` clears that map so remount cannot read leftovers. Two live mounts keep isolated maps (`.1092`). |
...
| `PLAN.md` | `.1095` dual-mount photos isolation claim |
