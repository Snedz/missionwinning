...
- **Tests to run:** `src/lib/mission-os/fakes.test.ts`, `src/lib/mission-os/host.test.ts`, `src/lib/mission-os/health.test.ts`, `src/lib/mission-os/clearshot.test.ts`, `src/lib/mission-os/mountIsolation.test.ts`, `src/lib/mission-os/deeplink.test.ts`, `src/lib/mission-os/billing.test.ts`, `src/lib/mission-os/photos.test.ts`, `src/lib/mission-os/listMounted.test.ts`, `src/lib/mission-os/identity.test.ts`, `src/lib/mission-os/storage.test.ts`, `src/lib/mission-os/allow.test.ts`, `src/lib/mission-os/call.test.ts`, `src/lib/mission-os/alreadyMounted.test.ts`, `src/lib/mission-os/unknownMini.test.ts`, `src/lib/mission-os/notMounted.test.ts`, `src/lib/mission-os/callNotMounted.test.ts`, `src/lib/mission-os/deeplinkUnknownMini.test.ts`, `src/lib/mission-os/deeplinkBadDeeplink.test.ts`, `src/lib/mission-os/dualMountStorage.test.ts`, `src/lib/mission-os/dualMountIdentity.test.ts`, `src/lib/mission-os/dualMountBilling.test.ts`, `src/lib/mission-os/dualMountPhotos.test.ts`, `src/lib/mission-os/unknownCapability.test.ts`, `src/lib/mission-os/storageCap.test.ts`, `src/lib/mission-os/denyCodes.test.ts`, `src/lib/mission-os/storageCapRemount.test.ts`, `src/lib/minisIsolation.test.ts`
...
| storage | `createStorageFake`. Closed methods `get` / `set`. Unscoped → `scope_denied` (does not write). Health (read + write) stub success. ClearShot write stub success; read stays `scope_denied`. Unknown name on a declared door → `unknown_method` (does not write). A name that is not a door → `unknown_capability` (does not write, `.1096`). Cap 32 keys / 4KB — overflow is `storage_cap` and does not write (`.1097`). `MiniHost.unmount(id)` clears that map so remount cannot read leftovers — including after `storage_cap`, so remount is not still capped (`.1099`). Two live mounts keep isolated maps (`.1092`). |
...
| `PLAN.md` | `.1099` remount after storage_cap claim |
