...
- **Tests to run:** `src/lib/mission-os/fakes.test.ts`, `src/lib/mission-os/host.test.ts`, `src/lib/mission-os/health.test.ts`, `src/lib/mission-os/clearshot.test.ts`, `src/lib/mission-os/mountIsolation.test.ts`, `src/lib/mission-os/deeplink.test.ts`, `src/lib/mission-os/billing.test.ts`, `src/lib/mission-os/photos.test.ts`, `src/lib/mission-os/listMounted.test.ts`, `src/lib/mission-os/identity.test.ts`, `src/lib/mission-os/storage.test.ts`, `src/lib/mission-os/allow.test.ts`, `src/lib/mission-os/call.test.ts`, `src/lib/mission-os/alreadyMounted.test.ts`, `src/lib/mission-os/unknownMini.test.ts`, `src/lib/mission-os/notMounted.test.ts`, `src/lib/mission-os/callNotMounted.test.ts`, `src/lib/mission-os/deeplinkUnknownMini.test.ts`, `src/lib/minisIsolation.test.ts`
...
Last-segment deeplink: `resolveMiniDeeplink` / `mountMiniByDeeplink` — closed table `health` → `l1.health`, `clearshot` → `utility.clearshot`. Long opaque paths (`mission://minis/utility.clearshot`) and an unknown last-segment (`mission://minis/totally-unknown`) are `unknown_mini` (same code as mount `.1087`; does not throw, does not mount).
...
| `PLAN.md` | `.1090` deeplink unknown last-segment refuse claim |
...
| `deeplink.ts` | Last-segment `clearshot` → `utility.clearshot` mount; unknown last-segment is `unknown_mini` |
