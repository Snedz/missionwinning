 ...
 *  - `src/page-components`: swept to zero — stays zero.
...
function countSynthesizedWeights(dir: string): { count: number; hits: string[] } {
  ...
  for (const file of walk(path.join(root, dir))) {
    // Comments stripped first: this repo documents reasoning at the call site
    // (`check-design-system.mjs` header), and a guard that punishes that gets
