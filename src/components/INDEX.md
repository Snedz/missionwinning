...
| `form/` | Form guides |
| `history/` | History charts, muscle heatmap, finished-session edit (`.997`), past-session backfill (`.1000`), merge duplicates (`.1002`), delete this finished session (`.1003`), restore a deleted session (`.1006`), name this finished session (`.1007`), start history from this date (`.1005`), export this diary (`.1011`), import that file back (`.1013`), this session as a file they own (`.1016`), month they own (`.1018`), Repeat this session (`.1026`), move this session to another day (`.1027`), log onto this empty day (`.1028`), this month as a file they own (`.1029`), copy this session onto another day (`.1030`), This month on the History calendar (`.1031`), trained day shows how many live sessions (`.1032`), this month shows how many live sessions (`.1033`), reorder lifts on a finished session (`.1034`), edit this session's logged duration (`.1035`), replace a lift on a finished session (`.1036`) |
