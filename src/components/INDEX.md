...
| `profile/` | Profile settings cards (privacy, backup, referral, wearables, What’s New). `HomeGymKitCard` — Account day-one Home gym kit (`.763`). `ProfileImportCard` — workout CSV preview + confirm (`.940`); session CSV download is header-only when empty (`.943`) |
