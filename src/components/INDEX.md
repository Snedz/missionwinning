...
| `flags/` | Founder staged-rollout console — [flags/INDEX.md](flags/INDEX.md). Closed catalog. Never a nav tab. |
| `form/` | Form guides |
| `house/` | Signed-in product house — [house/INDEX.md](house/INDEX.md) |
| `history/` | History charts, muscle heatmap, finished-session edit (`.997`), past-session backfill (`.1000`), merge duplicates (`.1002`), delete this finished session (`.1003`), restore a deleted session (`.1006`), name this finished session (`.1007`), start history from this date (`.1005`), export this diary (`.1011`), import that file back (`.1013`), this session as a file they own (`.1016`), month they own (`.1018`), Repeat this session (`.1026`), move this session to another day (`.1027`), log onto this empty day (`.1028`), this month as a file they own (`.1029`), copy this session onto another day (`.1030`), This month on the History calendar (`.1031`), trained day shows how many live sessions (`.1032`), this month shows how many live sessions (`.1033`), reorder lifts on a finished session (`.1034`), edit this session's logged duration (`.1035`), replace a lift on a finished session (`.1036`), add a lift to a finished session (`.1037`), remove this lift from a finished session (`.1038`), set kind on a finished set (`.1039`), optional 1–10 RPE on a finished set (`.1040`), optional 0–5 RIR on a finished set (`.1041`), optional L / R / Alt on a finished set (`.1042`), optional e-p-c tempo on a finished set (`.1043`), optional % of a known 1-rep max on a finished set (`.1044`), optional per-lift diary on a finished exercise (`.1045`), optional private session note on a finished log (`.1046`), optional exercise group on a finished session (`.1047`) |
