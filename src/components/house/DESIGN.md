...
| Train compose | `.house-compose-live` | Log set stays the filled action. Empty is house-lede. Add exercise is house-btn. Add Set is house-btn. Rest lanes are house-state; selected `#eee`. Number cells are house-num. Finish is house-btn, not filled. Extras in house-card Show all. In-set cues are a kicker + ghost hide / Learn door. Kind chips are house-state; selected `#eee`. Sidecar leftover is rest / skip / notes. History stays on Home. |
