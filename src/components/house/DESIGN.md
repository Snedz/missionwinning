...
| Plan empty | `.house-empty` + dock | Invite on the sheet; Generate stays one filled action on `/coach`. Coach generate dock is house leftover. Coach next-day cite is house leftover. Ghost Start — never a second filled action. Coach week dose is house leftover. Coach session card is house leftover. One filled Start this session on the boss card. Coach session lift is house leftover. Swap stays ghost. |
