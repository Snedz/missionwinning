...
| `SetLogTable.tsx` | **Desktop** set list — Strong/Hevy density (`Set · Prev · kg · Reps`); **Prev is the row anchor** (`data-prev-anchor`); E-Adjacency Target+cite stacked in the Prev cell; ≥44px inputs; one poster-red inline `Log set` (sole red at md+). Compact uses `SetLogRow` + `LogConsole` |
| `SetLogRow.tsx` | **Read-only set record** — **Target + cite above Prev** + this-session line (no "In the console" prose), kind/PR/RPE, `Check`. ≥44px row + RPE. No filled red — entry is `LogConsole` |
| `SetLogAdjacencyStack.tsx` | Shared Target / cite / Previous stack (compact + desktop Prev cell) |
