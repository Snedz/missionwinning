...
| `RestTimerBar.tsx` | Rest countdown — **takes the `ScreenDock` over from `LogConsole`, never both**. Full-bleed on compact, a centred `max-w-lg` panel at `md+`. Skip must stay exactly `Skip` (`logger-depth` matches `/^skip$/i`) |
