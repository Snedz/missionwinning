...
| `SessionCheckInSheet.tsx` | Pre-session readiness. Check-in lead cite is house leftover (`house-lede` / `--house-muted`). Check-in confirm is house leftover (Save house-btn, Not now ghost). Check-in scale is house leftover. Never gates Log set. |
...
| `HardSessionWarningSheet.tsx` | Pre-start hard-session warning — Back does not start; never gates Log set. Stop line follows pregnancy flag (`.746` v1). Hard-session lead cite is house leftover (`house-lede` / `--house-muted`). Hard-session confirm is house leftover. |
