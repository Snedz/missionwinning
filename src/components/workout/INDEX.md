...
| `SetLogTable.tsx` | **Desktop** set list — Strong/Hevy density (`Set · Prev · kg · Reps`); **Prev is the row anchor** (`data-prev-anchor`); pair mark `A1·n` when paired (`.749`); optional L/R/Alt; completed rows optional RIR beside RPE (`.756`); ≥44px inputs; one poster-red inline `Log set`. Compact uses `SetLogRow` + `LogConsole` |
| `SetLogRow.tsx` | **Read-only set record** — **Prev metric anchor** + this-session line, pair mark `A1·n` when paired (`.749`), kind/PR/RPE, optional L/R/Alt + RIR, `Check`. ≥44px row. No filled red — entry is `LogConsole` |
