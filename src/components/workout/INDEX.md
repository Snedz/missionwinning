...
| `SetLogTable.tsx` | **Desktop** set list — Strong/Hevy density (`Set · Prev · kg · Reps`), ≥44px inputs, one poster-red inline `Log set` on the active row (sole red at md+). Compact uses `SetLogRow` + `LogConsole` instead |
| `SetLogRow.tsx` | **Read-only set record** — metric-first `n · 8 × 60 kg` (no "In the console" prose), kind/PR/RPE, `Check`. ≥44px row + RPE taps. No filled red — entry is `LogConsole` |
...
| `RestTimerBar.tsx` | Rest countdown — **takes the `ScreenDock` over from `LogConsole`, never both**. Dense ink; at `md+` `#restDock` one row via `md:contents`. Skip via `data-testid="rest-skip"`; accent fill only in final ≤10s when rest owns the dock |
