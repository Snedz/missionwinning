...
| `SetLogTable.tsx` | **Desktop** set list — Strong/Hevy density (`Set · Prev · kg · Reps`); **Prev is the row anchor** (`data-prev-anchor`); pair mark `A1·n` when paired (`.749`); optional L/R/Alt; BW+ load on plus-load moves (`.758`); completed rows optional RIR + tempo beside RPE (`.756`/`.757`); ≥44px inputs; one poster-red inline `Log set`. Compact uses `SetLogRow` + `LogConsole` |
| `SetLogRow.tsx` | **Read-only set record** — **Prev metric anchor** + this-session line (BW + load when plus-load), pair mark `A1·n` when paired (`.749`), kind/PR/RPE, optional L/R/Alt + RIR + tempo, `Check`. ≥44px row. No filled red — entry is `LogConsole` |
...
| `LogConsole.tsx` | **Compact only** — **the only place a set is entered.** Dense ink `ScreenDock`: name + `Set n of m`, overload cue, collapsed Work/Kind chips, optional L/R/Alt on unilateral, **BW+ load stepper** on plus-load moves (0 = skip), 52px steppers, one poster-red `Log set` in the thumb zone (F-003) |
