...
| `MovementHistorySheet.tsx` | Prior sessions of the open lift — their diary. Empty invents nothing. Not a chart (`.993`). Close is house-btn; rows are house-movement-row. Movement-history empty cite is house leftover (`house-lede` / `--house-muted`). |
