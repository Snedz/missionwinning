...
| `ActiveExerciseHeader.tsx` | Title, menus, next line, e1RM. Name opens this-movement history (`.993`). Grip reorders the live list (`.998`). Exercise head is house leftover. Skipped-this-session cite is house leftover (`house-lede` / `--house-muted`). Form guide is house-btn ghost. Form guide confirm is house-btn, not filled. Form guide body is house leftover. Swap is house-btn ghost. Skip this exercise is house-btn hold. Exercise more is house leftover (ghost more + house-card overflow). Set options is house leftover (ghost more + house-card overflow). Reorder handle is house leftover (ghost house-btn grip and arrows). |
