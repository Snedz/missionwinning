...
| `SessionCheckInSheet.tsx` | Pre-session readiness. Check-in lead cite is house leftover (`house-lede` / `--house-muted`). Check-in scale value cite is house leftover (`house-lede` / `--house-muted`). Check-in confirm is house leftover (Save house-btn, Not now ghost). Check-in scale is house leftover. Never gates Log set. |
