...
| `SetLogTable.tsx` | Set list on **every** surface — Set · Prev · load · Reps · Log. Open row honors type (`.994`): weight · BW vest · time · assist. Empty / unknown stays kg × reps. Open empty load is blank, not 0 (`.1048`); store stays 0. Completed empty load is BW (`.1025`). Prev is the row anchor. ≥44px inputs. One poster-red inline `Log set`. Optional free W / D / F chips per set (`.966`). Optional % of a known 1-rep max (`.981`). After-complete cite via `SetLogNextCite`. Incomplete warmup batch rows show planned weight and can be removed (`.984` / stamp `.985`). Optional EMOM / AMRAP on the live row (`.987`). Quiet diary PR after a working set that beats a number they already logged (`.999`). |
