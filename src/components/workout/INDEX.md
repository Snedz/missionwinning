...
| `ActiveExerciseHeader.tsx` | Title, menus, next line, e1RM. Name opens this-movement history (`.993`). Grip reorders the live list (`.998`). Exercise head is house leftover. Skipped-this-session cite is house leftover (`house-lede` / `--house-muted`). Live-row next-target cite is house leftover (`house-lede` / `--house-muted`). Session e1RM cite is house leftover (`house-lede` / `--house-muted`). Form guide is house-btn ghost. Form guide confirm is house-btn, not filled. Form guide body is house leftover. Form guide breath cite is house leftover (`house-lede` / `--house-muted`). Form guide section items is house leftover (`house-lede` / `--house-muted`). Form guide figure caption is house leftover (`house-lede` / `--house-muted`). Swap is house-btn ghost. Skip this exercise is house-btn hold. Exercise more is house leftover (ghost more + house-card overflow). Set options is house leftover (ghost more + house-card overflow). Reorder handle is house leftover (ghost house-btn grip and arrows). |
...
| `MovementHistorySheet.tsx` | Prior sessions of the open lift — their diary. Empty invents nothing. Not a chart (`.993`). Close is house-btn; rows are house-movement-row. Movement-history empty cite is house leftover (`house-lede` / `--house-muted`). Movement-history list-row cites is house leftover (`house-lede` / `--house-muted`). |
...
| `SessionSwapSheet.tsx` | This-session swap door — garage stand-ins + another movement; confirm is house-btn, not filled (`.959`). Portaled overlay root carries `mw-house`. Exercise picker option details is house leftover (`house-lede` / `--house-muted`). Exercise picker selected cite is house leftover (`house-lede` / `--house-muted`). Exercise picker empty cite is house leftover (`house-lede` / `--house-muted`). |
| `GarageSwapList.tsx` | Short garage stand-ins for Train swap + Coach line. Garage swap is house leftover (ghost house-btn, selected `#eee`). Swap garage lead cite is house leftover (`house-lede` / `--house-muted`). Swap garage cues is house leftover (`house-lede` / `--house-muted`). |
| `SetLogTable.tsx` | Set list on **every** surface — Set · Prev · load · Reps · Log. Open row honors type (`.994`): weight · BW vest · time · assist. Empty / unknown stays kg × reps. Open empty load is blank, not 0 (`.1048`); store stays 0; `SetRowLoadField` keeps a local draft so `0.` / `2.5` stay typeable. Completed empty load is BW (`.1025`). Prev is the row anchor. Prev cite is house leftover (`--house-muted`). Set row hairline is house leftover (`--house-line`). Upcoming set row is house leftover (`--house-muted`). Kind chip row is house leftover (`--house-muted`). Set table size is house leftover (14px, not `text-sm`). ≥44px inputs. One house leftover `Log set` (`--house-press`). Log set size is house leftover (12px on the set table, not `text-xs`; LogConsole stays 19px). Log set weight is house leftover (700 on the set table, not `font-extrabold`; LogConsole stays extrabold). Log set leading is house leftover (1.25 on the set table, not `leading-tight`). Last-set ghost is house leftover (house-btn ghost, not filled). Warmup toggle is house leftover (ghost house-btn, hover is house-chip, not muted). Optional free W / D / F chips per set (`.966`). Optional % of a known 1-rep max (`.981`). Set-table RPE cite is house leftover (`house-lede` / `--house-muted`). Set-table in-set PR cite is house leftover (`house-lede` / `--house-muted`). Set-table vs-last cite is house leftover (`house-lede` / `--house-muted`). After-complete cite via `SetLogNextCite`. Incomplete warmup batch rows show planned weight and can be removed (`.984` / stamp `.985`). Optional EMOM / AMRAP on the live row (`.987`). Quiet diary PR after a working set that beats a number they already logged (`.999`). |
| `SetLogNextCite.tsx` | Skippable next-set cite after a completed working set (`.939`). Not a feed. |
| `SetLogPlateLine.tsx` | Skippable both-sides plate breakdown on the live barbell row (`.948`). Plate-line cite is house leftover (`house-lede` / `--house-muted`). Editable bar. Never blocks Log set. |
...
| `AddExerciseSheet.tsx` | `ExercisePicker` in a sheet with the confirm in the footer. Confirm is house-btn, not filled. Do not rewrite shared ExercisePicker. Exercise picker option details is house leftover (`house-lede` / `--house-muted`). Exercise picker selected cite is house leftover (`house-lede` / `--house-muted`). Exercise picker empty cite is house leftover (`house-lede` / `--house-muted`). **Test contract:** keeps the `search exercises` placeholder, `option` rows and `add selected exercise` name — `logger-depth`, `first-90` and `hero-flows` all drive them. Typed catalog miss can name a custom (`.990`). Hidden names stay off the list (`.1004`). Unlimited. Free. Empty invents nothing. |
