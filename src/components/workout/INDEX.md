...
| `ActiveWorkoutSheets.tsx` | Check-in · hard-session warning · form · add · plates · victory overlay cluster |
...
| `LogConsole.tsx` | **Compact only** — **the only place a set is entered.** Dense ink `ScreenDock`: name + `Set n of m`, overload cue, collapsed Work/Kind chips (ink selected, never accent fill), 52px steppers, one poster-red `Log set` in the thumb zone (F-003). F-013: dial already holds last load/reps (`log-console-reps` / `log-console-weight`) so Log is one confirm |
...
| `SessionCheckInSheet.tsx` | Pre-session readiness (skippable; not a logger gate) |
| `HardSessionWarningSheet.tsx` | Pre-start hard-session warning (`.727`) — Back does not start; never gates Log set |
...
| `VictoryReceiptStrip.tsx` | Per-lift Set · Prev · Load · vs last + PR badges (`.713`) |
