...
| `src/components/` | Page pieces. `PtnNav` / `PtnClose` are the unsigned Patreon chrome. `CtaSlot.astro` is the one action (black/white pills); `StagePhoto.astro` makes a photograph a ground; `LogToPlanDemo` and `CoachAdaptDemo` are the two live islands. Live tokens: `src/styles/patreonTokens.css` imported from `global.css`. Wireframe snapshot: `docs/archive/www-wireframe-1050/` |
