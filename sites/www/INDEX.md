...
| `src/components/` | Page pieces. `WwwNav` / `WwwFooter` are shared chrome. `CtaSlot.astro` is the one action; `StagePhoto.astro` makes a photograph a ground; `LogToPlanDemo` and `CoachAdaptDemo` are the two live islands |
