...
| **Starting a turn / choosing between the three loops** | `npm run harness` — it names the row, the route and the recipe ([docs/AGENT_RECIPES.md](docs/AGENT_RECIPES.md) §14). Route `stalled` means stop | Your memory of which row was open; a header line restating the queue; routing around the command instead of fixing it; treating `stalled` as a missing recipe |
