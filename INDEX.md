...
| **Starting a turn / choosing between the three loops** | `npm run queue:next` — it names the row, the route and the recipe ([docs/AGENT_RECIPES.md](docs/AGENT_RECIPES.md) §14) | Your memory of which row was open; a header line restating the queue; routing around the command instead of fixing it |
...
| [docs/GRAPH_LOOP.md](docs/GRAPH_LOOP.md) | Agent graph execution queue — live row from `npm run queue:next`, never restated here |
