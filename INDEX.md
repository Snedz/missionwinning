...
| **Starting a turn / choosing between the three loops** | `npm run harness` — it names the row, the route and the recipe ([docs/AGENT_RECIPES.md](docs/AGENT_RECIPES.md) §14). Surface hops: six-stage pipeline in [ORCHESTRATION.md](ORCHESTRATION.md). Start: `/harness` or `/harness 2 hours` (optional `/vision`) | Your memory of which row was open; a header line restating the queue; routing around the command instead of fixing it; inventing a letter after path proven |
...
| **Classification / dual-repo / open safely** | [docs/CLASSIFICATION.md](docs/CLASSIFICATION.md), [docs/DUAL_REPO.md](docs/DUAL_REPO.md), `npm run ops:sync`, `npm run snapshot:public` | Committing `ops/` or `.hermes/`; putting STRATEGY war-room detail in public product git long-term; treating the org snapshot as daily origin |
...
| **BANGERS / Dual Capture+** | [RESEARCH/BANGERS/dual-capture-plus/](RESEARCH/BANGERS/dual-capture-plus/) — sell-first paper; [FRAMEWORK.md](RESEARCH/BANGERS/dual-capture-plus/FRAMEWORK.md) then [SELL_FIRST.md](RESEARCH/BANGERS/dual-capture-plus/SELL_FIRST.md) | Building an iOS camera binary in this repo; starting [SPIKE_PLAN.md](RESEARCH/BANGERS/dual-capture-plus/SPIKE_PLAN.md) before the sell bar; inventing `PAYMENT_URL`; treating this as the MW iOS lane |
...
| **Framework program / DSAR / health-data bucket** | [docs/security/PROGRAM_STATUS.md](docs/security/PROGRAM_STATUS.md) + [`docs/compliance/controls.yaml`](docs/compliance/controls.yaml) + recipe 10 in [docs/AGENT_RECIPES.md](docs/AGENT_RECIPES.md) | Treating the 2026-07-22 compliance snapshot or the 08-13 hunt memo as live without re-verify |
