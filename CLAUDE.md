...
Node 22 (CI). PR CI is [.github/workflows/ci.yml](.github/workflows/ci.yml); minutes-heavy jobs (critical e2e, Android, Lighthouse) are manual/weekly in `ci-extended.yml`. **Whether Actions is currently running is recorded in exactly one place — `CONTEXT.md` `## Now`** — do not restate it elsewhere.

### `npm run gate` — 16 steps, in order
...
1. Build label + hard rule 5 · 2. Lint · 3. Typecheck · 4. Unit tests · 5. Route contract tests
6. i18n parity · 7. i18n coverage · 8. Dependency advisories · 9. Design system · 10. Locale split
11. Display type · 12. Token sync (web ↔ Android) · 13. Production build · 14. Bundle budget
15. Hero e2e (`@gate`) · 16. Accessibility (`@a11y`)
