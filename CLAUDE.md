...
| **Graph** | Two recall systems, not the process: `src/lib/graph/` (`npm run graph:ingest`) = memory of *our dev history* · `src/lib/ideaGraph/` + `docs/mechanics/` = the *idea* graph |
| **Harness** | The outer agent loop. Queue: `docs/GRAPH_LOOP.md` + `src/lib/loopQueue/` (`npm run harness`). Command: `/harness` (machine-local `~/.claude/skills/harness/`). `/graph` and `npm run graph` are aliases. |
...
npm run harness          # which loop runs next + which recipe (reads docs/GRAPH_LOOP.md)
...
| Which loop runs next | `npm run harness` — [docs/AGENT_RECIPES.md](docs/AGENT_RECIPES.md) §14, router in [src/lib/loopQueue/](src/lib/loopQueue/INDEX.md) |
