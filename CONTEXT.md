...
- **`.1070`:** (`2026.07-unified.1070`) **Feedback is the form, not a sign-in tour** — `/feedback` first paint is the form. Sign-in and legal footer are off the page. composeFeedbackNote stays the writer. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Learn stays the intro. Track stays the scale/tape log. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1069` (`1909ca4f`). Stamp `.1070`.
