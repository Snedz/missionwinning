...
## Now (2026-08-04 · web `2026.07-unified.488` · Android `1.24.1`)
...
- **`.488`:** Wedge i18n call-site alias guard (n/current, dose/count).
- **`.487`:** Free-beta mute guard + postal/launch-env honesty + re-entry dose wire.
- **`.486`:** Rest final Skip filled accent; hide presets in last 10s.
- **`.485`:** Rest clock accent in last 10s (outdoor “about to go”).
- **`.484`:** Locale packs use English `{{var}}` keys (not {{peso}}/{{単位}}).
- **`.483`:** Log console set ordinal uses `current`/`total` (not literal `{{current}}`).
- **`.482`:** Outdoor LogConsole set-kind collapse (Work + Kind expand).
- **`.481`–`.474`:** Form media + rest meter + Form Director residual.
