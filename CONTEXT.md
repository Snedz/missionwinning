...
> | Migrations | **P1–P12 + `mission_ids` + `feedback_reviews` applied 2026-08-14** on production `missionwinning`. **`.908` four files still pending apply:** `20260817_profiles_protect_nudge_cols.sql`, `20260817_fitness_test_results_api_only.sql`, `20260817_school_classes_revoke_anon_select.sql`, `20260817_leaderboard_snapshots_server_write.sql`. Week-4 proof SQL not run via Management API. Next file still goes in the runbook. |
...
- **`.908`:** (`2026.07-unified.908`) **Client writes close** — nudge mailbox is auth email; PFT and leaderboard scores are service-role only; class codes are not world-readable. Four migrations still need applying on the hosted project. No `PRIVATE_MODE` flip.
