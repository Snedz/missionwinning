...
## Now (2026-08-14 · web `2026.07-unified.768` · Android `1.24.1`)
...
> and `.768` dropped `.754` detail (rotated to docs/archive/log/LOG-rotate-753-for-768.md)
...
- **`.768`:** (`2026.07-unified.768`) **Legal footers stop prefetching the legal library** — shard 4 (diaspora+RU) heavy first paint: `AppLegalFooter`'s eight `<Link>`s sit in the viewport on I-Day and the gate, and App Router prefetched all of them. `/welcome` **416→334 KB gz, 53→37 requests**; `/log` 594→545. `prefetch={false}` on every 4+-info-link cluster (`SignInPanel`, `LegalNav`, `ServiceTermsPage`, `TermsPage`), product links untouched. Two ratchets, since `bundle-budget` is blind to runtime prefetch **and** to `/private` + `/welcome` (both dynamic, no prerendered HTML): `linkPrefetchWeight.test.ts` + `gate-smoke` script counts (`/private` cap 26/measured 20, `/welcome` 38/32). **Shard 4's other four findings are already fixed on master and undeployed** — production still serves `.697` (its `/private` is still `Checking sign-in…`); the buried Log button does not reproduce (docked, in viewport at 360×640 / 390×844 / 412×732).
