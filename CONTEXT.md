...
> and `.844` dropped `.828` detail (full text remains in LOG.md / archive)
...
- **`.844`:** (`2026.07-unified.844`) **GNT-1 smoother** — Victory-open empty Active mounts the honor sheet only (no “No session running” under Session locked); close is 44px. TAP_BUDGET / firstPaintFloor / bundle caps unchanged. No `PRIVATE_MODE` flip.
- **`.841`:** (`2026.07-unified.841`) **GNT-1 U1 R3** — empty-start / REACH / first-90 teaser + evening red match `npm run dev`. TAP_BUDGET / firstPaintFloor / bundle caps unchanged. No `PRIVATE_MODE` flip.
