...
- **`.970`:** (`2026.07-unified.970`) **Free set tags** — optional Warmup / Drop / Failure on the set row (concern `.966`). Warmup is not last time, vs-last, why-line, or Wednesday. Tags are free. No auto warmup formulas. Guest. First set ungated. Today still one Start. Resume `.963` kept. Fuel `.965` stays off Today. RPE `.967` stays. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Skips `.968` / `.969`.
