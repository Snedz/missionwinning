...
- **`.948`:** (`2026.07-unified.948`) **Plate math on the free set row** — after a barbell weight, skippable `45 + 2×45` (or kg). Bar defaults to 45 lb / 20 kg and is editable. Empty/0 invents no plates. Log a set never waits. Today untouched. No wearable. No `PRIVATE_MODE` flip.
