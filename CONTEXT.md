...
- **`.986`:** (`2026.07-unified.986`) **Drop-set rest-zero** — a set tagged drop does not start the rest timer; a running timer stays at zero so the next drop in that series is without rest. A working set still starts rest. Warmup stays warmup (out of cites). No new tag taxonomy. Optional. Guest. First set ungated. Empty invents nothing. Today still one Start. Resume `.963` kept. Warmup `.985` / notes `.983` / 1RM `.981` / supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` / cues `.973` / honesty `.971` / tags `.970` / RPE `.967` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.985` (`8a9fe41f`).
