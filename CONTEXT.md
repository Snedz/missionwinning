...
- **`.840`:** (`2026.07-unified.840`) **GNT-1 smoother** — Victory honor next is AdaptiveOverlay footer, not Dialog scroll under the rail. No `PRIVATE_MODE` flip.
- **`.839`:** (`2026.07-unified.839`) **GNT-1 U1 R3** — empty `/active` dock is Start workout, not Just Go preview. Logger-depth / REACH complete on `npm run dev`. No `PRIVATE_MODE` flip.
