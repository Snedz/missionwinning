...
- **`.1051`:** (`2026.07-unified.1051`) **Patreon costume on our skeleton** — Founder GO. Live tokens in `docs/DESIGN_PATREON.md` + `src/styles/patreonTokens.css` (`.ptn`). ABC Oracle Plus is the reference, not licensed — Archivo stays. Modernist `DESIGN_SYSTEM.md` intact. `sites/www` snapshotted at `docs/archive/www-wireframe-1050/`. Public www: sticky Train / Coach / History / About, photo hero, black/white pills, `homeContent.ts` copy. Signed-in dual rail; Today still one Start; `/account` two-column. `/private` diff vs `.1050` empty. Cookie `/` LandingPage stays `.696`. Guest. First set ungated. `[skip vercel]`. No `PRIVATE_MODE` flip. No production promote. Live www stays `.696`. Past master `.1050` (`c2b54d585c80ab1f5a2de2070f147ee549782c78`). Stamp stays `.1051`.
