...
- **`.949`:** (`2026.07-unified.949`) **One identity** — guest sets survive sign-in. Unbound adopt keeps the workout store; `SIGNED_IN` re-queues history (no Force Sync tap). Foreign owner still replaces. Restricted health still stripped. First set still ungated. Skips `.948` plate math. No `PRIVATE_MODE` flip.
