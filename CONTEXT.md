...
- **`.952`:** (`2026.07-unified.952`) **failRead comma** — `ProfileImportCard` toast object was missing a comma after the `description` ternary, so Preview/tsc died on `.951`. One character. No `PRIVATE_MODE` flip.
