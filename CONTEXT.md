...
- **`.1076`:** (`2026.07-unified.1076`) **Mission OS mini mount isolation tests** — Health `l1.health` (not retired `health.mini`) identity+storage granted; photos/billing `scope_denied`. ClearShot `utility.clearshot` photos+`storage.write` granted (photos stay `photos_stub`); billing `scope_denied`. CapResult sandbox: one mini cannot read another's storage keyspace on MiniHost fakes. Stubs stay stubby. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
