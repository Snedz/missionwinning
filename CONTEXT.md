...
- **`.1018`:** (`2026.07-unified.1018`) **Month they own** — History Show-all month: tap a live day, see that day's live rows, open the row they already have. Session count, not a fire count. Tombs stay unmarked. Start-from fold never erases the month. Search stays a query. Charts stay `liveHistory`. Empty month / empty day invents nothing. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Empty-load cite `.1017` / Session file `.1016` / Assisted cite `.1015` / Duration cite `.1014` / Import `.1013` / Titles `.1012` / Export `.1011` / Library skip `.1010` / BW cite `.1009` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1017` (`8a606a12`). Stamp stays `.1018`.
- **`.1017`:** (`2026.07-unified.1017`) **Last cite is BW, not 0, on empty load** — Next already prints `8 × BW` via `formatSetLoadLine` when weight is 0; Last / Prev / after-complete still interpolated `8 × 0`. Weight-type empty load prints `8 × BW`. Loaded stays kg. Custom stays type weight. Display only — store still `weight: 0`. Reuses `formatSetRowPrev`. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Session file `.1016` / Assisted cite `.1015` / Duration cite `.1014` / Import `.1013` / Titles `.1012` / Export `.1011` / Library skip `.1010` / BW cite `.1009` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1016` (`fa96397d5`). Stamp stays `.1017`.
