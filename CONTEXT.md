...
## Now (2026-08-14 · web `2026.07-unified.770` · Android `1.24.1`)
...
- **`.770`:** (`2026.07-unified.770`) **A dialog owned www's first paint; a protected Preview was never walked.** Shard 0 (ops #17) saw the "ungated Preview" 302 → `vercel.com/sso-api` on all 19 paths and filed *"Train unreachable"*; re-measured here, **19/19 requests refused by Vercel, none by the app**. Deployment Protection is a **project setting — `vercel.json` cannot change it**, and disabling it is the wrong fix, so `deploymentProtection.ts` carries the bypass secret / trusted-OIDC token (plus the bypass **cookie** for browser walks) into `gate-smoke` and Playwright: inert today, one founder action from walking any Preview (**F-035**, checklist §1.7, which also records that **F-017 lives on #523, not the #522 Preview** that was walked). `gate-smoke` prints one access verdict instead of 28 fake defects. Walking the same code locally on a gated production build then found the worst www defect of the series, *in front of `.769`*: **`LocaleCountryChooser` opened itself over the poster** at `z-[70]` on every route, so `elementsFromPoint` at the centre of the red **Log a set** button returned the sheet and a click on it **timed out** — visible, enabled, untappable. It was a dialog with no branch: the language was applied before it rendered, and Continue and the scrim persisted the same guess. Now the guess is **written, not posed**, the sheet **waits to be asked**, and the gate — which had no language control at all — carries one labelled in three scripts. Measured cold at 390×844: **0 taps → 5 taps** from gate to a set on the device. Writing the e2e to run in **both** gate configurations rather than skipping in one (`.213`) then found that the **landing page had no language control either** (`LocaleCountryControl` only ever reached `/bundle` and `/press`), and that **the gate's accessibility step has collected zero tests since `.766`** — `a11y.spec.ts` listed `/server` twice and Playwright will not load a file with a duplicate test title. Fixed; axe now runs, **58 pass / 13 fail, none from this branch**. **Founder call surfaced by it:** `/private` and `/` fail WCAG AA on the poster red — `#f3f2f2` on `#ec3013` is **3.75:1** against 4.5:1 for normal text (210 nodes on the gate). Large display type is fine at 3:1; this is small text on `--accent-poster`, so the fix is a design-token decision, not an agent's. **`master`'s gate is red at four steps** before this branch (coverage floors 432 vs 395 — this branch adds none; i18n coverage 70; bundle budget `/` +29 KB, `/log` +15 KB, `/active` +48 KB; `@gate` e2e 12/32), recorded rather than ratified by raising a floor. **Still terrible:** `/active` server-renders 113 chars, content area *"Loading Workout…"* — `.765`'s defect on the wedge. 15 mutants; the e2e fails on a rebuilt binary with the wall restored.
