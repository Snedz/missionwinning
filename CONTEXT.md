...
- **`.1069`:** (`2026.07-unified.1069`) **Learn is the intro, not a paths tour** — `/learn` first paint is QuietLearnIntroCard. Other paths, guidebook, sample workout, and premium are off the page. QuietLearnIntroCard stays the intro. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Move stays the quiet log. Mind stays the check-in. Track stays the scale/tape log. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1068` (`17e4fd6b`). Stamp `.1069`.
- **`.1068`:** (`2026.07-unified.1068`) **Track is the scale/tape log, not a walks tour** — `/track` first paint is BodyMetricsCard. Walks, GPS, import, wearables, and trends are off the page. BodyMetricsCard stays the writer. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Move stays the quiet log. Mind stays the check-in. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1067` (`7a3e4cca`). Stamp `.1068`.
- **`.1067`:** (`2026.07-unified.1067`) **Mind is the check-in, not a sessions tour** — `/mind` first paint is DailyCheckIn. Breathe, Show all, sessions, premium, and recent wins are off the page. DailyCheckIn stays the writer. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Move stays the quiet log. You stays title + Account. Account stays sign-in / return / prefs. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1066` (`d8a65d44`). Stamp `.1067`.
- **`.1066`:** (`2026.07-unified.1066`) **Account is sign-in / return / prefs** — `/account` first paint is those three. Explore, help, gym kit, referral, feedback, and Super Bundle are off first paint. Import stays via `#import`. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Move stays the quiet log. You stays title + Account. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1065` (`d611e326`). Stamp `.1066`.
- **`.1065`:** (`2026.07-unified.1065`) **You is quiet title + Account door** — `/profile` first paint is title + ghost Account. Identity kit, career, table, rewards, share, kits, and private note are off the page. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Move stays the quiet log. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1064` (`caac544d`). Stamp `.1065`.
- **`.1064`:** (`2026.07-unified.1064`) **Move is the quiet log, not a flow tour** — `/move` first paint is QuietMoveLogCard. Flow grid, collections, premium preview, recent wins, and Browse-flows empty are off the page. QuietMoveLogCard stays the writer. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays empty + Log meal, or today's meals. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1063` (`96f39a3b`). Stamp `.1064`.
- **`.1063`:** (`2026.07-unified.1063`) **Fuel is the log, not a tour** — `/nutrition` first paint is empty + Log meal, or today's meals. Remaining, notepad, Show more, recipes, restock, week, wizard, adapt, weight, and past days are off the page. FuelLogSheet stays the meal writer. Log set stays filled. `generateWeek` stays on `/coach`. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1062` (`8a2119ea`). Stamp `.1063`.
- **`.1062`:** (`2026.07-unified.1062`) **Coach is the week writer, not a tour** — `/coach` first paint is empty + Generate, or this week's session. Week strip, dose, live voice, next-day cite, Show all, chat, and Super Bundle taster are off the page. `generateWeek` stays on `useCoachPlan`. Log set stays filled. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1061` (`9f07f336`). Stamp `.1062`.
- **`.1061`:** (`2026.07-unified.1061`) **Victory is a receipt, not a tour** — overlay first paint is title + stats + one next. Show all, feel, share, rewards, vs-last table, jot, name, save-routine, and start-again are off the overlay. Name / notes / train again stay on History. Finish on 0 sets stays a no-op. Log set stays filled. Generate stays on `/coach`. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1060` (`157a1c65`). Stamp `.1061`.
- **`.1060`:** (`2026.07-unified.1060`) **I-Day is not a wall** — `/log` and `/active` bypass JourneyGuard. Welcome Skip stamps beginner / bodyweight / strength and lands Today. Begin stays the filled press. Skip is ghost (`welcomeSkipSignIn` copy, not Skip-sign-in). Coach / History still bounce. Start still writes then `/active`. Log set stays filled. Generate stays on `/coach`. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1059` (`59501192`). Stamp `.1060`.
- **`.1059`:** (`2026.07-unified.1059`) **Today is date + Start** — `/log` first paint is date + session + one Start. First rooms, week strip, and Generate this week are off the canvas. Start still writes then `/active`. Log set stays filled. Generate stays on `/coach`. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1058` (`3761ca38`). Stamp `.1059`.
- **`.1058`:** (`2026.07-unified.1058`) **Train does not mint a Coach week** — `/active` no longer mounts `useCoachPlan` (that hook auto-`generateWeek`s). Volume trim in Show all reads/writes the stored plan only. Generate stays the week writer on `/coach`. Today Start still writes then `/active`. Log set stays filled. Finish stays outline. Park leftover PR #889. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1057` (`f302f40f`). Stamp `.1058`.
