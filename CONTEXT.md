...
- **`.1056`:** (`2026.07-unified.1056`) **From-scratch signed-in house** — Icon rail 72 + second bar 264 (hides &lt;723), hover chips (Today/Train/Library/You/More), Got it coach-mark, checklist under the week (N of M). Start is honest (no dummy; lands `/active`). MW rooms only. Isolation + Start-order stay. `/private` / landing / www untouched. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1055` (`e44351c3`). Stamp stays `.1056`.
