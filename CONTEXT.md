...
- **`.991`:** (`2026.07-unified.991`) **This session becomes a Start** — from the close receipt or History they can Start this again (same movements, last loads as targets). Empty invents nothing. Not a shop. Honor notebook `.960` stays program-in. Guest. First set ungated. Today still one Start. Resume `.963` kept. Track trend `.989` / EMOM `.988` / drop-set `.986` / warmup `.985` / notes `.983` / 1RM `.981` / supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` / cues `.973` / honesty `.971` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.989` (`adad3e58`).
...
- **`.971`:** (`2026.07-unified.971`) **Thin-history honesty** — Wednesday does not invent a next day from one or two sessions. Week strip empty days stay empty; no fake streak / on-track / consistency score. Empty cite has no Start (notebook `.960` wins). Set-row empty copy stays. Resume / finish-partial `.963` stays. Tags `.970` stay. RPE `.967` stays optional. Fuel `.965` stays off Today. Guest. First set ungated. Today still one Start. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.970`; skips `.972`. Title may stay `.964`.
