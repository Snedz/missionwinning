...
- **`.701`:** (`2026.07-unified.701`) **Exquisite gated www** — four cinematic scenes on `/private` + post-unlock `/`. N1: SET = full-viewport LOG SET (TARGET / set table, quiet Coach line). Anywhere = type on the still. Week = authored miss/travel/band. Door = poster red; copy pack Free beta / Enter with code / Get notified; form on a paper strip. HUD nav. Quiet L2 after the door (athlete page you author; not a feed). Draft; no Preview. [docs/design/WWW_NIGHT.md](docs/design/WWW_NIGHT.md).
