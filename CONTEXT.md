...
- **`.925`:** (`2026.07-unified.925`) **Train empty Start is Start Workout** — packs win. Same dock. Logger stays free. No `PRIVATE_MODE` flip.
- **`.924`:** (`2026.07-unified.924`) **Today Coach pin is Coach** — not AI weekly plan. Same pin. Same Talk door. No `PRIVATE_MODE` flip.
- **`.923`:** (`2026.07-unified.923`) **PAR-Q continue is Continue to this week** — not Submit Assessment. Assessments keeps its lecture. Same door. No `PRIVATE_MODE` flip.
- **`.922`:** (`2026.07-unified.922`) **Talk retry is Retry** — not Try again. Packs win. Same Talk path. No mic. No `PRIVATE_MODE` flip.
- **`.921`:** (`2026.07-unified.921`) **Talk placeholder is Ask about today’s session…** — curly apostrophe, packs win. Same composer. No mic. No `PRIVATE_MODE` flip.
- **`.920`:** (`2026.07-unified.920`) **Adjust session is Adjust today’s session** — curly apostrophe, packs win. Same sheet. No `PRIVATE_MODE` flip.
- **`.919`:** (`2026.07-unified.919`) **Unlock waitlist is Get notified** — not Join founders list. Packs win. Same form. No `PRIVATE_MODE` flip.
- **`.918`:** (`2026.07-unified.918`) **Fuel house door is Search, barcode & recipes** — not Show all. Packs win. Same notepad. No `PRIVATE_MODE` flip.
- **`.917`:** (`2026.07-unified.917`) **Excellence path map names Cue me and builder** — `src/components/speech` and `src/lib/builder` are on the map. Same gate. No `PRIVATE_MODE` flip.
- **`.916`:** (`2026.07-unified.916`) **Show all is Show all** — Train, Coach, Mind, Victory, History, Library, Builder. Fuel keeps Search, barcode & recipes. No `PRIVATE_MODE` flip.
