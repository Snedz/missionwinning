...
- **`.1078`:** (`2026.07-unified.1078`) **MiniHost unmount/remount isolation** — In-memory `MiniHost.unmount(id)` clears that mini's fake keyspace. Remount cannot read leftovers from another mini or from its previous mount. Remount without unmount still keeps the same mini's keyspace (`.1076`). Stubs stay stubby. No Photos / Billing / Android. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
