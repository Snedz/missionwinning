...
- **`.946`:** (`2026.07-unified.946`) **F-013 smart defaults** — next set starts from the last working set. Empty or warmup-only history is empty (no fake 10, no bump). Editable. Log a set never waits. Cite / Last / Prev stay their lanes. Rebased past `#783` `.945`. No `PRIVATE_MODE` flip.
