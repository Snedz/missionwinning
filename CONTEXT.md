...
- **`.1075`:** (`2026.07-unified.1075`) **PathShell, not House** — Signed-in chrome is word nav for Start / Log / Generate. House floor unmounted. Finish keeps the open session clientId. Coach does not auto-mint a week. Coach and History are not I-Day walls. Log set stays filled. `generateWeek` stays on `/coach`. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1074` (`543823f4`). Stamp `.1075`.
