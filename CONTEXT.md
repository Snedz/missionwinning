...
> | Migrations | **P1–P12 + `mission_ids` + `feedback_reviews` applied 2026-08-14** on production `missionwinning`. **`.908` four files applied 2026-08-17** (SQL Editor): `20260817_profiles_protect_nudge_cols.sql`, `20260817_fitness_test_results_api_only.sql`, `20260817_school_classes_revoke_anon_select.sql`, `20260817_leaderboard_snapshots_server_write.sql`. **`.958` `20260824_profiles_open_session.sql` is in the runbook — not applied.** **`.1067` `20260914_feature_flags.sql` is in the runbook — not applied.** Week-4 proof SQL not run via Management API. Next file still goes in the runbook. |
...
- **`.1067`:** (`2026.07-unified.1067`) **Feature flags console** — Founder staged rollouts at `/account/flags`. Closed catalog (`example_staged` default 0%). Percent / allowlist / kill in Postgres. Kill beats allowlist. Train never imports the domain. Athlete `GET /api/flags` is a boolean map for this subject. Missing table is 503 `flags_unavailable`, never an empty list. `PRIVATE_MODE` and surface parking stay deploy-time. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
