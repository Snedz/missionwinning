...
- **`.1057`:** (`2026.07-unified.1057`) **Left second bar + transferred rooms** — Home/Library open a left column next to the icon rail (72+264, hides &lt;723), not a far-right More sheet. Start in that bar is one click → `/active` compose. This week pane shows the real week and doors `generateWeek` on `/coach` only. History and Weekly plan are the real rooms. More is leftover (Fuel/You/Account + quiet Move/Mind/Track/Learn/Feedback/Garage). One Got it. Checklist ticks from visits. Hover chips stay. `/private` / landing / www untouched. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1056` (`9daa7d18`). Stamp stays `.1057`.
