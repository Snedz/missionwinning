...
## Now (2026-08-04 · web `2026.07-unified.481` · Android `1.24.1`)
...
- **`.481`:** Library form media parity + rest outdoor meter thickness.
- **`.480`:** Form guide mid-set autoplay loops (+ reduced-motion still).
- **`.479`:** Landmine-press loop pilot PASS; row/squat still-only.
- **`.478`:** Bench loop PASS; pull-ups still-only (head crop at top).
- **`.477`:** Empty-bar front-squat + RDL + row loops PASS.
- **`.476`:** Empty-bar deadlift loop PASS; OHP still-only (behind-head I2V FAIL ×2).
- **`.475`:** RDL bar-axis fix — bar in front of legs (not through thighs).
- **`.474`–`.467`:** Empty-bar collars + Form Director system + bulk gen.
