...
> and `.700` dropped `.629` detail (full text remains in LOG.md / archive)
...
> | GitHub Actions | **blocked again at `runner_id: 0` — the quota ran out.** Measured 2026-08-09 05:22Z on PR #440: `build-and-test`, `gitleaks` and `aikido-security` all failed **without executing a step** — `runner_id: 0`, empty `runner_name`, `started_at`→`completed_at` of **2s**. Not this PR: `master` has **0 successes in its last 30 runs**, every Ratchets run plus both cron jobs, the earliest an hour before that branch existed (`f333c28` Ratchets, 04:21:59→04:22:02Z, **3s**). The 2026-08-02 measurement below was real and is now superseded: that run finished at 00:41, eight minutes before the repo went private, and standard runners are free only on public repos — so every minute since drew on the account quota until it was gone. **`npm run gate` is once again the only thing guarding `master`.** Clearing the billing is founder-owned; the lean-CI split stays load-bearing for when it returns. Job logs are unreadable from an agent session (`403`/`404` on the Actions artifact API), so this row is measured from job metadata, not logs. |
...
- **`.700`:** (`2026.07-unified.700`) **Local design studio** — [docs/design/concepts/studio.html](docs/design/concepts/studio.html). Does **not** replace `/` or `/start`. `npm run design:concepts` serves `docs/design/` on **127.0.0.1:4177** (fixed; `EADDRINUSE` fails). 04 vs current `/start` at 390 and 1440; fold stills via `npm run design:stills`. Control frames skip when www is down. Type on 04 is still Barlow — sign-off is structure, not the live face.
...
- **`.644`:** (`2026.07-unified.644`) **Three concepts from zero** — [docs/design/concepts/](docs/design/concepts/index.html). The eight-variant board shared one DOM *by design*, and that froze the original wireframe into all eight. These share nothing: **01 The Week** (product IS the page, reader presses) · **02 Anywhere** (pinned scroll, 5 places, one unchanging plan, JS-required) · **03 Field Manual** (a document, hub-and-spoke, **no JS**). Copy identity was the wrong assertion — the build now asserts **structural distinctness** across 7 axes and refuses to write if two match. Found: **npm registry IS reachable** (lenis vendored) and **4 OFL type families on disk**, so "Archivo everywhere" was avoidable. Claim ban list is now a build check.
- **`.643`:** (`2026.07-unified.643`) **Eight design directions on a board** — [docs/design/variants/](docs/design/variants/index.html). A1 modernist (shipped) · B1/B2/B3 (three leashes: reds pinned · reds+type pinned · nothing) · C1 dossier · C2 instrument · C3 cinematic · C4 zine. **All eight render the same DOM from `sites/www/src/lib/homeContent.ts` and the generator refuses to write if any variant's visible text differs** — without that the board silently becomes a comparison of two pages, and the difference is invisible at the size the decision gets made. Lives in `docs/` because seven are illegal on `sites/www` by construction; exploring them there would mean allowlisting the core palette rule. Design-system scan 935→937, **both from `sites/www`** — the variants are invisible to it. **Founder picks; the winner is ported back, where every rule applies again and each page prints the cost.**
- **`.642`:** (`2026.07-unified.642`) **WWW `/start`** — a dedicated conversion page, additive (moves no SEO URL). Wave 11 §11.4 overturned the assumption: what separates a landing page from a homepage is **CTA discipline**, not length or stripping — Freeletics' Nutrition LP keeps full nav and runs as long as their homepage, but repeats one string 4× against the homepage's 5 different actions. Nav-less because that is what **auth/checkout** surfaces do. `/` hands off, `/start` terminates at the private gate (the only live capture; a static page cannot POST to `/api/leads` past `form-action 'self'`). **Both browser guards now sweep every emitted route** — `/start` would have shipped unmeasured, and `www-bundle-budget` hard-fails a route with no budget. `/start`: 93.6% image, 998px, 1 red action. Also killed a **vacuous MARKERS entry** in the class contract (its own stated reason made it unreachable) and recorded the `class:list` blind spot.
- **`.641`:** (`2026.07-unified.641`) **WWW composition** — the page matched the reference type scale AND the reference rhythm with five guards green, and still rendered as a **wireframe**: nothing had ever measured **imagery**. Wave 11 — refs 76–81% image area, 81–100% above the fold, 40–100% of text spans *on* the picture; this page was ~4% / ~0% / none. Rebuilt on photographic grounds with type over them, paper dominant, **one** ink field two-thirds down (`--foreground` as a ground, no new token), red close. `www-composition.mjs` guards four floors published in §11.5 *before* the build. Now **fold 100% · page 36.8% · 2 full-bleed · 24 text-over-image**, 2.1 KB JS, 1 red control, complete with JS off. 5 mutants — and two earlier ones were *bad mutants, not survivals*, recorded because those look identical in a terminal.
- **`.640`:** (`2026.07-unified.640`) **WWW link contract** — every CTA on the marketing homepage was a **dead link** (`/#invite`, an id that exists nowhere); the guard found 11 more on its first run (the whole compare rail, plus `/welcome`). All were written as if this were the Next app: `astro build` emits one HTML file, so `/compare/*`, `/welcome` and `/private` must be absolute to the Vercel origin. Targets now live in one module. **A dead link is invisible to every other guard and to a screenshot** — a button that scrolls nowhere photographs like one that works. 5 mutants. **Open, founder-owned:** which origin serves `www.missionwinning.com` — `astro.config.mjs` and `seoMetadata.ts` both claim it.
