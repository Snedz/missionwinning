...
- **`.1072`:** (`2026.07-unified.1072`) **Mission OS capability interfaces** — Named doors + in-memory fakes (`createIdentityFake`, `createBillingFake` Stripe HOLD, `createPhotosFake`, `createStorageFake`). `MiniHost.mount` happy-path unit tests. Continues #935 / `.1068`. No ClearShot UI. No remake of Today. No ClearShot in MW APK. Isolation holds. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
