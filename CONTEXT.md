...
## Now (2026-08-14 · web `2026.07-unified.810` · Android `1.24.1`)
...
- **`.810`:** (`2026.07-unified.810`) **Visibility Alpha frame** — Super Bundle row says mute-pay, not free-first beta. No `PRIVATE_MODE` flip.
- **`.809`:** (`2026.07-unified.809`) **Builder first-paint** — program count interpolates; empty saved matches pack. Drift cap 214. No `PRIVATE_MODE` flip.
- **`.808`:** (`2026.07-unified.808`) **Help first-paint** — title is Help, not Frequently asked. Drift cap 216. No `PRIVATE_MODE` flip.
- **`.807`:** (`2026.07-unified.807`) **www CTA is Alpha** — gated invite stays; “when the beta opens” gone. No `PRIVATE_MODE` flip.
- **`.806`:** (`2026.07-unified.806`) **Builder empty exit** — category empty goes to `/active`. Outline EmptyState. No `PRIVATE_MODE` flip.
- **`.805`:** (`2026.07-unified.805`) **CareerLine empty exit** — Your record empty goes to `/active`. Outline EmptyState. No `PRIVATE_MODE` flip.
- **`.804`:** (`2026.07-unified.804`) **Alpha copy leftover** — in-app “open beta” → Alpha. `isFreeBeta()` still mute-pay. No `PRIVATE_MODE` flip.
- **`.803`:** (`2026.07-unified.803`) **Planned rest** — Rest today on Mission progress. Bridges a streak. Never a session. Weekly goal stays boss. No `PRIVATE_MODE` flip.
- **`.802`:** (`2026.07-unified.802`) **Programs merch honesty** — Coach pointer; education outlines not a second store; $ prices stay behind !FREE_BETA. No `PRIVATE_MODE` flip.
- **`.801`:** (`2026.07-unified.801`) **In-app Help/FAQ** — `/help` from `docs/help` (no school/PFT). Footer link. Logger never gated. No `PRIVATE_MODE` flip.
- **`.800`:** (`2026.07-unified.800`) **Track strap honesty** — disclosure is Body & trends; no-strap line; card still null when wearables parked. No `PRIVATE_MODE` flip.
- **`.799`:** (`2026.07-unified.799`) **Astro `/compare` index** — points at Next vs-pages. Stories stay on Next. No `_redirects`. No `PRIVATE_MODE` flip.
- **`.798`:** (`2026.07-unified.798`) **Astro `/vision` only** — fourth-surface vision page. Next keeps the host and Next `/vision`. No `_redirects`. No `PRIVATE_MODE` flip.
- **`.797`:** (`2026.07-unified.797`) **Today Coach week dose + adapt** — week strip on Today shows the same dose line and compact adapt banner as `/coach`. No second red. No `PRIVATE_MODE` flip.
