...
- **`.747`:** (`2026.07-unified.747`) **Blue-team P0 harden** — signup mint + waitlist use `supportedRegions` (blocked ISO on any platform CDN header wins; Vercel prod denies missing country); mobile Coach uses `isPrivateModeEnabled()`; production query/stale cookie cannot skip the gate; account id is session-only; guest push cannot overwrite another account; analytics strips pregnancy/PT keys; notes stay text-escaped; `next=` allowlist tightened. Maps #540 P1-2/P1-3/P1-4. Excellence-Override: blue-team P0 harden.
...
- **`.714`:** (`2026.07-unified.714`) **Public GitHub pass / Beta 0.1** — mission sentence on README + About 01 + vision north star; public stamp `0.1 (beta)`; dual-repo honesty. Founder still owns visibility; `PRIVATE_MODE` unchanged.
