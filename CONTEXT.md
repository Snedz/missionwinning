...
- **`.961`:** (`2026.07-unified.961`) **Quiet week strip** — Mon–Sun glance on Today. Done days marked. Empty days stay empty. Next Start is still the one Start. No shame ✕. Guest. First set ungated. Blank notebook valid. Saved routine still owns Start. Wednesday still a cite. `/private` stays the tight lock. No `PRIVATE_MODE` flip.
