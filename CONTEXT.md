...
- **`.1003`:** (`2026.07-unified.1003`) **Delete this finished session** — open a finished session from History, confirm, then that one log is gone. Cannot be recovered. Empty / live / missing invents nothing. Does not wipe the account. Does not discard a live in-progress session. Other days stay. History / week strip / this-movement / PR diary skip the tombstone. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Merge `.1002` / Pause `.1001` / Backfill `.1000` / in-set PR `.999` / Reorder `.998` / History Edit `.997` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1002` (`f0b10181`).
...
- **`.992`:** (`2026.07-unified.992`) **Custom exercise** (title `.990`) — during a live session they can type a name the library does not have and keep logging. Unlimited. Free. Guest. First set ungated. Empty invents nothing. Stays on Repeat last / Start this again / the local diary. Not a public catalog. No video. No paywall. Today still one Start. Resume `.963` kept. Start this again `.991` still holds. Track trend `.989` last-vs-this stays on the strip cell. EMOM `.988` / drop-set `.986` / warmup `.985` / notes `.983` / 1RM `.981` / supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.991` (`d561b8a5`).
