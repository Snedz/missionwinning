...
## Now (2026-08-04 · web `2026.07-unified.386` · Android `1.24.1`)
...
- **`.386`:** Active `isActiveSetCell` extract + wiring guard.
- **`.385`:** Session more menu a11y — HoldToConfirm outside `role=menu` (aria-required-children).
- **`.384`:** BetaAdmin + landing chrome solidify; Loop 27 closed.
- **`.383`:** Active `holdsActiveExercise` extract + wiring guard.
- **`.382`:** Seeded axe on Active set options menu.
- **`.381`:** Fitness inputs + leaderboard flavor chrome; Loop 26 closed.
- **`.380`:** Active `firstPlannedSetIdx` extract + wiring guard.
- **`.379`:** Seeded axe on Active rest timer dock.
- **`.378`:** Off-palette red error chrome → destructive; Loop 25 closed.
- **`.377`:** Active `exerciseHasPlannedSet` extract + wiring guard.
- **`.376`:** Seeded axe on Active exercise note open.
- **`.375`:** HoldToConfirm + button/OTP hover chrome solidify; Loop 24 closed.
- **`.374`:** Active `exerciseHasCompletedSet` extract + wiring guard.
- **`.373`:** Form guide seeded a11y — AdaptiveOverlay scroll region + Avoid heading destructive ink.
- **`.372`:** Button/Input/Dialog/Otp + oauth chrome solidify; Loop 23 closed.
