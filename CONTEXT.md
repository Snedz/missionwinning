...
- **`.1057`:** (`2026.07-unified.1057`) **Left second bar + house sheet** — Home/Library open a left column next to the icon rail (72+264, hides &lt;723), not a far-right More sheet. Desktop wraps second + canvas in one white 12px sheet on grey. Compact unwraps it. On the sheet, Today starts with the session; Start hero is flush. Library catalog and History first-paint rows are house items. Weekly plan empty is house-empty; Generate stays the one filled dock on `/coach`. Train plus is a 40px white circle. Start → `/active`. Selected `#eee`. House DS is the signed-in token table. Field-manual stays on `/private` / www. Today has one filled action (Start). One Got it. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1056` (`9daa7d18`). Stamp stays `.1057`.
