...
- **`.1073`:** (`2026.07-unified.1073`) **Help is the FAQ, not a legal-index tour** — `/help` first paint is the FAQ. Legal footer is off the page. HELP_FAQ stays the object. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Feedback stays the form. Explore stays the board. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1072` (`11deecbb`). Stamp `.1073`.
- **`.1072`:** (`2026.07-unified.1072`) **Explore is the board, not a GPS tour** — `/explore` first paint is board + pin list + add a place. GPS nearby, session tag, and maps are off the page. Add a place stays the writer. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Feedback stays the form. Garage stays quiet title. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1071` (`b10307dc`). Stamp `.1072`.
- **`.1071`:** (`2026.07-unified.1071`) **Garage is quiet title, not a messenger tour** — `/server` first paint is quiet title. Rooms, buddy list, chat, and presence are off the page. Engine stays on disk. Log set stays filled. `generateWeek` stays on `/coach`. Fuel stays the log. Feedback stays the form. Coach never reads Garage. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1070` (`7ea67a72`). Stamp `.1071`.
