...
## Now (2026-08-25 · web `2026.07-unified.1011` · Android `1.24.1`)
...
- **`.1011`:** (`2026.07-unified.1011`) **Export this diary** — import + backfill dumped years; search finds a row; missing was a file they can take. History overflow saves the live diary as CSV (JSON is the same rows). Empty invents nothing — Save stays off. Tombs stay out. Start-from does not shrink the file. Not a Feed. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Library skip `.1010` / BW cite `.1009` / Search `.1008` / Name `.1007` / Restore `.1006` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1010` (`4d128292d`). Stamp stays `.1011`.
