...
- **`.967`:** (`2026.07-unified.967`) **RPE / RIR on the set row** — optional 1–10 RPE and/or RIR on a logged set. Never required to save. Cite / why / Wednesday quote the last work set when present. Empty stays empty. No invented number. Guest. First set ungated. Today still one Start. Resume `.963` kept. Fuel `.965` stays off Today. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Skips `.966` / `.968`.
