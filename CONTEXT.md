...
- **`.965`:** (`2026.07-unified.965`) **This week's restock they take** — Fuel Show more assembles this week's logged meals (or a typed list) into a keepable list they copy. Empty invents nothing. You shop. We do not order. Not on Today, Train, or `/private`. Guest. First set ungated. Today still one Start. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Rebased past `.963`; skips `.964`.
...
- **`.959`:** (`2026.07-unified.959`) **Skip / swap this exercise, this session** — taken cable: skip this one or swap to another movement once. Plan and Wednesday stay. Skip does not fail the session. Confirm-gated. Guest. First set ungated. Today still one Start. `/private` stays the tight lock. No `PRIVATE_MODE` flip.
- **`.958`:** (`2026.07-unified.958`) **Desk → gym, one session** — signed-in Train Start on laptop continues on phone Train. Same `clientId`. No Force Sync. Guest stays on this device until sign-in. Finish tombstones the open session. Confirm before replacing local logged work. `/private` stays the tight lock. No `PRIVATE_MODE` flip.
