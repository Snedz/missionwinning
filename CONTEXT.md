...
- **`.1053`:** (`2026.07-unified.1053`) **Product IA skeleton** — Bones, not lipstick. `docs/IA_SKELETON.md` locks LOG / WEEK / GARAGE, the existing room map, Horizon 0 vs later, and the refuse list. Isolation keeps `social/isolation.test.ts` + domainBoundary C1–C3 and now walks AppLayout chrome, Coach chat, and the `generateWeek` door. `/server` stays More → You. Dual-writer leak documented, not a join. No costume. No CSS / `/private` / www / nav-appearance edits. `[skip vercel]`. No `PRIVATE_MODE` flip. No production promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1052` (`895d940d`). Stamp stays `.1053`.
