...
- **`.985`:** (`2026.07-unified.985`) **Warmup batch** (concern `.984`) — from a working set, add a small batch of warmup sets (½ / ⅔ / ¾ of that weight). Simple fractions. Athlete can edit or delete any. Empty invents nothing. Warmups stay excluded from cites. No calculator paywall. No injury warmup. Guest. First set ungated. Today still one Start. Resume `.963` kept. Notes `.983` / 1RM `.981` / Supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` / cues `.973` / honesty `.971` / tags `.970` / RPE `.967` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.983` (`046fe67e`).
- **`.983`:** (`2026.07-unified.983`) **Private session notes** (concern `.982`) — optional notes on the live session and the close receipt. Strong-style: add notes if you have more. Empty invents nothing. Stored locally with the session. Not a Feed. No public URL. Guest. First set ungated. Today still one Start. Resume `.963` kept. 1RM `.981` / Supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` / cues `.973` / honesty `.971` / tags `.970` / RPE `.967` / Fuel `.965` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.981` (`81b23935`).
