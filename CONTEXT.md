...
- **`.1079`:** (`2026.07-unified.1079`) **Billing CapResult deny consistency** — When a mini lacks billing scope (`utility.clearshot`, `l1.health`), every billing method on the fake bus (`read` / `checkout` / `portal`) returns `{ ok: false, code: 'scope_denied' }`. Test-only `test.billing` gets a stub success path without Stripe. Not a product mount. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
