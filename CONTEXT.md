...
- **`.1080`:** (`2026.07-unified.1080`) **Photos CapResult deny consistency** — When a mini lacks photos scope (`l1.health`), every photos method on the fake bus (`read` / `write`) returns `{ ok: false, code: 'scope_denied' }`. `utility.clearshot` (has photos) stays `photos_stub` on every method. No camera / MediaStore / Android. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
