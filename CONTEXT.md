...
- **`.1006`:** (`2026.07-unified.1006`) **Restore a deleted session** — confirm-gated delete still tombs one finished History log; Restore on History brings that Monday back. Empty / not-deleted / missing invents nothing. Do not undelete a live session. Not wipe-account. After delete the detail stays on the tombstone. Overflow Deleted sessions when any tombstone exists. Revision wins, including clearing `deletedAt`. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Start-from `.1005` / Hide `.1004` / Delete `.1003` / Merge `.1002` / Pause `.1001` / Backfill `.1000` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1005` (`a6856d74`). Stamp stays `.1006`.
- **`.1005`:** (`2026.07-unified.1005`) **Start history from this date** — pick a date on History; older logs stay in the diary but fold out of the week strip / Coach / streak so week 1 is honest. Data stays. Fold, don't erase. Empty / missing / future invents nothing. Confirm-gated if it hides a lot of days. Clearing restores the full diary. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Hide `.1004` / Delete `.1003` / Merge `.1002` / Pause `.1001` / Backfill `.1000` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Rebased onto master `.1004` (`b48e54b0`). Stamp stays `.1005`.
- **`.1004`:** (`2026.07-unified.1004`) **Hide this exercise from the library** — a name stops appearing in Add / search / picker. Hidden ≠ deleted: past sets, PRs, notes stay. Unhide restores the list. Empty / missing / already-hidden invents nothing. Do not auto-hide lookalikes. A hidden movement already in a live session stays on that session. Library / overflow door. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Delete `.1003` / Merge `.1002` / Pause `.1001` / Backfill `.1000` / in-set PR `.999` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Rebased onto master `.1003` (`4c810d57`). Stamp stays `.1004`.
- **`.1003`:** (`2026.07-unified.1003`) **Delete this finished session** — open a finished session from History, confirm, then that one log is a tombstone. Restore is `.1006`. Empty / live / missing invents nothing. Does not wipe the account. Does not discard a live in-progress session. Other days stay. History / week strip / this-movement / PR diary skip the tombstone. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Merge `.1002` / Pause `.1001` / Backfill `.1000` / in-set PR `.999` / Reorder `.998` / History Edit `.997` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1002` (`f0b10181`).
- **`.1002`:** (`2026.07-unified.1002`) **Merge duplicate exercises** — pick a source and a keeper on History or library, confirm, then history / PRs / notes / tags / rest / 1RM / in-set PR diary ride the keeper. Cannot be undone. Empty / same-id / missing invents nothing. Do not auto-merge lookalikes. Live session: source becomes the keeper, or vanishes if the keeper is already there — sets travel. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Pause `.1001` / Backfill `.1000` / in-set PR `.999` / Reorder `.998` / History Edit `.997` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Rebased onto master `.1001` (`91938fc4`). Stamp stays `.1002`.
- **`.1001`:** (`2026.07-unified.1001`) **Pause the live clock** — tap elapsed on the Train bar to pause / resume the session clock. Pause stops accumulating. Resume clock starts from the paused total. Finish writes duration from elapsed-while-running only. Leaving Today does not auto-pause (Resume `.963` keeps the set). Rest `.995` and EMOM `.988` stay independent. Empty invents nothing. Free. Guest. First set ungated. Today still one Start. Backfill `.1000` / In-set PR `.999` / reorder `.998` / History Edit `.997` / note+pin `.996` / rest `.995` / set-row type `.994` / history `.993` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Rebased onto master `.1000` (`1ab24514`). Stamp stays `.1001`.
- **`.1000`:** (`2026.07-unified.1000`) **Backfill a past session** — if they trained and never opened the app, log that session from History with the date it happened. Optional start/end; timing off omits duration. Empty invents nothing. Not Resume. Not Edit. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. In-set PR `.999` / Reorder `.998` / History Edit `.997` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.999` (`fb7f9d34`).
- **`.999`:** (`2026.07-unified.999`) **In-set PR they hit** — after a working set that beats a number they already logged (heaviest / most reps / best logged 5), a quiet PR on that live set. First session invents nothing. No projected 1RM banner. vs-last stays. e1RM hide stays. Honesty `.971` still scores Train session count. Free. Guest. First set ungated. Today still one Start. Resume `.963` kept. Reorder `.998` / History Edit `.997` / note+pin `.996` / rest `.995` / set-row type `.994` / history `.993` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.998` (`8d5d1c1d`).
- **`.998`:** (`2026.07-unified.998`) **Live session reorder** — in the live Train session they can drag the lifts to a new order. Sets travel with the card. Tap the name still opens this-movement history. Empty invents nothing. Honesty `.971` still applies. Free. Guest. First set ungated. Today still one Start. Resume `.963` kept. History Edit `.997` stays on History. Note+pin `.996` / rest `.995` / set-row type `.994` / history `.993` / custom `.992` / Start this again `.991` stay. Swap/skip stay the replace path. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.997` (`bb13642b`).
- **`.997`:** (`2026.07-unified.997`) **Edit a finished session** — open a finished session from History, edit the sets they actually logged, then Save. Confirm before a destructive change. Empty invents nothing — does not wipe the session. Resume `.963` stays the live set. Honesty `.971` still applies. Free. Guest. First set ungated. Today still one Start. Note+pin `.996` / rest `.995` / set-row type `.994` / history `.993` / custom `.992` / Start this again `.991` / Track trend `.989` / EMOM `.988` / drop-set `.986` / warmup `.985` / notes `.983` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.996` (`2be854e6`).
- **`.996`:** (`2026.07-unified.996`) **Exercise note + pinned reminder** — on the open Train lift they write a note for this movement, and can pin a reminder that returns next time. The note is theirs. The pin is theirs. Last History note is not a pin. Pin does not appear on History. Empty invents nothing. Honesty `.971` still applies. Free. Guest. First set ungated. Today still one Start. Resume `.963` kept. Rest `.995` / set-row type `.994` / history `.993` / custom `.992` / Start this again `.991` / Track trend `.989` / EMOM `.988` / drop-set `.986` / warmup `.985` / notes `.983` stay. Cues `.973` stay ours. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.995` (`8f7f19c9`).
- **`.995`:** (`2026.07-unified.995`) **Per-exercise rest** — rest duration lives on the exercise; warmup vs work can differ. Bench can rest 3:00 while laterals rest 1:00. Empty invents nothing. Honesty `.971` still applies. Free. Guest. First set ungated. Today still one Start. Resume `.963` kept. Set-row type `.994` still honors weight · bodyweight · duration · assisted. History `.993` / custom `.992` / Start this again `.991` / Track trend `.989` / EMOM `.988` / drop-set `.986` / warmup `.985` / notes `.983` / 1RM `.981` / supersets `.980` / Learn `.978` / week strip `.977` stay. Global Default stays the dock fallback — not a second home. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.994` (`bc03b98e`).
- **`.994`:** (`2026.07-unified.994`) **Set-row type** — the open Train row honors the movement: weight · bodyweight reps (optional +kg vest) · duration · assisted. Empty / custom leftover stays weight × reps. Vest is extra only — never Track / profile bodyweight. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. This-movement `.993` / custom `.992` / Start this again `.991` / Track trend `.989` / EMOM `.988` / drop-set `.986` / warmup `.985` / notes `.983` / 1RM `.981` / supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.993` (`75350127`).
- **`.993`:** (`2026.07-unified.993`) **This-movement history** — tap the open lift to see prior sessions of that movement (their diary). Empty / short list invents nothing. Honesty `.971` still applies. Not a projected-max chart. Not another human's number. Not a Feed. Not a shame slope. Optional. Guest. First set ungated. Today still one Start. Resume `.963` kept. Custom `.992` / Start this again `.991` / Track trend `.989` / EMOM `.988` / drop-set `.986` / warmup `.985` / notes `.983` / 1RM `.981` / supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.992` (`7bb7c464`).
