...
- **`.1022`:** (`2026.07-unified.1022`) **Heatmap empty-load volume is reps, not 0** — spark `.1020` already scores `8 × 0` as 8; anatomy still did `reps * weight`. `workingSetVolume` is the one helper: empty load is reps, loaded is kg. Heatmap + spark share it. History kg totals stay kg. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Chat cite `.1021` / spark `.1020` / never-trained `.1019` / month `.1018` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1021` (`710d0b7de`). Stamp stays `.1022`.
