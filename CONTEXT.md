...
## Now (2026-08-15 · web `2026.07-unified.846` · Android `1.24.1`)
...
- **`.846`:** (`2026.07-unified.846`) **Half the "missing" translations were never missing** — `i18n-coverage` built its EN set from `LOCALE_EXPORTS`, so five unexported `*Locales.ts` packs were invisible; 10 `localeChooser*` keys were reported untranslated while carrying 21–40 translations each. `englishKeys()` now discovers every pack and **throws** on one with no `*StringsFor`/`*OverlayFor`. 74 → 46. Added the 24 genuinely-absent EN keys (46 → 22); no component edits — the call sites already interpolated, under different variable names. Machine translation is **blocked by network policy** (MyMemory + `translate.google.com` `connect_rejected`), so keys stay EN-only; `today/fr` crossed its placeholder cap and was translated directly, marked agent-written. **22 remain — all `BundleShopStack`, which nothing renders**, scoped to its own PR. 3 mutants killed. No `PRIVATE_MODE` flip.
...
- **`.841`:** (`2026.07-unified.841`) **Guards match shipped code** — `master` was 3454/3 red and unseeable (Actions billing-blocked; `tsx --test` exits 0 while printing `# fail 3`). Two guards read `defaultValue` fallbacks, so they banned copy the packs have shipped all along; `/log` left `MUST_STAY_GATED` after `.839`. Arrows stay in the packs — a founder copy call. 5 mutants killed. No `PRIVATE_MODE` flip.
- **`.840`:** (`2026.07-unified.840`) **GNT-2 opened; Coach week is magnitude-blind** — `coachEval.test.ts` sweeps 0→20 logs and finds one boundary, not a gradient: `48 sets/0 rec` cold, `34/2` for *every* non-empty history. Floor 2 documents it; raising it is the U1 brief. GNT-1 `ready-for-founder`. No `PRIVATE_MODE` flip.
