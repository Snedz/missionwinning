...
- **`.1074`:** (`2026.07-unified.1074`) **Health mini slug** — `l1.health` at `mission://minis/health` (last-segment `health`; not opaque `mini`). Identity + storage only. Not `health.train`. No Health UI. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
