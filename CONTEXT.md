...
- **`.978`:** (`2026.07-unified.978`) **Quiet Learn** — `/learn` first paint is one free intro (existing Strength Basics `sb-0`: log a set, then Coach from those logs). Other paths under Show more. Cue list can open it when they want more than a rack card. Empty invents nothing. Not on Today, Train-as-home, or `/private`. Guest. First set ungated. Today still one Start. Resume `.963` kept. Week strip `.977` / Quiet Track `.976` / Move `.974` / cues `.973` / honesty `.971` / tags `.970` / RPE `.967` / Fuel `.965` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.977` (`79791d8b`).
