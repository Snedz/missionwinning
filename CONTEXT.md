...
- **`.956`:** (`2026.07-unified.956`) **E-Victory close receipt** — after Finish, first paint is the private keepable receipt (sets, load, vs last if we have it, duration if we have it). Empty session invents none. Guest. No public permalink. Today / next-day cites untouched. No `PRIVATE_MODE` flip.
