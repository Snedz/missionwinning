...
- **`.951`:** (`2026.07-unified.951`) **Hevy-native diary in** — workouts + measurements on the same Account path. Wide `measurement_data.csv` (`date` / `weight_kg` / `fat_percent`). Preview + confirm. Existing native fields win. Guest. No one-import cap. No zip. No export-layout vanity. Does not touch plate math / `#786`. No `PRIVATE_MODE` flip.
