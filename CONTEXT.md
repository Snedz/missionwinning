...
## Now (2026-08-13 · web `2026.07-unified.740` · Android `1.24.1`)
...
> and `.740` dropped `.636` detail (full text remains in LOG.md / archive)
...
> | Migrations | **10 recorded pending** — [LAUNCH_RUNBOOK](docs/LAUNCH_RUNBOOK.md) §3, enforced by `src/lib/migrationLedger.test.ts`. Includes `20260813_week_logged.sql` (CoS applies). |
...
- **`.740`:** (`2026.07-unified.740`) **Week-4 retained logger events** — `set_logged` / `week_logged` on working sets (warmup excluded); this-install `retained_week_4` on `/account` Under the Hood; guests local-only; [docs/METRICS.md](docs/METRICS.md). No invented traction.
