...
> | Migrations | **P1–P12 + `mission_ids` + `feedback_reviews` applied 2026-08-14** on production `missionwinning` (tables + tombstone columns + `mw_week4_retention` verified). Week-4 proof SQL not run via Management API (transactional `auth.users` fixture). Next file still goes in the runbook. |
...
- **`.899`:** (`2026.07-unified.899`) **Builder first paint is Blank workout** — one red start, saved routines if any. Program templates and sign-in in Show all. Same drafts. No `PRIVATE_MODE` flip.
- **`.898`:** (`2026.07-unified.898`) **Library first paint is a pick list** — search + filters + rows you can pick. Posters, blurbs, templates in Show all. Same catalog. No `PRIVATE_MODE` flip.
- **`.897`:** (`2026.07-unified.897`) **History first paint is the log** — search + rows. Calendar, charts, journal, glance, pillar wins in Show all. Same sessions. No `PRIVATE_MODE` flip.
- **`.896`:** (`2026.07-unified.896`) **Coach first paint is this week’s session** — week strip + one Start. Grid, cite, manage, voice, load, chat in Show all. Same engine. No `PRIVATE_MODE` flip.
- **`.895`:** (`2026.07-unified.895`) **Search is a field over the rooms** — type to find a room; Fuel/Coach/Train pin to Summary from the row. No `/search` route. No `PRIVATE_MODE` flip.
- **`.894`:** (`2026.07-unified.894`) **Fuel first paint is a notepad** — remaining + type-in + today’s lines. Recipes, week, wizard, photo, sign-in in Show all. No floating Log food. No `PRIVATE_MODE` flip.
- **`.893`:** (`2026.07-unified.893`) **Pins tap and Edit persists** — session pin starts/resumes; Fuel/Coach optional doors. Highlights count today’s sets from the log or stay empty. Default still one pin. No `PRIVATE_MODE` flip.
- **`.892`:** (`2026.07-unified.892`) **Cold chrome is Summary + Search** — dock is Summary + Search; Train tab only while a session is live. Lean Today is date, pins, one highlights sentence, one Start. Journey strip / rewards / continuity / day-review cut from lean. No `PRIVATE_MODE` flip.
- **`.891`:** (`2026.07-unified.891`) **Today Summary is one pin** — lean `/log` mounts a 0–4 pin grid (cold default one). Encourage + coach-invite off lean. Start stays the docked red field. No `PRIVATE_MODE` flip.
- **`.890`:** (`2026.07-unified.890`) **Today is Summary: Start, not First Steps** — tour card off both Today shells. Checklist stays in More. Re-entry quiet line stays. One poster-red Start. No `PRIVATE_MODE` flip.
- **`.889`:** (`2026.07-unified.889`) **Train set table is the phone atom** — `/active` enters sets in `SetLogTable` on every surface; dock is rest-only. Table is `table-fixed` at 390 (no sideways scroll). Same log, same outbox. No `PRIVATE_MODE` flip.
- **`.888`:** (`2026.07-unified.888`) **Competitor names stay in ops** — public tip no longer names other consumer fitness apps. Workout CSV is generic; vs-routes redirect; `names:check` when ops is mounted. Named intel + Apple Journal stay in `ops/intel/`. Later GTM compare posts are a founder flip. No `PRIVATE_MODE` flip.
- **`.884`:** (`2026.07-unified.884`) **H01 www composition floors pass** — `/about` 15.1%, `/compare` 0 images, `/vision` 20% against a 35% page floor. Same three photographs as grounds, not a fourth pack. Stamp `COMPOSITION_PASS.md` written by the check. Path hop is a gauntlet unit (bar = `www-composition`). No `PRIVATE_MODE` flip.
