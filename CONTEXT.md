...
## Now (2026-08-10 · web `2026.07-unified.644` · Android `1.24.1`)
...
- **`.644`:** (`2026.07-unified.644`) **Today health chips + pillar labels** — CrossPillarCoachChips uses insight/action floors (not actionLabel as message default); PillarScoreBreakdown never paints raw keys (`train`→`Train`).
- **`.643`:** (`2026.07-unified.643`) **Daily coach insight floors** — useDailyCoachInsight never paints raw message/action keys; cross-pillar floors.
- **`.642`:** (`2026.07-unified.642`) **Coach why floors** — adapt banner "Why today's plan" never blanks on hydrate.
- **`.641`:** (`2026.07-unified.641`) **Victory next single source** — mw-core owns pickVictoryNextAction (week-1 session 2 first).
