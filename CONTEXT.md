...
- **`.701`:** (`2026.07-unified.701`) **Exquisite gated www** — four cinematic scenes on `/private` + post-unlock `/`. SET is a full-viewport LOG SET field (not a split widget). Cover: Train Anywhere. Win Daily. / Log a set. Offline. Order SET → Anywhere → Week → Door. HUD nav (no bar). Quiet L2 after the door (athlete page you author; not a feed). One red LOG SET. CTA pack Free beta / Enter with code / Get notified. Draft; no Preview until Hobby ~Thu 14:30 ET. [docs/design/WWW_NIGHT.md](docs/design/WWW_NIGHT.md).
