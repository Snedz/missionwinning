...
## Now (2026-08-08 · web `2026.07-unified.617` · Android `1.24.1`)
...
- **`.615`:** (`2026.07-unified.615`) **Public OSS scrub + dual-repo.** War-room/GTM full text → private mission-ops (stubs in product); `secrets:scan` 0; contracts + mw-core economy/module; classification guard; [SECURITY_PUBLIC_OSS_AUDIT_2026-08-08.md](docs/SECURITY_PUBLIC_OSS_AUDIT_2026-08-08.md). Visibility flip still founder-owned.
- **`.614`:** (`2026.07-unified.614`) **The Account/You split** ([docs/IDENTITY_SOCIAL_PLAN.md](docs/IDENTITY_SOCIAL_PLAN.md) S2). `/profile` was a settings screen wearing the nav label "You" — and `.610` had just added the Athlete Card to it. Settings moved to **`/account`**; `/profile` became the **Athlete Page** carrying `.610`'s card as a block — Identity (call sign, previously editable only from the parkable `/leaderboard`) · The line ([`careerLine.ts`](src/lib/careerLine.ts): sessions, volume, exercises, best week, days, tombstones excluded) · the shipped badge shelf. Local only. `/profile` pinned at **0** red actions — a record is not a task. **Re-exporting locales for one new namespace found a 26-key i18n hole**: every badge and rank title `rewards/catalog.ts` emits was defined in **no catalogue**, surviving only in a stale committed `today.json` — so badges and ranks read English in all 15 languages since `.505`–`.543`. `.252`'s `coachWhySteadyWeek` again; invisible to `i18n:parity` (packs vs packs) and `i18n:coverage` (greps `.tsx` literals, never a data table). New `rewardsKeyParity.test.ts` derives the set from `BADGE_DEFS`/`RANKS`. Second collision fixed: `learnExpandedBanner` was claimed by two namespaces, guidebook merged last, so Learn shipped copy its own `defaultValue` contradicted. Tests 2254→**2271**, coverage floor 391→**394** (lockstep). **Bundle budget still red and nudged +0.1/+0.2 KB** — pre-existing, still its own PR.
- **`.613`:** **A hero test that was really asking what time it is.** `workout complete updates Mission Score` had no pinned clock, so it asserted a layout Today only produces before 18:00. Today spends `TODAY_MAX_TOP_LEVEL_BLOCKS = 6` and `dashboard` is priority **32**, behind `day-review` (**15**, from `DAY_REVIEW_START_HOUR = 18`) and `week-recap` (**30**, on activity *or* week-end) — Kaizen K1's explicit choice (`.294`) that the Mission Score *should* spill into "Today details" on the densest evening rather than crowd the session. Spilled, the band sits in a collapsed `<details>`: in the DOM, `hidden` forever; the failure snapshot shows `group: Today details +` with the band inside. On a Saturday after 18:00 the test read a deliberate product decision as a defect. **Falsified both ways on one unchanged build** — pinned 19:30 fails in 23.1s with the lane's identical `Received: hidden`, pinned 09:30 passes in 3.9s. **`.612` blamed three innocent commits**, because it had passed on `master`/`.608` and only this branch had changed: every fact true, conclusion wrong — *what changed in the repo* and *what changed in the world* are different questions and only one shows up in `git diff`. "Passes alone, fails in the lane" was read as load; the honest reading is the two runs happened at different times, and the isolated one raced `belowFoldReady`'s 1200ms idle callback before the evening blocks mounted. `.211` had already paid for this class; its `fixedTimeAt` sat in `first-90.spec.ts` where no other spec could reach it, and now lives in `tests/e2e/helpers/fixedClock.ts` with `UNCONTENDED_HOUR`. **Assertion not loosened** — the band must still be visible above the fold with a real number; it just says which hour it asks about. Hero **71/71**; a11y 63/71, the same 8 `master` carries.
- **`.612`:** **The Athlete Card did not pay its costs.** `.610` broke three `@gate` specs and nothing caught it for two ships, because the gate aborts at step 16 on a pre-existing bundle red and **steps 17–18 are therefore unreachable through `npm run gate`** — they ran here only out-of-band, against a production build with the gate's own `BUILD_ENV`. Chips shipped `size="sm"` (36px) and the share button default `h-10` (40px) against `thumbSweep`'s 44px floor, while two chip rows already on `/profile` use `min-h-[44px] tap-target`. The share button also took the default red fill, putting **2 red actions on a route capped at 1** — the exact class `.241` fixed repo-wide at ~20 sites, `/profile` named among them, reintroduced in a new file. And `.610`'s cosmetics catalog compiled into `/active` (via `WorkoutVictorySheet` → `shareCard.ts`) for frames a victory card can never have; painters moved to `src/lib/share/cardCosmetics.ts` and injected as closures. **The estimate was wrong and is corrected in the LOG:** `/active` 443.5 (`.608`) → 444.6 (`.611`) → **444.3** (`.612`) — the catalog is verifiably gone (literals now only in `/profile`'s chunk) but was worth **0.3 KB**, not 1.1; gzip dedupes canvas calls against the renderer already there, so raw source predicts nothing. The residual ~0.8 KB is **not attributed**. **The guard for that could not see its own defect** — it matched `from '…'` only, so a bare side-effect `import` survived the mutant; now all four spellings, each falsified. One hero spec stayed red here and **this bullet blamed the wrong thing — see `.613`**. Hero 68/71 → **70/71** on the two `/profile` repairs; the third was never this branch's. 10 mutants / 6 classes; tests 2246→**2252**.
- **`.611`:** **A name goes to strangers with nothing checking it.** `/leaderboard` has published a 24-char free-text name to every signed-in user since the full-launch override, and `saveOperatorName` was `trim().slice(0,24)` — no validation at all, on the one string the product shows other people. `CLUB_PLAN:188` says a reserved-word list + report flow *"must ship with the first public board"*; the board shipped, they did not. **Deliberately not a profanity filter** — a word list pretending to cover 15 languages is `.219`'s shape (a check that cannot do its named job), so the module says what it does not do and **a test asserts an ordinary rude word passes**, failing the day someone adds a list without updating the copy. Covers the tractable half: **impersonation** ("Mission Winning Support" beside a score is a phishing surface) and **injection** (URLs, controls, zero-width joiners, bidi overrides — one reverses every row after it), folding homoglyphs and leetspeak first. **My first matcher was wrong and this file's own test caught it:** substring matching rejected `Rooted Oak` and `Officially Tired`; generic words now match on word boundaries, only brand terms on the collapsed form. Rejection says why, restores the stored name, and is announced. **"Call sign" → "Name"** — A7 calls the military register cheap to soften and a visible identity layer is where that stops being true; the ranks were already civilian-friendly. 4 mutants; tests 2238→2246. Server half (eligibility trigger, `board_visible`, opt-in OFF, age band) queued — 9 migrations already unapplied, so it would ship inert.
- **`.610`:** **The Athlete Card — identity you can wear and send.** Asked to improve the social aspect (MySpace, WeChat). Audits found **exactly one mechanic in the codebase puts one human's data on another's screen** (`leaderboard_snapshots`) — no follows/friends/feed/comments/posts table, and no dead code for any — while **1 of 19 profile cards was identity**: no avatar, no name on `/profile`, no public profile, and rank + 13 badges stuck in `localStorage`. **Not a feed**, and not because a doc refuses it: a feed needs density we lack, moderation is an operational cost that lands on one person forever, and `shareCard.ts` already had the better answer — *"ours is share OUT"*. **Correction that shaped it:** cloud-persisted rewards would be a **client grant API** (upload your XP, forge your rank — `CLUB_PLAN:72`) *and* unnecessary, since every XP source is already server-visible, making it a recompute not a sync. So the card is **100% local** — no table, no migration, no route, no policy change. **Customization is the design system's own vocabulary getting denser** (hairline→rule→double→poster), so off-palette is impossible, and **picks-from-sets means no free text and no uploads** — moderation-safe by construction. **The clamp is the security model:** `resolveCardCosmetics` (in mw-core, Android needs the same mapping) falls back to the earned default rather than render a locked pick, and badges are passed in rather than trusted — a badge is the one thing on a shared PNG nobody else can check. No unlocks table: it would be a denormalized copy of a pure function. 9 mutants; tests 2221→2238.
- **`.609`:** **The privacy policy could not be edited.** One constant did two jobs: the pages rendered `CURRENT_PRIVACY_VERSION` as "Last updated" and `hasValidPrivacyConsent()` compared it. So fixing a typo meant either **re-prompting every device for consent** or shipping a date that lies — and faced with that, nobody edits the policy. A stale privacy page is the exact failure the consent record exists to prevent, and the record was the mechanism guaranteeing it. Split into `*_DISPLAY_DATE` (any edit) and `*_CONSENT_VERSION` (material change only). **Two guards, because the values are equal today so a value check cannot see the coupling come back:** the consent version may lag the shown date but never lead it (a user may not accept a version the page has not admitted to publishing), and a source scan fails if `hasValidPrivacyConsent` mentions a display date. Prerequisite for any policy text change. 2 mutants; tests 2219→2221.
- **`.608`:** **The load model gets a screen.** `coach/load.ts` — Foster sRPE + EWMA acute:chronic from logged sets, `null` under 14 days, cap-only into planning via `loadGuard` — had **zero `.tsx` consumers**. It reached planning, the LLM context and one post-session debrief line; no athlete could go and look at the thing `.606` just promoted into the wedge's second beat. New `CoachLoadBand` on `/coach`, with the decision in the pure, unit-tested `resolveLoadBandView` (`.598`'s pattern) rather than a JSX branch — and it asks `loadBands` whether there is enough evidence instead of re-deriving, so the screen cannot drift from the model. **The refusal is the feature:** under 14 days it says "not enough history yet" plus days remaining, never a zero or a dash dressed as a reading — a ratio on day three is `.602`'s seeded 50/50/50 in a lab coat, and worse, because load *looks* like physiology. Copy stays descriptive per `REDTEAM` A8. 3 mutants (speak anyway; never speak; blank empty state). Coverage floor 391→392, documented escape hatch, lockstep. Tests 2216→2219.
- **`.607`:** **The Mission Score decays.** A third of the score was permanent, on the number the whole six-pillar thesis rests on. `totalSessions`/`totalVolume` were **lifetime** at both call sites, `savedCount` counted templates saved ever, and `countLearnLessonsThisWeek` unioned three **undated** id sets — sessions 10 + volume 8 + saved 4 + Learn 10 = **≈32/100 that never decayed**. A lapsed athlete scored about what a hard week scored. `CLUB_PLAN` calls it "a weekly grade that resets" and reserves the odometer role for points; a third of the grade was an odometer. **The Learn term sat four lines beneath `countPillarWinsThisWeek`** — the file held both the defect and its cure; all three Learn paths already log a `'learn'` win. **Params renamed on purpose** (`sessionsThisWeek`/`volumeThisWeek`): a caller holding a lifetime figure now fails typecheck, which is exactly what caught the test file while production compiled. Lifetime figures stay where they are still right — the `.602` em-dash guard really does ask "ever trained". **The rebalance refused the obvious move:** weighting tonnage would break `CLUB_PLAN`'s promise that a bodyweight athlete in a Lagos park earns what a barbell athlete earns, so days (16) + sessions (12) carry Train and tonnage is a 4-pt bonus — three training days still beat four mobility flows. **Guard runs end-to-end from a 25-session career** because the arithmetic was never wrong, the callers were (`.597`'s shape). **A mutant found a missing test**: reverting Learn killed nothing — `pillarScoreInputs.ts` had no colocated test; the new one kills the revert *and* the lazy `return 0`. 6 mutants; tests 2207→2216. Local scores will drop — that is the fix.
...
- **`.605`:** (`2026.07-unified.605`) **The Log↔Social boundary, as contracts.** The week is planned from logs alone, so an athlete who withholds a bad session makes the planner prescribe for someone who does not exist — social comparison is an input-integrity attack, not a tone problem (Kolnes et al. 2026, n=225: runners *deleted their own sessions* over slow pace). New `domainBoundary.ts`/`.test.ts` enforce **C1** planner blindness (transitive, static + dynamic, type imports included), **C2** logger emits through a declared **door symbol** and discards the result, **C3** no social primary tab, **C7** first run clean; **C4** is a `club-identity` axis in `reentryTone.ts`. The contract as first drafted **contradicted its own diagram** — it forbade the emit `workoutStore` has always made — and was retyped around readers vs emitters. The door is a *symbol*: `leaderboardSync` exports both a push and a fetch. Repo's first real import-graph guard; `launchTruth`'s depth-3 static DFS was not a wall. **12 mutants killed**, zero exemptions, tests 2203→**2214**, no new i18n key. Plan: [docs/IDENTITY_SOCIAL_PLAN.md](docs/IDENTITY_SOCIAL_PLAN.md) · research + source verification: [docs/DESIGN_RESEARCH.md](docs/DESIGN_RESEARCH.md) Wave 9 · third design brief: [docs/DESIGN_PROPOSAL_3.md](docs/DESIGN_PROPOSAL_3.md). **Founder-open:** S2 (`/profile`→Athlete Page, settings→`/account`), and striking the T4 season-start boost.
