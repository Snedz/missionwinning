...
- **`.991`:** (`2026.07-unified.991`) **This session becomes a Start** — from the close receipt or History they can Start this again (same movements, last loads as targets). Empty invents nothing. Not a shop. Honor notebook `.960` stays program-in. Guest. First set ungated. Today still one Start. Resume `.963` kept. Track trend `.989` / EMOM `.988` / drop-set `.986` / warmup `.985` / notes `.983` / 1RM `.981` / supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` / cues `.973` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.989` (`adad3e58`).
