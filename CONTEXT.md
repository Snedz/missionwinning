...
- **`.960`:** (`2026.07-unified.960`) **Honor the notebook they brought** — save one named routine they typed, finished, or picked from History. Start uses it. Wednesday from logs does not overwrite a saved PPL. Blank notebook still logs. Confirm-gated. Guest. First set ungated. Today still one Start. `/private` stays the tight lock. No `PRIVATE_MODE` flip.
