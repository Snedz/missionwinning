...
- **`.941`:** (`2026.07-unified.941`) **Account-lite auth harden** — `SIGNED_OUT` no longer wipes a guest log; only an explicit leave does. First set stays ungated. Mission ID mint stays server-only. Rebased past `#779` `.940`. Do not steal `.935` `#778`. No `PRIVATE_MODE` flip.
- **`.940`:** (`2026.07-unified.940`) **Strong CSV import** — English export, preview + confirm, skipped rows counted. More than one file. No account. Rebased past `#777` `.939` + Dependabot. No `PRIVATE_MODE` flip.
- **`.939`:** (`2026.07-unified.939`) **Next-set cite after a logged set** — skippable load/reps or last rest on the Train set row, cited from the log. Empty history does not invent. Supersedes closed #487 (engines + last-live reader; TARGET-above-PREVIOUS stays unmounted). Rebased past `#775` `.938`. No `PRIVATE_MODE` flip.
