...
## Now (2026-08-03 · web `2026.07-unified.308` · Android `1.24.1`)
...
- **`.308`:** `resolveRepeatLastTarget` extract + solid onInk/outline/Fuel/footer chrome.
- **`.307`:** Fuel adapt/targets/weight/recipes leftovers i18n; uncovered **447→381**.
- **`.306`:** Mind check-in / breathing / page i18n batch; uncovered **473→447**.
- **`.305`:** Zero-state **`/move` 1→0**; Move + assess submit/risk i18n; seeded TimedFlowRunner a11y; uncovered **489→473**.
- **`.304`:** Soft chrome on Coach/Active ink (solid borders + solid hovers); Fuel search/describe/estimate i18n; uncovered **515→489**.
- **`.303`:** Active `resolveActiveSetDial` — prescribed vs freestyle carry/suggestion one helper + wiring guard.
- **`.302`:** Victory debrief replies are **stable ids** + i18n labels; SessionDebriefCard chrome translated.
- **`.301`:** Coach Adjust/chat/today + History + Active set/jot i18n; uncovered **566→515**.
- **`.300`:** Zero-state **`/track` 3→1**, **`/mind` 2→1**; Victory seeded a11y; heatmap aria-label; Today soft-chrome residue cleared.
- **`.299`:** i18n L4 batches — Active chrome/session + photoLog/fuelQuick + week-recap/debrief; uncovered **647→566**.
- **`.298`:** Fuel NL **tsp / ml / plate of** + `mealDraft` extract (`api`→database guard); bare portion words stay rough.
- **`.297`:** Active `buildConsoleSet` + `planApplyTargets` extracted — logger decisions unit-tested; wiring guards keep them off the page.
- **`.296`:** Victory + SessionCheckIn i18n — ~33 keys into `activeWorkoutLocales`; finish toast desc no longer English-only; uncovered **680→647**.
- **`.295`:** Seeded a11y History/Coach green; anatomy links named; zero-state **`/builder` 3→1**, **`/profile` 4→1**; Active `rankSwapCandidates` extract; i18n uncovered **685→680**.
- **`.294`:** Today composure — Mission Score **spills** on densest evening (`dashboard` 22→32; top-level budget **7→6**); Fuel photo heuristic **never claims high** + DB picks log as `api` (not heuristic).
