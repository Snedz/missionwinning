...
- **`.1056`:** (`2026.07-unified.1056`) **Signed-in desk names the three loops** — Rail is Log / Week / Catalog / You. Today first-paints a Week door to `/coach`. Library + Builder share catalog tabs. History joins the isolation walk. Dashboard stays parked. One Start. Start order unchanged. Visual preview — do not merge, do not promote. `[skip vercel]`. No `PRIVATE_MODE` flip. Live www stays `.696`. Guest. First set ungated. Past master `.1055` (`e44351c3`). Stamp stays `.1056`.
...
- **Alpha 0.1.0 · mute-pay:** LLC + EIN pending — Super Bundle shop merchandised, checkout muted; full depth unlocked; More **depth pillars demoted until first workout** (F-004 / `.695`; Fuel stays). Offer is free tracker or Super Bundle ([docs/FREE_BETA.md](docs/FREE_BETA.md)).
