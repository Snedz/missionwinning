...
- **`.1085`:** (`2026.07-unified.1085`) **CapResult unknown-method consistency** — Calling a method outside the closed set on a declared door returns `{ ok: false, code: 'unknown_method' }` (`identity.foo`, `billing.cancel`, `photos.delete`, `storage.clear`). Undeclared door + the same unknown name stays `scope_denied` (deny-before-unknown). Known methods keep `.1079`–`.1084`. No UI / Stripe / camera. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1084`:** (`2026.07-unified.1084`) **CapResult allow-path consistency** — When a mini declares billing / photos / identity / storage, every granted method returns the existing stub envelope (`ok` muted/held / guest snapshot / storage map; photos stay `photos_stub`). Test-only `test.granted` is the allow fixture. No UI / Stripe / camera. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1083`:** (`2026.07-unified.1083`) **Storage CapResult deny consistency** — When a mini lacks storage (`test.nostorage`, also `test.billing`), every storage method on the fake bus (`get` / `set`) returns `{ ok: false, code: 'scope_denied' }` and does not write the map. `l1.health` keeps stub success; `utility.clearshot` `set` succeeds, `get` stays `scope_denied` (no `storage.read`). No durable browser write / Stripe / camera / Android. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
