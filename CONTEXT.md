...
- **`.1020`:** (`2026.07-unified.1020`) **Library spark is reps, not 0, on empty load** — spark still did `reps * weight`; cite already prints BW; store is `weight: 0`, so eight push-ups plotted as a flat zero. Empty-load working sets score as reps. Loaded stays kg. Warmups stay out. Tombs stay out. Empty invents nothing. Display only — store still `weight: 0`. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Never-trained `.1019` / Month `.1018` / Empty-load `.1017` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1019` (`cdb1b5a5e`). Stamp stays `.1020`.
