...
- **`.957`:** (`2026.07-unified.957`) **Tight `/private` lock** — gated first paint is hero + notify + Enter with code again. Four-scene field unmounted. Copy lock stays Log a set. Offline. F-039 and `/notify` stay. Cookie `/` stays `.696`. No `PRIVATE_MODE` flip.
