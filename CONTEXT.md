...
> | Fact | State |
> |---|---|
> | **REDTEAM A5 falsifier** ([REDTEAM.md](docs/REDTEAM.md)) — *"14 days… still no 10 beta users"* | **FIRED** as a measurement — still no 10 beta. **Not a public-flip gate.** Launch shape is a **public alpha** when the founder says the product is ready ([ORCHESTRATION.md](ORCHESTRATION.md)). Craft continues. Agents never flip `PRIVATE_MODE` or invent traction. |
...
> | `MAIL_POSTAL_ADDRESS` | **unset by founder choice (2026-08-16).** No list email, no invite senders, no waitlist HTML. Not a public-alpha blocker. Do not invent a footer. Set only when the founder decides to send commercial mail ([LEGAL_SAFETY.md](docs/LEGAL_SAFETY.md) §3). |
...
- **`.871`:** (`2026.07-unified.871`) **`/harness` pastes harvest** — living `idea:next` → `idea:paste`. Empty + generate legal → generate. Two zero generates → C5. No `PRIVATE_MODE` flip.
- **`.870`:** (`2026.07-unified.870`) **Harness follows Horizon W** — empty queue + empty harvest → path (recipe 15), not stop. No `PRIVATE_MODE` flip.
...
- **Free-first mute-pay:** LLC formed + EIN issued — **no Bundle checkout** + **full depth unlocked**; pricing study later; More/rail **Pillars demoted until first workout** (F-004 / `.695`) — still no Bundle ([docs/FREE_BETA.md](docs/FREE_BETA.md)).
...
- **Founder:** Accept B on Android + keep building the wedge until the public-alpha flip. **No invite cohort.** `PRIVATE_MODE` stays on until the founder flips it. **Postal stays unset** until they choose to send commercial mail. Before that flip: `npm run secrets:scan`; **Lifetime vs Grok** ([LAUNCH_RUNBOOK](docs/LAUNCH_RUNBOOK.md) §5). Pricing study later.
...
1. **Horizon rule** — Horizon W: build wedge excellence (Train/Today/Victory/Coach). No new pillars/locales/America/F5 without explicit founder override. Public alpha when the founder says the product is ready — no ≥10-invite gate.
