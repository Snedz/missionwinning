...
- **`.1067`:** (`2026.07-unified.1067`) **next 16.3.4 AVIF re-enable** — Dependabot `#922` already merged unstamped (`5566967c`). Follow-up to `16.3.3`: re-enables AVIF Image Optimization and requires `sharp` `^0.35.4` (already on master via `#917` / `.1065`). Not a new CVE. Security RCEs were in `16.3.3` (`#911` / `.1060`). Paper stamp only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
