...
- **`.1016`:** (`2026.07-unified.1016`) **This session as a file they own** — History detail saves THAT finished log as a local CSV (JSON is the same rows). Reuses `decideExportDiary` columns. The file re-imports via `.1013`. Empty / missing / tomb invents nothing — Save stays disabled. Honest logged columns only. No public URL, Feed, share, email, or clipboard permalink. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Assisted cite `.1015` / Duration cite `.1014` / Import `.1013` / Titles `.1012` / Export `.1011` / Library skip `.1010` / BW cite `.1009` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1015` (`4046afd9`). Stamp stays `.1016`.
