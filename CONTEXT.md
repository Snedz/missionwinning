...
- **`.1057`:** (`2026.07-unified.1057`) **Left second bar + house sheet** — Home/Library open a left column next to the icon rail (72+264, hides &lt;723), not a far-right More sheet. Desktop wraps second + canvas in one white 12px sheet on grey. Compact unwraps it. On the sheet, Today starts with the session (date stays; no second Today H1); Start hero is flush. Start in that bar is one click → `/active` compose. Train plus is a 40px white circle, 1px line, no shadow. Selected row/rail is `#eee`. Library filter/empty are house objects. This week pane doors `generateWeek` on `/coach` only. History / Weekly plan / Library / Builder drop the old pillar title. House DS is the signed-in token table. Field-manual stays on `/private` / www. Today has one filled action (Start). One Got it. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1056` (`9daa7d18`). Stamp stays `.1057`.
