...
## Now (2026-08-04 · web `2026.07-unified.341` · Android `1.24.1`)
...
- **`.341`:** Active `resolveSwapCandidatesWhenOpen` extract + wiring guard.
- **`.340`:** Seeded axe on Coaching filled interest form.
- **`.339`:** Seeded axe on offline banner on Today; Loop 12 closed.
- **`.338`:** Active `bodyScoreDeltas` extract + wiring guard.
- **`.337`:** Soft chrome residue (toast/BetaAdmin/Benchmarks/Guide/Calc).
- **`.336`:** Seeded axe on Benchmarks anatomy; Select name + Table keyboard scroll.
- **`.335`:** Soft chrome + `shouldOfferVolumeTrim` extract.
- **`.334`:** Compare/Benchmarks/About/nav leftovers i18n; uncovered **44→16** (Bundle refused).
- **`.333`:** Seeded axe on Library detail sheet open.
- **`.332`:** `resolveFormGuideSheet` extract + SignIn/DangerZone/Intent soft chrome.
- **`.331`:** Programs/Learn/Library/Builder/CoachVoice i18n; uncovered **69→44**.
- **`.330`:** Seeded axe on Learn no-match search EmptyState.
- **`.329`:** Active `resolveActiveDockMode` extract + wiring guard.
- **`.328`:** Soft chrome residue (FileUpload/History/WeekStrip/Move/Today/Builder).
- **`.327`:** Learn + analytics consent + Coach demo i18n; uncovered **92→69**.
