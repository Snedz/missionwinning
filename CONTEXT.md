...
- **`.1082`:** (`2026.07-unified.1082`) **Identity CapResult deny consistency** — When a mini lacks identity scope, every identity method on the fake bus (`read`) returns `{ ok: false, code: 'scope_denied' }`. `l1.health` and `utility.clearshot` keep stub success. Test-only `test.noidentity` is the deny fixture (no product mini lacks identity). No auth UI. No Supabase. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
