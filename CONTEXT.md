...
> | Repo visibility | **Working copy `Snedz/missionwinning` is PUBLIC** (verified 2026-09-15 via the GitHub API; this row previously claimed "private", which was wrong). Public means the free GitHub security features all apply here: **secret scanning + push protection**, **code scanning**, **private vulnerability reporting**, and the **Dependency Review API** are available. It also means **every tracked path in this tree is world-readable** — INTERNAL-class text must live in the private `mission-ops` repo and never here ([docs/CLASSIFICATION.md](docs/CLASSIFICATION.md)). The `mission-ops/` typo that leaked an INTERNAL architecture freeze into this tree is the standing proof. |
