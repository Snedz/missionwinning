...
- **`.1019`:** (`2026.07-unified.1019`) **Never-trained is not overdue** — Anatomy painted `daysSince >= 7` overdue, so never-trained (`days = 99`) Chest read as a missed session. Overdue is days ≥ 7 and not the 99 sentinel. Trained-then-left (including outside the 14-day window) still overdue. Score 99 sentinel stays. Grid heatmap idle `>= 99` stays. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Month they own `.1018` / Empty-load `.1017` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1018` (`b376d4ecd`). Stamp stays `.1019`.
