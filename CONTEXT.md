...
- **`.974`:** (`2026.07-unified.974`) **Quiet Move** — optional rest-day walk / easy session on `/move` (distance or minutes, or just the kind). Empty week-strip days stay empty. Not a second Today Start. Not a ring. Not a Health gate before Train. Not a feed of walks. Guest. First set ungated. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.973`. Keeps cues `.973`, honesty `.971`, set tags `.970`, RPE `.967`, Fuel `.965`, resume `.963`. Concern `.969`.
- **`.973`:** (`2026.07-unified.973`) **Cues on the movement** — opening the live exercise shows a short written cue list (setup first, cap 3). Optional still only from media we already have. Hide never blocks Log set. Full guide stays behind Info. Not a clip feed. Not on Today. Guest. First set ungated. Today still one Start. Resume `.963` kept. Honesty `.971` / tags `.970` / RPE `.967` / Fuel `.965` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.971`; skips `.972`.
