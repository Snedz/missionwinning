...
- **`.988`:** (`2026.07-unified.988`) **EMOM/AMRAP timer** (title `.987`) — optional interval (EMOM minute) or countdown (AMRAP window) on the live set row. Not the rest timer. Empty invents nothing. Rest still runs for ordinary rest. Drop-set rest-zero `.986` stays. Guest. First set ungated. Today still one Start. Resume `.963` kept. Warmup `.985` / notes `.983` / 1RM `.981` / supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` / cues `.973` / honesty `.971` / tags `.970` / RPE `.967` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Not Watch-as-pitch. Past master `.986` (`9c3b2ce6`).
