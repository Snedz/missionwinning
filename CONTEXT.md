...
- **`.963`:** (`2026.07-unified.963`) **Resume / finish-partial** — leave Today / week / Wednesday / a receipt, come back, same session. Finish keeps logged sets; leftover empty sets invent no volume. Empty invents nothing. Guest. First set ungated. Today still one Start. Saved routine still owns a cold Start. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Skips `.962` (Fuel list if it lands).
