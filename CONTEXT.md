...
- **`.945`:** (`2026.07-unified.945`) **Missed-day re-entry** — if a planned day was skipped, Today/Coach offer do it now, skip, or slide. Skip is not a fail identity. No streak theater. Train set row untouched. Rebased past `#782` `.944`. No `PRIVATE_MODE` flip.
- **`.944`:** (`2026.07-unified.944`) **E-Victory receipt** — after Finish, vs-last is the same lift shape, not the workout name. Empty history is honest empty — no fake PR. Sheet reused. Today untouched. No feed. No `PRIVATE_MODE` flip.
- **`.943`:** (`2026.07-unified.943`) **Strong CSV export** — Account session CSV is the interchange download. Empty history is header-only. One imported fixture round-trips; skipped rows stay skipped. No account. Rebased past `#778` `.942`. No `PRIVATE_MODE` flip.
- **`.942`:** (`2026.07-unified.942`) **Gated www is the four-scene field** — `/private` mounts SET → ANYWHERE → WEEK → DOOR. Brand pack stays master’s (Log a set. Offline.). `/today` `/train` 308 before the gate. `/notify` is the Super Bundle form. Rebased past `#780` `.941`. Cookie `/` stays `.696`. No `PRIVATE_MODE` flip.
