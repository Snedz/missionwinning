...
- **`.1070`:** (`2026.07-unified.1070`) **Coverage floor 438 → 493 + i18n + bundle + Hero e2e** — Floor 493 after #935. Four catalog keys. Bundle `/` 351 (LandingPage surface; cookies() so no index.html) · `/log` 417 · `/active` 511. A11y unique `/server`. Hero 37-red was house leftover selectors (Start / More / `/notify` / `/bundle` 307), not a PRIVATE_MODE flip. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
