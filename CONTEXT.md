...
- **`.980`:** (`2026.07-unified.980`) **Supersets** (concern `.979`) — two or more exercises group as one superset in the live log. Rest and vs-last treat the group as one round, not a new exercise at A2. Empty invents nothing. Optional. Guest. First set ungated. Today still one Start. Resume `.963` kept. Learn `.978` / week strip `.977` / Track `.976` / Move `.974` / cues `.973` / honesty `.971` / tags `.970` / RPE `.967` / Fuel `.965` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.978` (`1a06fbd7`).
