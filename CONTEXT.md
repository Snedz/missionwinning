...
- **`.1057`:** (`2026.07-unified.1057`) **Left second bar + house design system** — Home/Library open a left column next to the icon rail (72+264, hides &lt;723), not a far-right More sheet. Start in that bar is one click → `/active` compose. This week pane shows the real week and doors `generateWeek` on `/coach` only. History / Weekly plan / Library / Builder sit in the canvas without the old pillar title. House DS (`src/components/house/DESIGN.md`) is the signed-in token table (white / zinc / 16px / black pills). Field-manual stays on `/private` / www. Second-bar rows stagger 40ms. Coach inner costume eyebrow gone. Today has one filled action (Start); week generate is a door to `/coach`. More leftover keeps a quiet house title. One Got it. Hover chips stay. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1056` (`9daa7d18`). Stamp stays `.1057`.
