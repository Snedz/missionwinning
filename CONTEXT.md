...
- **`.1021`:** (`2026.07-unified.1021`) **Coach chat empty load is BW, not 0** — logger cites already print `8 × BW`; chat interpolated `Push-ups 0 × 8`. `formatCoachLogFactCite` via `formatSetLoadLine`. Empty load is `8 × BW`. Loaded stays `80 × 5`. Display only — store still `weight: 0`. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Library spark `.1020` / Never-trained `.1019` / Month `.1018` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1020` (`2e7764963`). Stamp stays `.1021`.
