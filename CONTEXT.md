...
- **`.548` (D14 wave):** PWA update-ready toast (dark until flip) · log-driven week strip where the coach strip can't mount · Today→`/track` quick-add · 48h "How was {{name}}?" journal invite (+ `/history?tab=journal`) · journal counters on /mind · fuel logged-day averages · i18n coverage debt 48→3 repaid (new `rewards` namespace; th/zh/id/ar today bodies now spread `...en`) · coverage floor repaid 412→**406**, ratchet 407→406 (eleven logic modules tested, 32 mutants killed).
- **Two ratchets were found red on `master`, not by this wave (`.548`).** Coverage floor: 415 untested files against a floor of 407 — repaid to 406 here. Bundle budget: `/log` 281.6 KB (budget 280) and `/active` 442.4 KB (budget 435), both measured on a clean `origin/master` worktree; this wave adds +0.5 KB each and **the budget was not loosened**. Clearing `/active` is its own optimisation change. Root cause is the same for both: **`npm run gate` is not being run before ships**, and the PR CI lane that would catch it is not firing.
...
- **`.536`:** ContinuityStrip on full Today Dashboard (web-first UX).
- **`.535`:** Super Bundle depth — premium Move 40 + Mind 48 (D1 targets).
