...
- **`.1037`:** (`2026.07-unified.1037`) **Add a lift to this finished session** — History edit can change sets, reorder, and replace a lift. It cannot add a movement they forgot. Backfill `.1000` is a new row. Add set is on an existing lift. `decideAppendFinishedExercise` appends `{ exerciseId, sets: [{ reps: 0, weight: 0 }] }`. Empty / junk / empty next id invents nothing. Unknown lift is noop. Duplicate ids allowed. Same finished log. Same id. ExercisePicker at the bottom of History edit (same as backfill / replace). Save still confirm-gated `decideEditSave`. Empty 0/0 still needs evidence. Does not write Wednesday / saved / live Start. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Replace `.1036` / Duration `.1035` / Reorder `.1034` / Edit `.997` / Backfill `.1000` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1036` (`3408cbfef`). Stamp stays `.1037`.
- **`.1036`:** (`2026.07-unified.1036`) **Replace this lift on a finished session** — History edit can change sets and reorder lifts. It cannot swap the movement they logged by mistake. `decideReplaceFinishedExercise` replaces that index's `exerciseId`; sets ride unchanged. Empty / junk index / empty next id invents nothing. Same id / unknown lift / out of range is noop. Same finished log. Same id. ExercisePicker on History edit (same as backfill). Save still confirm-gated `decideEditSave`. Does not write Wednesday / saved / live Start. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Duration `.1035` / Reorder `.1034` / Edit `.997` / Copy `.1030` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1035` (`a5236f986`). Stamp stays `.1036`.
- **`.1035`:** (`2026.07-unified.1035`) **Edit this session's logged duration** — History already prints `log.durationSeconds`. Set-hold duration is already editable. The session clock they logged is not. `decideEditSessionDuration` parses seconds or mm:ss via `parseDurationSeconds`. Cap 86400. Empty / junk / negative / over-cap invents nothing. Missing / tomb / live-open / same value is noop. `0` clears the clock. Apply keeps same id, same sets, same startedAt/completedAt. Never invents elapsed from `startedAt`. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Reorder `.1034` / Name `.1007` / Edit `.997` / Move `.1027` / Copy `.1030` stay. Live pause `.1001` stays on Train. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1034` (`a075f994f`). Stamp stays `.1035`.
- **`.1034`:** (`2026.07-unified.1034`) **Reorder lifts on this finished session** — Live Train already drags this session (`.998`). History edit `.997` could change sets, not lift order. `decideReorderFinishedExercises` wraps `reorderSessionExercises`. Empty / junk indexes invent nothing. Same index / one lift / out of range is noop. Apply reorders lifts; sets ride with the lift. Same finished log. Same id. Up / Down on History edit (2+ lifts). Save still confirm-gated `decideEditSave`. Does not write Wednesday / saved / live Start. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Edit `.997` / live `.998` / Repeat `.1026` / Move `.1027` / Copy `.1030` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1033` (`51e4df41c`). Stamp stays `.1034`.
- **`.1033`:** (`2026.07-unified.1033`) **This month shows how many live sessions** — History calendar footer already printed training days. Two logs on Tuesday is still one training day. `decideMonthSessionCount` prints the live session count for the month on screen. Empty / junk / no live rows invents nothing. Never apply 0. Tombs stay out. `startFrom` does not shrink. Training-days summary stays. Empty month stays “Nothing logged this month.” Not a fire. Not a year picker. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Day-cell `.1032` / This month `.1031` / Copy `.1030` / month file `.1029` / empty-day `.1028` / Move `.1027` / Repeat `.1026` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1032` (`56b7e3eab`). Stamp stays `.1033`.
- **`.1032`:** (`2026.07-unified.1032`) **Trained day shows how many live sessions** — History calendar DayCell already had `MonthDay.sessions` from `monthLiveFacts`. It still painted one dumbbell for one session or three. `decideDaySessionCount` prints the live count with the dumbbell when mark is trained and sessions is a finite integer ≥ 1. Empty / junk / blank / logged / future invents nothing. Never a fire. Never missed ✕. Dumbbell stays (WCAG 1.4.1). History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. This month `.1031` / Copy `.1030` / month file `.1029` / empty-day `.1028` / Move `.1027` / Repeat `.1026` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1031` (`12a2bcfbb`). Stamp stays `.1032`.
- **`.1031`:** (`2026.07-unified.1031`) **This month on the History calendar** — after paging prev/next, one **This month** jumps back to the current local month and today. Outline, 44px, not primary-fill. `decideThisMonth` is empty on junk, noop when already this month, apply otherwise. Does not mutate history. Does not invent sessions. Not a year list. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Copy `.1030` / month file `.1029` / empty-day `.1028` / Move `.1027` / Repeat `.1026` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1030` (`674983aa9`). Stamp stays `.1031`.
- **`.1030`:** (`2026.07-unified.1030`) **Copy this session onto another day** — History detail copies the finished live log onto another day as a new row. New id. New clientId. Same sets. Same name/title/notes. Duration copied as logged. Clock stays. Source day still lists the original. Destination day lists the copy. Empty / missing / tomb / live-open / junk / future invents nothing. Same-day is noop. Not Move (same id). Not Repeat (live Start). History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Month file `.1029` / empty-day `.1028` / Move `.1027` / Repeat `.1026` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1029` (`fad56f1f3`). Stamp stays `.1030`.
- **`.1029`:** (`2026.07-unified.1029`) **This month as a file they own** — History calendar Save this month writes THAT viewed month's live rows as a local CSV (JSON is the same rows). Reuses `decideExportDiary` columns. File matches the month on screen, not today's month after they page. Tombs stay out. Start-from fold does not shrink the file. Empty / missing / junk invents nothing — Save stays disabled. No public URL, Feed, share. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Empty-day `.1028` / Move `.1027` / Repeat `.1026` / session file `.1016` / diary `.1011` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1028` (`65f50302e76`). Stamp stays `.1029`.
- **`.1028`:** (`2026.07-unified.1028`) **Log onto this empty day from the month** — History month they own: empty past or today day (`kind: none`) gets one plus / log onto this day. Prefills `.1000` backfill on that dateKey. New row. Save still `decideBackfillSession`. Empty / missing / junk / future invents nothing. Vacated day opens on that date. Tombs stay out unless restored. Overflow `.1000` stays "Log a past session". History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Move `.1027` / Repeat `.1026` / set-table `.1025` / month `.1018` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1027` (`c721fa5b36`). Stamp stays `.1028`.
- **`.1027`:** (`2026.07-unified.1027`) **Move this session to another day** — History detail re-dates the finished live log. Same id. Same sets. Clock stays. Vacated day drops that row. Destination day shows it. Empty / missing / tomb / future invents nothing. Tombs stay out unless restored. Edit `.997` is sets. Backfill `.1000` is a new row. History only. Not Today. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Repeat `.1026` / set-table `.1025` / History volume `.1024` / month `.1018` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1026` (`f3a273ca`). Stamp stays `.1027`.
- **`.1026`:** (`2026.07-unified.1026`) **Repeat this session** — History detail copies the sets they actually logged into the one live Start. Warmup stays. Empty-load 0 stays. Missing / tomb invents nothing. Not save-as-template (`.991` receipt + Save as routine stay). Lands on the existing Start. Off Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Set-table `.1025` / History volume `.1024` / citation `.1023` / heatmap `.1022` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1025` (`24f69641`). Stamp stays `.1026`.
- **`.1025`:** (`2026.07-unified.1025`) **Completed set-table empty load is BW, not 0** — cites already print `8 × BW`; completed kg cell still painted stored `0`. Empty-load cell is `BW` via `formatCompletedWeightCell`. Loaded stays the number. Plus-load extra stays `BW+N`. Assisted 0 mute stays later. Display only — store still `weight: 0`. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. History volume `.1024` / citation `.1023` / heatmap `.1022` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1024` (`72685e9b`). Stamp stays `.1025`.
- **`.1024`:** (`2026.07-unified.1024`) **History empty-load volume is reps, not 0 kg** — Victory already says 8 reps; History list + detail still interpolated `totalVolume` 0 as `0 kg`. Empty-load session prints `8 reps` via `formatLogVolumeDisplay`. Loaded stays kg. Career briefing / avg stay kg-honest. Display only — store still `weight: 0`. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Citation `.1023` / heatmap `.1022` / chat `.1021` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1023` (`825bd9fc`). Stamp stays `.1024`.
