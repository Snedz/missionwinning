...
- **`.1056`:** (`2026.07-unified.1056`) **From-scratch signed-in house** — Icon rail + adjacent second bar (Today rooms / Library objects), hover labels, first-run popup + checklist under the week, live Train compose+sidecar. Start is honest (no dummy; lands `/active`). More is leftover, not the Home column. Reference behavior, not a pasted brand. Not #885. Not HomeTodayLean. Isolation + Start-order stay. `/private` / landing / www untouched. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1055` (`e44351c3`). Stamp stays `.1056`.
