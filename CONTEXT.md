...
- **`.1057`:** (`2026.07-unified.1057`) **Crew board + founder gate** — Signed-in More room at `/crew`. Six seats (Scout · Desk · Logger · Week · Scribe · Chief) with owns / stops / sign. Vote locked until that charter is signed. 0/6 → 6/6 is real state. Gate holds send / delete / publish / promote; a signature has no undo and does not send, delete, publish, or promote. House chrome, not a clinic or a lab. Never a rail icon. Never Today. `/server` stays a drawer. Isolation + `logSet` stay. Zero diffs on `/private`, landing, www. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. From house `.1056` (`2b609256`). Visual only. Do not merge. Stamp stays `.1057`.
- **`.1056`:** (`2026.07-unified.1056`) **From-scratch Patreon-class house** — New signed-in website (icon rail + Account avatar, Today desk, live Train compose+sidecar, Library states, HouseMore). Not #885. Not HomeTodayLean relabel. Not RAIL_GROUPS. Isolation + Start-order stay. `/private` / landing / www untouched. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past master `.1055` (`e44351c3`). Stamp stays `.1056`.
