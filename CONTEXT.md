...
- **`.1077`:** (`2026.07-unified.1077`) **ClearShot mini deeplink** — last-segment `clearshot` at `mission://minis/clearshot` (not a long opaque path) resolves to `utility.clearshot` mount. Mirrors Health `l1.health` at `mission://minis/health`. Stubs stay stubby. No Photos / Billing / Android. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
