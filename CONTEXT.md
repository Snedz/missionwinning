...
- **`.1081`:** (`2026.07-unified.1081`) **MiniHost.listMounted inventory** — CapResult list of mounted mini ids + declared scopes only. Undeclared peek → `scope_denied`. Never-mounted id → `unknown_mini`. In-memory host/fakes. No UI / Stripe / camera. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
