...
- **`.1023`:** (`2026.07-unified.1023`) **Coach citation empty load is BW, not 0kg** — decision already refuses a 0 kg set; `coachCitationFact` still printed `0kg × 8`. Empty-load set fact is `8 × BW` via `formatSetLoadLine`. Loaded stays `80kg × 5`. Display only. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Heatmap `.1022` / chat `.1021` / spark `.1020` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1022` (`934524eca`). Stamp stays `.1023`.
