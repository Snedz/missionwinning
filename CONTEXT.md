...
- **`.1086`:** (`2026.07-unified.1086`) **MiniHost already_mounted** — Second `MiniHost.mount` of the same mini id while still mounted returns `{ ok: false, code: 'already_mounted' }`. Does not throw. Does not replace the live instance. Unmount first (`.1078`); remount after unmount cannot read leftovers. `listMounted` is one entry while mounted, zero after unmount. No UI / Stripe / camera. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
