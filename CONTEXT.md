...
- **`.1072`:** (`2026.07-unified.1072`) **Mission OS capability interfaces** — Named doors: `IdentityCapability`, `BillingCapability` (Stripe HOLD — interface only), `PhotosCapability`, `StorageCapability`, `MiniHost.mount(manifest)`, `CapResult`. Stubs + in-memory doubles in `src/lib/mission-os/`. Continues #935 / `.1068` function bus. No ClearShot UI. No remake of Today. Isolation holds. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1071`:** (`2026.07-unified.1071`) **TodayDesk concept chip** — One `house-state` glance before Start (`today-concept-chip`): `Today: Legs · from last log` (or Push/Pull/Recovery) from `plan.nextConceptId` or the next open PlanSession (CoachConcept `.1067`). Guest + signed-in. Absent when freestyle / no plan / week complete. No Feed. No remake of Today. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1070`:** (`2026.07-unified.1070`) **Coverage floor 438 → 493 + i18n + bundle + Hero e2e** — Floor 493 after #935. Four catalog keys. Bundle `/` 351 (LandingPage surface; cookies() so no index.html) · `/log` 417 · `/active` 511. A11y unique `/server`. Hero 37-red was house leftover selectors (Start / More / `/notify` / `/bundle` 307 / desk History not score-band), not a PRIVATE_MODE flip. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1069`:** (`2026.07-unified.1069`) **Font + i18n CI hygiene** — Library hidden-name row `font-medium` → `font-semibold` (Archivo 400/600/800). `ar`/`id`/`pt`/`zh` `activeLastPerformance` uses `{line}` to match `en`. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1068`:** (`2026.07-unified.1068`) **Mission OS minis** — Manifest + capability-bus stubs (`identity`, `billing`, `photos`, `storage`). `utility.clearshot` reserved at `mission://minis/clearshot`. Deny undeclared scopes. Isolation: coach / store / Today / Train stay blind. No ClearShot UI. Android library deferred. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1067`:** (`2026.07-unified.1067`) **CoachConcept + Victory concept cite** — Discrete `CoachConceptId` on plan/session; Finish adapt stamps `nextConceptId` for the next open day; Victory muted `victory-next-env-cite` prefers concept-first text (“Next: Legs · updated from this workout”). NCP apply (concept before words); no foundation-model weights; no Feed; no invented loads. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1066`:** (`2026.07-unified.1066`) **Victory next-env cite** — After Finish, when Coach `adaptPlan` marks this log done and a next planned session remains, Victory shows one muted line (`data-testid="victory-next-env-cite"`): “Next session updated from this workout”. Finish runs the existing adapt path sync so the cite is honest; no Feed; no invented loads. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1065`:** (`2026.07-unified.1065`) **sharp libheif GHSA** — Dependabot high alert #99 / GHSA-rgj7-g3m4-5g8c. Root `overrides.sharp` → `0.35.4` (was lock `0.35.3` via Next optional). Paper only. `[skip vercel]`. No promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1064`:** (`2026.07-unified.1064`) **Astro AVIF RCE** — Dependabot critical alert #87 / GHSA-26w7-cxv4-gfx2. Bump `sites/www` `astro` `^7.2.0` → `^7.2.8` (lock resolves `7.3.2`, sharp `0.35.4`). Paper only. `[skip vercel]`. No promote. Live www stays `.697`. PRIVATE_MODE stays.
- **`.1062`:** Empty `docs/harness/HOP.md` template + CONTEXT `## Now` back to ≤25 (dropped `.1043`/`.1042`). Paper only — do not promote; live www stays `.697`.
- **`.1061`:** (`2026.07-unified.1061`) **eslint-config-next ^16.3.3 + js-yaml 4.3.2 override** — lock restored. Paper only. `[skip vercel]`. No promote. Live www stays `.697`. Prior paper `.1060` = Next 16.3.3 (#911).
- **`.1059`:** (`2026.07-unified.1059`) **House C2 Instrument 4th skin** — `.mw-house` remaps to ground `#0a0c0f` + signal `#ffb000`. HOLD + nits folded. Leftover hop (empty): Coach / Builder house-empty is clinical short copy. `/private` / landing / www untouched. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.697`. Guest. First set ungated. Today still one Start. Past master `.1057` (`f302f40f`). Stamp stays `.1059`. Visual only.
- **`.1058`:** (`2026.07-unified.1058`) **`/active` first paint is a set table** — You private note summary cite is house leftover. You table Edit summary cite is house leftover. You athlete card Edit summary cite is house leftover. You identity Edit summary cite is house leftover. You identity saved cite is house leftover. You identity name label cite is house leftover. You identity number label cite is house leftover. You athlete card badges preview cite is house leftover. You athlete card badges legend cite is house leftover. You kit summary cite is house leftover. You athlete card backdrop legend cite is house leftover. You athlete card frame legend cite is house leftover. You athlete card privacy cite is house leftover. You kit saved cite is house leftover. You kit locked cite is house leftover. Account more-settings calculators body cite is house leftover. Account first-paint free-beta foot cite is house leftover. Account first-paint sign-in hint cite is house leftover. Account first-paint Explore places body cite is house leftover. Account first-paint language switcher cite is house leftover. Account first-paint visibility body cite is house leftover. Account first-paint reminders kinds list is house leftover. Account first-paint feedback body cite is house leftover. Account first-paint referral sign-in hint cite is house leftover. Account first-paint referral body cite is house leftover. Account first-paint reminders hint cite is house leftover. Account first-paint reminders unavailable cite is house leftover. Account first-paint home gym kit body cite is house leftover. You kit body cite is house leftover. You table row label cite is house leftover. You table body cite is house leftover. You private note saved cite is house leftover. You private note counter cite is house leftover. Account first-paint goals hint cite is house leftover. Account first-paint language hint cite is house leftover. Account first-paint units hint cite is house leftover. You private note body cite is house leftover. You share body cite is house leftover. You private note textarea is house leftover. You table Edit / row hairlines is house leftover. You athlete card Edit / signature cites is house leftover. Account first-paint goals textarea is house leftover. You identity Edit / signature cites is house leftover. Under the Hood week-4 diagnostic card is house leftover. Account first-paint reminders day-review row is house leftover. Account owner-tools beta admin cards is house leftover. Account owner-tools cards is house leftover. Account owner-tools founder status board is house leftover. Account more-settings sync status row is house leftover. Account more-settings import card is house leftover. Account more-settings backup card is house leftover. Account more-settings privacy card is house leftover. Account more-settings What’s New card is house leftover. Account more-settings wearables card is house leftover. Account more-settings journey card is house leftover. Account more-settings beta journey card is house leftover. Account more-settings assessment card is house leftover. Account more-settings pregnancy card is house leftover. Visibility first-paint report row is house leftover. You first-paint career card is house leftover. You first-paint share card is house leftover. You first-paint private note card is house leftover. You first-paint kit card is house leftover. Account first-paint premium card is house leftover. Account first-paint feedback card is house leftover. Account first-paint referral card is house leftover. You first-paint rewards card is house leftover. Account first-paint goals card is house leftover. Account first-paint language card is house leftover. Account first-paint units card is house leftover. Account first-paint home gym kit is house leftover. Account first-paint reminders card is house leftover. Account first-paint account card is house leftover. You first-paint table card is house leftover. You first-paint athlete card is house leftover. Account first-paint visibility card is house leftover. Account first-paint sign-in block is house leftover. You first-paint identity card is house leftover. Garage first-paint board is house leftover. Log set leading is house leftover. Log set weight is house leftover. Log set size is house leftover. Set table size is house leftover. First-rooms Week navigates from first paint (`href="/log#today-week"`). Builder Show-all door is house leftover. Coach Show-all door is house leftover. Library Show-all door is house leftover. History Show-all door is house leftover. Show-all door is house leftover. In-set cue line size is house leftover. Session clock size is house leftover. In-set cue mark is house leftover. In-set cue still is house leftover. In-set cue line is house leftover. Session title is a house title. Session clock cite is house leftover. Kind chip row is house leftover. Upcoming set row is house leftover. Set row hairline is house leftover. Prev cite is house leftover. First-rooms History navigates from first paint (`href="/history"`). `/history` day client nav is not group Loading. Coach first paint is not a plan skeleton. History first paint is not Loading sessions. `/builder` client nav is not group Loading. `/coach` client nav is not group Loading. `/history` client nav is not group Loading. `/library` client nav is not group Loading. `/log` client nav is not group Loading. `/active` client nav is not group Loading. First-rooms Log a set navigates from first paint (`href="/active"`). Compact hero Start navigates from first paint (`href="/active"`). Form / Swap sheets open on click from first paint. Sidecar first paint does not wait on persist. `/active` first paint Log set does not wait on persist. Cold Train is a static compose page, not `RouteLoading` / Restoring session. Layout effect writes today's session before paint. Persist merge keeps an in-memory Just Go over a persisted null; logged work on disk still wins. Reconcile after hydrate re-seeds an empty canvas. Start on `/log` writes then navigates — no second start after `loadCoachTodayOptional`. Course first paint is house leftover (title + locked / empty — not RouteLoading). Guide chapter first paint is house leftover (title + body — not RouteLoading). Guidebook first paint is house leftover (title + chapter list — not RouteLoading). Learn first paint is house leftover (title + quiet intro — not RouteLoading). Mind first paint is house leftover (title + check-in / breathe — not useSearchParams skeleton). Move first paint is house leftover (title + flow list / quiet log — not useSearchParams skeleton). Super Bundle first paint is house leftover (title + offer — not RouteLoading; free-beta still 307s to `/notify`). Benchmarks first paint is house leftover (title + stats / empty — not RouteLoading). Leaderboard first paint is house leftover (title + board — not RouteLoading). Visibility first paint is house leftover (title + report — not RouteLoading). Under the Hood first paint is house leftover (title + weights — not RouteLoading). Track first paint is house leftover (title + weight / tape — not RouteLoading). Fuel first paint is house leftover (title + notepad / today log / remaining — not RouteLoading). Builder first paint is house leftover (title + Blank workout / saved — not RouteLoading). Coach first paint is house leftover (title + empty/generate or week — not RouteLoading). Library first paint is house leftover (title + catalog list — not RouteLoading). History list first paint is house leftover (title + list/empty — not RouteLoading). History day first paint is house leftover (date + that day's rows). Calendar / charts stay parked. Log set is house leftover press (`--house-press` / #18181b), filled `primary-action`, ungated. LogConsole Log set is house leftover press (`--house-press` / #18181b), not poster-red. Form guide + Swap portal is house leftover (`mw-house` on the overlay root). Merge-exercises dialog is house leftover (`mw-house` on Radix Content). Account first paint is house leftover (sign-in / return / prefs) — not `RouteLoading` / `useSearchParams` skeleton. You first paint is house leftover (identity / kit) — not `RouteLoading`. Guest. First set ungated. Today still one Start. Resume `.963` kept. `/private` stays the tight lock. No `PRIVATE_MODE` flip. No promote. Live www stays `.697`. `[skip vercel]`. Past master `.1057` (`f302f40f`). Stamp stays `.1058`.
- **`.1057`:** (`2026.07-unified.1057`) **Left second bar + house sheet** — Home/Library open a left column next to the icon rail (72+264, hides &lt;723), not a far-right More sheet. Desktop wraps second + canvas in one white 12px sheet on grey. Compact unwraps it. Today starts with the session; Start hero is flush. Today first paint stays the Thursday desk (Just Go, first rooms, week strip) — never hide them until snap. Start on `/log` writes today's session (Just Go with last loads) before Train opens. `/active` first paint is add-exercise + a set row + Log set — hydrate does not own the canvas. Never Restoring-session as the product. Library, History, and Builder first-paint rows are house items. Weekly plan empty is house-empty; Generate stays the one filled dock on `/coach`. You leftover is `house-profile`; Fuel leftover is `house-fuel`; Fuel first-paint notepad is house leftover (recents are house-state; type is house-field; water is house-btn, not filled); Fuel first-paint today log is house leftover (Load from Cloud is house-btn, not filled); Fuel first-paint remaining is house leftover (kicker not uppercase tracking; MeterBar stays). Account leftover is `house-account` (sign-in / return / prefs first; Explore / more / help are house-card; sidecar is Account / You only — never History or Coach on the right). Explore leftover is `house-explore` (board + pin list first; Add a place is house-card; never a rail). Assessment leftover is `house-assess` (form is the object; one filled submit; EN floors on first paint; stage prompts in Show all). Calculator leftover is `house-calc` (1RM / macros / plates first; chips are house-state; tools in house-card; premium in Show all; never a rail; Account More settings doors it). Human coaching leftover is `house-coaching` (form is the object; one filled submit; never a rail; not Mission Coach). Programs leftover is `house-programs` (outlines first; chips are house-state; Unlock / price in Show all; never a rail; not a shop). Help leftover is `house-help` (FAQ is the object; hairline items; never a rail). Cookies leftover is `house-cookies` (overview + inventory first; table is a house object; never a rail). Privacy leftover is `house-privacy` (jump chips are house-state; sections are house-card; never a rail; legal copy unchanged). Terms leftover is `house-terms` (jump chips are house-state; sections are house-card; never a rail; legal copy unchanged). Refunds leftover is `house-refunds` (jump chips are house-state; sections are house-card; never a rail; legal copy unchanged). DMCA leftover is `house-dmca` (jump chips are house-state; sections are house-card; never a rail; legal copy unchanged). Usage leftover is `house-usage` (jump chips are house-state; sections are house-card; never a rail; legal copy unchanged). Regions leftover is `house-regions` (jump chips are house-state; sections are house-card; never a rail; legal copy unchanged). Service-terms leftover is `house-service-terms` (jump chips are house-state; sections are house-card; never a rail; legal copy unchanged). Accessibility leftover is `house-a11y` (jump chips are house-state; sections are house-card; never a rail; legal copy unchanged). More leftover Fuel / You / Account are 12px rows, selected `#eee`. Quiet More foot is stacked 13px muted rows; current leftover is ink, not `#eee`. Train compose leftover: empty is house-lede; Add exercise is house-btn; extras in house-card Show all; in-set cues are a kicker + ghost hide / Learn door; kind chips are house-state selected `#eee`; Add Set is house-btn; rest lanes are house-state selected `#eee`; number cells are house-num; Finish is house-btn, not filled; session more is house leftover (ghost more, house-card overflow; plates / tip stay there); Skip this exercise is house-btn hold, not filled; Swap is house-btn ghost, not filled; Swap confirm is house-btn, not filled; Form guide is house-btn ghost; Form guide confirm is house-btn, not filled; Repeat last set is house-btn, not filled; Pin and Note are house-field; Exercise more is house leftover (ghost more, house-card overflow); Add-exercise search is house leftover; This-movement history is house leftover (Close is house-btn; rows are house-movement-row); extra set cells are house-num; Next-cite Skip is house-btn; Set options is house leftover (ghost more, house-card overflow); Reorder handle is house leftover (ghost house-btn grip and arrows); Show-all extras are house leftover; Rest dock is house leftover; Last-set ghost is house leftover (house-btn ghost, not filled); Load-% cell is house-num; Readiness extra is house leftover; Warmup toggle is house leftover; Set side is house-num; Form guide body is house leftover; Garage swap is house leftover; Exercise card is house leftover; Exercise head is house leftover; Add-exercise sheet is house leftover; Check-in confirm is house leftover; Check-in scale is house leftover; Hard-session confirm is house leftover; Overlay header is house leftover; Swap sheet is house leftover; Movement history sheet is house leftover; Overlay footer is house leftover; Overlay panel is house leftover; Form guide sections is house leftover; Set table head is house leftover; Set row kicker is house leftover; Logged check is house leftover; Completed row is house leftover; Plus-load prefix is house leftover; Kind badge is house leftover; Movement history date is house leftover; Session more hold is house leftover; Cue me is house leftover; Plate loader is house leftover; History tools are house leftover; Library Filters is house leftover; Library Filters sheet is house leftover; Library detail is house leftover; Library hidden is house leftover; Library search is house leftover; History search is house leftover; Builder Show all extras is house leftover; Coach generate dock is house leftover; Coach next-day cite is house leftover; Coach week dose is house leftover; Library showing count is house leftover; Coach session card is house leftover; Coach session lift is house leftover; Coach live voice is house leftover; Coach adapt banner is house leftover; Coach Show all extras is house leftover; Coach week strip is house leftover; Library pick bar is house leftover; Library Show all extras is house leftover; History Show all extras is house leftover; Today Start quiet offers are house leftover; Log set stays filled and ungated; sidecar is rest / skip / notes (History stays on Home). Quiet More leftovers Move / Mind / Track / Learn carry house marks; Move first-paint flow list is house leftover; Move first-paint quiet log is house leftover; Track first-paint metrics is house leftover; Mind first-paint check-in is house leftover; Mind first-paint breathe is house leftover; Learn first-paint intro is house leftover. Feedback leftover is `house-feedback` (form is the object; one filled submit; Sign-in stays extra). Garage leftover is `house-garage` (quiet foot only; never a rail). Train plus is a 40px white circle. Start → `/active`. Selected `#eee`. House DS is the signed-in token table. Field-manual stays on `/private` / www. Today has one filled action (Start). One Got it. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.697`. Guest. First set ungated. Today still one Start. Past master `.1056` (`9daa7d18`). Stamp stays `.1057`.

- **`.1056`:** (`2026.07-unified.1056`) **From-scratch signed-in house** — Icon rail 72 + second bar 264 (hides &lt;723), hover chips (Today/Train/Library/You/More). One Got it mark (dismiss only). Persistent checklist under Start (Log a set / Open this week / History after a finish; inferred ticks; locked History why-tooltip). Week row slides a back-chevron pane. Short eased motion on chips / pane / fold / Got it (reduced-motion off). Start is honest (no dummy; lands `/active`). MW rooms only. Isolation + Start-order stay. `/private` / landing / www untouched. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.697`. Guest. First set ungated. Today still one Start. Past master `.1055` (`e44351c3`). Stamp stays `.1056`.
- **`.1055`:** (`2026.07-unified.1055`) **Later-door object model + isolation holds** — Fold structure-only object model into `docs/IA_SKELETON.md`. Isolation already enforced: Today/Train do not import social; dock = `/log`+`/active`; `generateWeek` never reads `/server`; chat never withholds a set (`isolation.test.ts` + `domainBoundary` C1–C3 + `weekWriter.test.ts`). No new tests. No costume. No `/private` restyle. `[skip vercel]`. No `PRIVATE_MODE` flip. No production promote. Live www stays `.697`. Guest. First set ungated. Today still one Start. Past master `.1054` (`20b99670`). Stamp stays `.1055`.
