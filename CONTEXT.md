...
## Now (2026-08-13 · web `2026.07-unified.728` · Android `1.24.1`)
...
> and `.728` dropped `.253` detail (full text remains in LOG.md / archive)
...
> | `PRIVATE_MODE` | **on** in production. The gate is up on www; `/` serves the `/private` teaser. Vercel Preview is ungated in code when `VERCEL_ENV=preview` (`.728`) — agents still never flip the Production env. Production also disables the service worker, so **no www tester can install the PWA or log offline** until the flip. Post-flip check is in [LAUNCH_RUNBOOK](docs/LAUNCH_RUNBOOK.md) §5. |
...
- **`.728`:** (`2026.07-unified.728`) **E-Day ungated Preview** — `VERCEL_ENV=preview` turns PRIVATE_MODE off; production stays gated. Open-beta copy (not invite-only). Draft PR; do not merge / do not flip Production.
