...
- **`.1075`:** (`2026.07-unified.1075`) **ClearShot utility mini stub** — `utility.clearshot` at `mission://minis/clearshot` (last-segment `clearshot`). `mountClearShotMini` on existing MiniHost + fakes. Photos + `storage.write`. Identity optional. Billing / `storage.read` `scope_denied`. Photos stay `photos_stub`. Not `l1.health`. No ClearShot UI. No Stripe. No ClearShot in MW APK. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
