...
- **`.1015`:** (`2026.07-unified.1015`) **Next cite is BW, not 0 kg, on assisted 0** — BW cite `.1009` closed vest-0; assisted 0 still printed `8 × 0 kg`. Logged unassisted help prints `8 × BW`. Help still `8 × −20 kg`. Missing invents nothing. Display only — store still `weight: 0`. Reuses `formatSetRowLine` / `formatSetRowPrev`. Custom prefix stays weight. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Duration cite `.1014` / Import `.1013` / Titles `.1012` / Export `.1011` / Library skip `.1010` / BW cite `.1009` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1014` (`f4b852279`). Stamp stays `.1015`.
- **`.1014`:** (`2026.07-unified.1014`) **Next cite is 0:45, not mute** — Prev already says 0:45 for a plank; live Next / Last / after-complete still dropped `durationSeconds` so the cite stayed mute. Logged hold prints `0:45`. Missing invents nothing — not `45 × 0 kg`, not a second. Copy last hold. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Import `.1013` / Titles `.1012` / Export `.1011` / Library skip `.1010` / BW cite `.1009` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1013` (`3a28b75fa`). Stamp stays `.1014`.
- **`.1013`:** (`2026.07-unified.1013`) **Our export comes back** — confirm-gated re-import of the Mission Winning diary file `.1011` saved. Merge / upsert is the default. Replace is a second named confirm, never the default. Cancel leaves the diary unchanged. Empty / garbage / wrong-header invents nothing. Honest logged fields only — no invented 1RM, duration, PRs, or sessions. Tombs stay tombs unless the file has a restored live row. History only. Not Hevy import. Not a Feed. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Title-fix `.1012` / Export `.1011` / Library skip `.1010` / BW cite `.1009` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1012` (`aa23a539`). Stamp stays `.1013`.
- **`.1012`:** (`2026.07-unified.1012`) **This-movement title is the date or the name** — History list already uses `historySessionLabel`; tap the open lift and the sheet still bolded the template. Untitled is the date. Named is the private title. Template is subtitle only when it differs. Empty / missing / tomb invents nothing. Not a Feed. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Export `.1011` / Library skip `.1010` / BW cite `.1009` / Search `.1008` / Name `.1007` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1011` (`8df323949`). Stamp stays `.1012`.
- **`.1011`:** (`2026.07-unified.1011`) **Export this diary** — save the live History diary as a CSV (JSON is the same rows). Honest logged fields only. Empty invents nothing — Save stays disabled. Tombs stay out unless restored. Start-from does not shrink the file. Not a Feed. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Library skip tombs `.1010` / BW cite `.1009` / Search `.1008` / Name `.1007` / Restore `.1006` / Start-from `.1005` / Hide `.1004` / Delete `.1003` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1010` (`4d128292`). Stamp stays `.1011`.
- **`.1010`:** (`2026.07-unified.1010`) **Library skips deleted sessions** — History already skips tombs; Library “Logged in N sessions” and the volume spark still counted a deleted Monday. Count + spark skip `deletedAt`. Tomb-only is empty. Empty invents nothing. Not a Feed. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. BW cite `.1009` / Search `.1008` / Name `.1007` / Restore `.1006` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1009` (`20faec3ab`). Stamp stays `.1010`.
- **`.1009`:** (`2026.07-unified.1009`) **Next cite is BW, not 0 kg** — Victory already says 8 reps; Prev already says BW; the open lift Next / Last / after-complete cite no longer print `8 × 0 kg`. Vest is `BW + 20`. Loaded stays kg. Empty next / no ghost invents nothing. Display only — store still `weight: 0`. Reuses `formatSetRowLine`. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Search `.1008` / Name `.1007` / Restore `.1006` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1008` (`727c6f484`). Stamp stays `.1009`.
- **`.1008`:** (`2026.07-unified.1008`) **Find a past session** — search the History list by the name they gave that session, the template, the date, a lift, or a private note. Empty query invents nothing — same live rows. No matches stays empty. Tombs stay under Deleted sessions. Not a Feed. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Name `.1007` / Restore `.1006` / Start-from `.1005` / Hide `.1004` / Delete `.1003` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1007` (`cd066471`). Stamp stays `.1008`.
- **`.1007`:** (`2026.07-unified.1007`) **Name this finished session** — private title on the History row / receipt. Empty title is the date. Does not rename the template (Start this again / Save as routine still use `workoutName`). Empty / missing / tombstone / live invents nothing. Not a Feed. No Today chrome. Guest. First set ungated. Honesty `.971` still scores Train. Today still one Start. Resume `.963` kept. Restore `.1006` / Start-from `.1005` / Hide `.1004` / Delete `.1003` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.1006` (`a913ffae`). Stamp stays `.1007`.
