...
- **`.981`:** (`2026.07-unified.981`) **1RM percent** — optional percent on the live set row / next-set cite. Type 80% and get a load when a known 1-rep max exists, or show percent of that max next to the target. Empty invents nothing. No invented max from one set. No autoreg-as-identity. Guest. First set ungated. Today still one Start. Resume `.963` kept. Supersets `.980` / Learn `.978` / week strip `.977` / Track `.976` / Move `.974` / cues `.973` / honesty `.971` / tags `.970` / RPE `.967` stay. `/private` stays the tight lock. No `PRIVATE_MODE` flip. Past master `.980` (`17039cb3`).
