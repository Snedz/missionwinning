...
- **`.1074`:** (`2026.07-unified.1074`) **Legal is the policy, not a legal-index tour** — Privacy / cookies / terms / refunds / DMCA / usage / regions / service-terms / accessibility first paint is jump chips + sections. Legal footer is off those pages. Legal copy unchanged. Log set stays filled. `generateWeek` stays on `/coach`. Help stays the FAQ. Park leftover PR #889. Continue #890. `[skip vercel]`. No `PRIVATE_MODE` flip. No promote. Live www stays `.696`. Guest. First set ungated. Today still one Start. Past this branch `.1073` (`81726238`). Stamp `.1074`.
