...
- **`.887`:** (`2026.07-unified.887`) **Week One is three bars, not a triangle** — `week_one.svg` was a closed triangle and read as a broken-image warning on Victory. Three session bars (last red). Guard fails if the old path returns. Vision U2 R1. No `PRIVATE_MODE` flip.
