...
- **`.1070`:** (`2026.07-unified.1070`) **Coverage floor 438 → 493 + i18n + bundle HOLD** — Floor 493 after #935. Four catalog keys. Bundle `/` 351 (LandingPage surface; cookies() so no index.html) · `/log` 417 · `/active` 511. Not the whole-manifest 459. `Alpha` allowlisted. A11y `GATED_ROUTES` dropped duplicate `/server` (Playwright title collision). No invented tests. No TodayDesk craft. Paper only. `[skip vercel]`. No tip-promote. Live www stays `.697`. PRIVATE_MODE stays.
