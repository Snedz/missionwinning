...
| `@missionwinning/mw-core/module` | Mini-app manifest types + `health.train` seed + `social.server` messenger + capability bus (`assertCapability`, `readBilling` / `checkoutBilling` / `portalBilling`) + reserved `UTILITY_CLEARSHOT_MANIFEST` (`mission://minis/clearshot`) + `HOST_SHELL` |
