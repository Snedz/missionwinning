...
/**
 * The same nullification, on the weight axis. The display classes set Archivo
 * 800; a `font-semibold` beside one wins in the utilities layer and quietly
 * renders 600 — exactly how `PillarPageHeader` would have re-inked every pillar
 * screen at the wrong weight the day it adopted `.display-section`. The rule
 * caught `text-*` and was blind to this, which is `.212`'s lesson again: a
 * guard keyed to one spelling of a defect has only ever tested that spelling.
 * The list is closed because these are Tailwind's named weights;
 * `font-extrabold` is excluded — it restates the value the class already sets.
 */
const WEIGHT =
...
console.error(
  'A `text-*` or `font-*` weight utility beats the display class and discards its\n' +
    'clamp()/800 weight. Drop the utility,\n' +
