...
| `MAIL_POSTAL_ADDRESS` | **Yes before any list mail** | Physical address printed in every email footer. CAN-SPAM §7704(a)(5) requires it; the HTML emails refuse to render without it (waitlist confirm degrades to text-only, launch broadcast and beta invite refuse to send). **Unset by founder choice (2026-08-16)** — not a public-alpha flip blocker. Founder-owned — never a placeholder; do not invent or commit a street. |
...
| `NEXT_PUBLIC_PWA_ENABLED` | **Derived — never set by hand** | Computed in `next.config.js` from the same `pwaDisabled` expression that decides whether Serwist builds, so "the UI may claim offline" and "a service worker exists" cannot disagree. Read by the SW registration and by `isOfflineInstallable()` (`src/lib/offlineCapability.ts`), which gates every install/offline **capability claim** in the UI. Setting it manually re-opens `.600`: the app advertises installable offline logging in a build that ships no worker. Pinned by `src/lib/offlineHonesty.test.ts` |
