...
**79** `app/api/**/route.ts` (discovered). Perimeter: surface parking (`isPathEnabled`) then `PRIVATE_MODE` then handler auth. Count is enforced by `src/lib/apiInventory.test.ts`.

| Bucket | Count | Notes |
|--------|------:|-------|
| Public / no login in handler | 21 | Includes password gate, HMAC unsubscribe, optional-session push, parked youth, optional-session flags evaluate |
...
| Admin / cron / privileged | 9 + `health?deep=1` | `CRON_SECRET` or beta admin |
...
| **P1-5** shared-device `mw_*` not account-scoped | **CLOSED (`.774` · `.941`)** | Explicit sign-out wipes athlete keys. Listener `SIGNED_OUT` (boot / expiry) keeps the guest log. `mw_storage_owner` binds the device. Foreign leftover does not OR-merge PAR-Q onto the next account. |
