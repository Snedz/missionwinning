...
| Pregnancy / miscarriage / postpartum | Optional athlete-owned flag; v1 changes only the hard-session stop line; not prenatal care; counsel-hold — [PREGNANCY_SAFETY.md](PREGNANCY_SAFETY.md) |
...
- [ ] **Business postal address — deferred.** Founder (2026-08-16): **leave `MAIL_POSTAL_ADDRESS` unset.** Do not put the formation / registered-agent street in git, the site, or email. CAN-SPAM §7704(a)(5) still requires a valid physical postal address in every commercial email footer — so **send no list mail** (waitlist stays text-only; launch broadcast + invite senders refuse) until the founder sets a public mailing line they chose for that purpose (PO box or CMRA, not an RA street unless counsel says it may be advertised). Not a public-alpha flip blocker. **Same address will serve the DMCA row below** when they decide.
...
| Pregnancy / miscarriage / postpartum | `pregnancySafety.ts`, Account More settings, hard-session stop line — [PREGNANCY_SAFETY.md](PREGNANCY_SAFETY.md) |
