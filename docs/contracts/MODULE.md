...
| `storage.read` | Read mini-scoped key-value (capped). Named fake methods `get` / `set` all return `CapResult` — unscoped is `scope_denied` and does not write. Health (declares read + write) is stub success. ClearShot does not declare this scope, so `get` stays `scope_denied`. |
| `storage.write` | Write mini-scoped key-value (capped). Same CapResult shape as `storage.read`. Health and ClearShot stub-succeed. Unscoped deny does not write the map. |
