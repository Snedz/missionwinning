...
| **C5** | **Free text never reaches another screen.** Local-only is permitted; the public projection and share card carry only picks-from-sets and derived numbers | [`publicProjection.ts`](../packages/mw-core/src/identity/publicProjection.ts) closed key set + [`publicProjection.test.ts`](../src/lib/identity/publicProjection.test.ts). Cloud URL still S4 | **DTO enforced**; surface On S4 |
| **C6** | **Page kits are manifest entries, not stylesheets.** No user-authored CSS, HTML, colour or font, ever | [`pageKitsContract.test.ts`](../src/lib/identity/pageKitsContract.test.ts) discovers `PAGE_KITS` and runs `pageKitProblems` (token ids only; hex/CSS/unknown fields fail). Manifest: `packages/mw-core/src/identity/pageKits.ts` | **Enforced** (default kit only until design kits land) |
...
| ~~**S2**~~ | **Shipped `.606` / `.614`.** Settings moved to `/account`; `/profile` is the Athlete Page — Identity (call sign, first session) + The line (`careerLine.ts`) + the shipped badge shelf. Local only, no server, no projection | W craft window | The D8 void rule is met by The line, which is never empty after one session and states what it will hold before that. `/profile` pinned at **0** red actions — a record is not a task |
| ~~**S2.5**~~ | **Shipped `.620`.** Composition polish: first-viewport hero (name · 00–99 number · signature line) · live card preview · number on share title. Still no kits/table/public projection | W craft window | Page still reads as a record (0 red); number clamp rejects out-of-range; signature is career counts only (never rank/XP) |
| ~~**S3a**~~ | **Shipped `.621`.** The table (4 rows, picks-from-sets, local) + C6 kit manifest guard + `default` kit only | W craft window | Table is optional authorship; 0 red actions; clamp drops free-text / unknown picks |
| ~~**S3b**~~ | **Shipped `.622`.** Structural kits Stack · Field · Ledger · Poster (token composition only, tier-gated) · kit picker · layout classes · C5 public DTO pure | W craft window | Kits visibly differ; locked kits listed not selectable; projection rejects `bio` |
| ~~**S4a**~~ | **Shipped `.623`.** Athlete Page **share-out PNG** from C5 DTO + private local note (never on share). Still no public URL | W craft window | Share path grepped free of private-note key; 0 red actions |
| **S4b** | **Public URL / projection host** — still no boards, no graph | [CLUB_PLAN.md](CLUB_PLAN.md) C2 shipped (ledger + card) | Kill on any moderation incident the projection was supposed to make impossible |
