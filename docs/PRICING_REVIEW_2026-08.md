...
| Issue | Where | Fix |
|-------|--------|-----|
| ~~STRATEGY still says bundleConfig has **3mo/$33, 12mo/$96**~~ — **fixed; this row was itself the stale one.** `.606` checked: STRATEGY.md L52 already reads monthly/$11.99, 12mo/$59, lifetime/$149 and points here. A drift table that is not re-checked becomes a second source of drift. | STRATEGY.md L52 | Done — no action |
