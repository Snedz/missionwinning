...
|----|-----|---------|----------|-------------------|----------------|
| **F1** | **High (config / GA)** | Private gate ignores mobile **Bearer** JWT | `proxy.ts` API branch: cookie \|\| `hasValidSupabaseSession`. Session helper reads **cookies only** and requires `PRIVATE_ALLOW_AUTH_BYPASS=true` (`privateGate.ts`). `mobileAccess.ts` understands Bearer **inside** routes, but requests die at 403 before handlers when gated. | Attackers read exact gate; operators/forks copy “bake cookie” anti-pattern into release APKs | Before public product + Android GA: allowlist `/api/mobile` **or** teach proxy to verify Bearer via `getUser()`, **or** `PRIVATE_MODE=false` at launch. Never ship prod gate cookies in Play builds. |
