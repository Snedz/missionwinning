...
Opened on its written gate — GNT-1's report is done, so the campaign is eligible. **U1 R3 + U2 R1 critic PASS (engine).** U3 LEAD: the original "deload / adapt on missed or high-strain" brief mashed two axes — high-strain shape is U1; `adaptPlan` already records misses and does not cut remaining sets. Next spawn: BUILDER U3 R1 lands `coachEvalAdapt.test.ts` (instrument only; do not deload a miss). Details in [GNT-2-coach-plan-quality.md](GNT-2-coach-plan-quality.md).
