...
Opened on its written gate — GNT-1's report is done, so the campaign is eligible. U1's instrument landed first (`src/lib/coach/coachEval.test.ts`). R1 found the week magnitude-blind (every non-empty history the same plan). R2 corrected: strain already graded; `loadZone` was discarded. `.845` wired the band. **U1 R3 critic PASS (engine)** — TAP `# tests 3 # pass 3 # fail 0`; floor 3; `steady` ≠ `high` at the same strain. Next spawn: LEAD commissions U2. Details in [GNT-2-coach-plan-quality.md](GNT-2-coach-plan-quality.md).
