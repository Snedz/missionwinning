...
Opened on its written gate — GNT-1's report is done, so the campaign is eligible. **U1 R3 critic PASS (engine).** U2 LEAD: the original "clean block raises prescribed work" brief was imprecise — `nextTargets` already load-ups on all-easy and `generateWeek` already programs it (bench 100 → 102.5 vs hard 90; week stays `46:0`). Next spawn: BUILDER U2 R1 lands `coachEvalProgression.test.ts` (instrument only; do not add a set). Details in [GNT-2-coach-plan-quality.md](GNT-2-coach-plan-quality.md).
