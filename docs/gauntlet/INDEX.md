...
Opened on its written gate — GNT-1's report is done, so the campaign is eligible. **U1–U4 critic PASS (engine).** U4 TAP `# tests 2 # pass 2 # fail 0` — catalog discover, every programmed id legal for declared gear. Next spawn: SMOOTHER, then the campaign report (`ready-for-founder`). Details in [GNT-2-coach-plan-quality.md](GNT-2-coach-plan-quality.md).
