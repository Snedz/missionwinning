...
Opened on its written gate — GNT-1's report is done, so the campaign is eligible. **U1–U3 critic PASS (engine).** U4 LEAD: the original "sane for declared gear" brief must not become `!id.includes('bench')` — `equipmentMatches` already filters the catalog and `generateWeek` already programs 0 illegal moves (130-exercise catalog). Next spawn: BUILDER U4 R1 lands `coachEvalGear.test.ts` (instrument only; discover the catalog). Details in [GNT-2-coach-plan-quality.md](GNT-2-coach-plan-quality.md).
