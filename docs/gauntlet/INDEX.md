...
Opened on its written gate — GNT-1's report is done, so the campaign is eligible. **U1 R3 + U2 R1 critic PASS (engine).** U3 LEAD corrected the brief; **U3 R1** landed `coachEvalAdapt.test.ts` (`.848`, instrument only). Next spawn: CRITIC U3 R1. Details in [GNT-2-coach-plan-quality.md](GNT-2-coach-plan-quality.md).
