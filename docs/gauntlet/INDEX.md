...
Opened on its written gate — GNT-1's report is done, so the campaign is eligible. **U1 R3 + U2 R1 + U3 R1 critic PASS (engine).** U3 TAP `# tests 1 # pass 1 # fail 0` — miss recorded, remaining fits, remaining sets do not rise. Next spawn: LEAD commissions U4 (discover the catalog, do not enumerate ids). Details in [GNT-2-coach-plan-quality.md](GNT-2-coach-plan-quality.md).
