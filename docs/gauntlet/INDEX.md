...
Opened on its written gate — GNT-1's report is done, so the campaign is eligible. **U1–U3 critic PASS (engine).** U4 LEAD corrected the brief; **U4 R1** landed `coachEvalGear.test.ts` (`.849`, instrument only). Next spawn: CRITIC U4 R1. Details in [GNT-2-coach-plan-quality.md](GNT-2-coach-plan-quality.md).
