...
**Next spawn:** LEAD on **U2 R1** — U1 R3 critic PASS (engine). Commission U2's instrument first (bar: progression on green logs — a clean block raises prescribed work). No legal bar yet → first round is the instrument. Do not write `status: pass`.
