...
**Next spawn:** `CRITIC · GNT-1 U4 R2` — builder-ref #653. Quiet line render instrument on `npm run dev`. Walk beats `missed-3` · `missed-7` · `missed-14`.

Unused cap is success. The cap counts only PRs that touch `src|app|scripts|supabase`. U1/U2 already-true did not spend it. U1 R2 already-true (docs) does not spend it. U1 R3 spends `.839`. U2 R2 already-true (docs) does not spend it. U3 R2 tests/docs does not spend it. U4 R2 tests/docs does not spend it.
...
| **U4** | Missed-day re-entry without shame | `npx tsx --test src/lib/reentryCopyGuard.test.ts`. **Do not** use `scripts/seed-coach-adapt-demo.mjs`. Render (U4 R2 — logger persist helper, 390×844): `npx playwright test tests/e2e/gnt1-reentry-render.spec.ts --project=mobile-chrome` | Stills after 3 / 7 / 14 missed days + guard last lines. | 2 rounds |
...
| U4 | 2 | [#653](https://github.com/Snedz/missionwinning/pull/653) tests/docs on `origin/cursor/gnt-1-u3-r2-f368` `.839` (`npm run dev`, ungated). No `APP_BUILD_LABEL` bump (no `src|app|scripts|supabase`). Logger persist helper seeds 3/7/14 calendar-day gaps; Today dock shows the quiet line. Does not plant `mw_coach_plan`. **Do not** use `seed-coach-adapt-demo.mjs`. Commands (last lines): `npx tsx --test src/lib/reentryCopyGuard.test.ts` → `# tests 5` `# pass 5` `# fail 0`. `npx playwright test tests/e2e/gnt1-reentry-render.spec.ts --project=mobile-chrome` → `gnt1_u4_missed_3_url=http://localhost:3000/log` · `gnt1_u4_missed_3_line="Three days off. Here's the 20-minute version."` · `gnt1_u4_missed_3_forbidden=0` · `gnt1_u4_missed_7_url=http://localhost:3000/log` · `gnt1_u4_missed_7_line="Seven days off. Here's the 20-minute version."` · `gnt1_u4_missed_7_forbidden=0` · `gnt1_u4_missed_14_url=http://localhost:3000/log` · `gnt1_u4_missed_14_line="14 days off. Here's the 20-minute version."` · `gnt1_u4_missed_14_forbidden=0` / `3 passed (4.3s)`. `/log` `/active` HTTP 200. GRAPH_LOOP AL1 left open. Cap not spent. | | |
...
| U3 | R2: `U3-R2-coach-history-a.png` · `U3-R2-coach-history-b.png` · `U3-R2-today-history-a.png` · `U3-R2-today-history-b.png` | **R2 critic paste:** engine **2 pass / 0 fail**. `npx playwright test tests/e2e/gnt1-coach-history-render.spec.ts --project=mobile-chrome` → **2 passed (6.2s)** with A/B dose, mobility 0 vs 2, cite session vs set. | no founder FLOORS — do not FAIL |
