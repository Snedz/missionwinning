...
**Next spawn:** `CRITIC · GNT-1 U2 R2` — already-true on U1 R3 product `.839` under `npm run dev` (not gated `next start`). Critic stills + paste. Boot: `npm run dev`, viewport 390×844. Do not `next start`.

Unused cap is success. The cap counts only PRs that touch `src|app|scripts|supabase`. U1/U2 already-true did not spend it. U1 R2 already-true (docs) does not spend it. U1 R3 spends `.839`. U2 R2 already-true (docs) does not spend it.
...
| U2 | `U2-R1-cold.png` (zero-state failure capture). returning / mid-plan **not taken** — R1 `/log` e2e never painted Today. R2 stills: critic. | **2026-08-15 critic R1:** `npx tsx --test src/lib/todayPrimaryAction.test.ts src/lib/today/todayBlockBudget.test.ts` → **14 pass / 0 fail**. `npx playwright test tests/e2e/zero-state.spec.ts --project=mobile-chrome -g "/log offers"` → **FAIL** `/log: 1 red actions, cap 0` (cap reason: docked hero is grey Just Go before any data). Failure PNG is the Alpha Notify-me poster (`/private` or `/`), not JourneyHero. **2026-08-15 builder R2 (`npm run dev`):** full `zero-state.spec.ts` → **17 passed (1.2m)** including `/log` (2.9s). Isolated `/log offers` → **1 passed (3.6s)**. Units → **14 pass / 0 fail**. Probe: `url=http://localhost:3000/log` `title=Today · Mission Winning` `redCount=0` `notifyMeCount=0` dock Just Go. | no founder FLOORS — do not FAIL |
