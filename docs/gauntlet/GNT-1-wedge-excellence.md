...
**Next spawn:** `BUILDER · GNT-1 U1 R3` — U1 R2 critic landed. Continue→`/log` is PASS. Brief = U1 R2 gap only: logger-depth empty-start, REACH, and first-90 thumbSweep/redActions never complete because `/log` and `/active` `waitUntil: 'networkidle'` hit the 60s timeout on origin/master `npm run dev`. Last U1 round (budget 3). Do not raise TAP_BUDGET.

Unused cap is success. The cap counts only PRs that touch `src|app|scripts|supabase`. U1/U2 already-true did not spend it. U1 R2 already-true (docs) does not spend it.
...
|------|--------|------------------|-------------------------|
| U1 | R1: `U1-R1-set-row.png` · `U1-R1-rest.png` · `U1-R1-ghost.png` · `U1-R1-finished.png` · `U1-R1-first-90-landed-active.png`. R2: `U1-R2-after-continue.png` · `U1-R2-set-row.png` · `U1-R2-rest.png` · `U1-R2-ghost.png` · `U1-R2-finished.png` · `U1-R2-timing.txt` | **R1:** first-90 cold visitor **FAIL** landed `/active`; logger-depth empty-start **FAIL** networkidle 60s. **R2 critic paste:** `npx playwright test tests/e2e/logger-depth.spec.ts tests/e2e/first-90.spec.ts tests/e2e/mobile-nav.spec.ts` → **21 failed / 11 passed (18.8m)**. Cold visitor **PASS** `landed_after_continue=http://localhost:3000/log`. logger-depth empty-start + 4 resilience **FAIL** `page.goto /active waitUntil networkidle` 60s; sideways-scroll ✓. mobile-nav REACH 3 cases **FAIL** `page.goto /log waitUntil networkidle` 60s. Outdoor stills PASS (PREV column; REST 1:30; Session locked). | no founder FLOORS — do not FAIL |
