...
**Next spawn:** `SMOOTHER · GNT-1` — U3 R4 `/coach` stills green on `npm run dev` (ungated). All five units have a critic PASS. Fresh smoother now that U3 is green; then the campaign report. Do **not** write `status: pass`.
