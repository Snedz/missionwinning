...
| [`concepts/index.html`](concepts/index.html) | **Three concepts, from zero** — `.644`. No shared structure, stylesheet or copy between them; three different page architectures. The build asserts they are structurally distinct from each other **and** from the shipped page, and refuses to write if any two rows of that table match. Generated by `scripts/design-concepts/build.mts`. | this file |
