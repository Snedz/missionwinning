...
| [`concepts/index.html`](concepts/index.html) | **Three architectures, and the landing they fuse into** — `.699`. 01–03 share nothing; **04-combined** is the continuous-scroll landing (Anywhere → Week → Manual → invite), one type system, real `INVITE_URL` / homepage links. The build asserts structural distinctness (04 has its own honest row) and refuses a bare `href="#"`. Generated by `scripts/design-concepts/build.mts`. | this file |
