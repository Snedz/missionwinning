...
- **History** — past sessions (date, muscles, set count); tap a row to open that log and edit the sets you logged, then Save. Confirm before a destructive change. Empty invents nothing. Search by name, date, or lift. In **Show all** → Calendar, tap a trained day to open those sessions. Deleted days stay unmarked. An empty month invents nothing. Open a session to **Repeat this session** — copies the sets you logged into Start. Empty or deleted invents nothing. Open a session to **Move to another day** — the session leaves the old day and shows on the new one. Empty, deleted, or a future day invents nothing. Open a session to **Save this session** as a CSV (JSON is the same rows). Empty or deleted invents nothing. **Export this diary** saves the live diary as a file. Empty invents nothing. Deleted sessions stay out. **Import this diary** brings that file back after you confirm. Empty invents nothing. You can also delete that one finished session after confirm — Restore is on History → Deleted sessions. Other days stay.
