...
- **Builder** — custom workouts and program templates. Show all also lists an offline program (weeks and suggested sets). Start opens Train. That list is not your logged history, and it is not a paid gate.
...
- **History** — past sessions (date, muscles, set count); tap a row to open that log and edit the sets you logged, then Save. While editing, optional 1–10 RPE on a finished set; empty is valid. While editing, optional 0–5 RIR on a finished set; empty is valid. While editing, **Move up** / **Move down** reorders lifts; Save still confirms. Confirm before a destructive change. Empty invents nothing. Search by name, date, or lift. In **Show all** → Calendar, tap a trained day to open those sessions. A trained day shows how many live sessions, not a fire. The month on screen shows how many live sessions, not a fire. Deleted days stay unmarked. An empty month invents nothing. After paging months, **This month** jumps back to the current month. No year picker. Open a session to **Repeat this session** — copies the sets you logged into Start. Empty or deleted invents nothing. Open a session to **Move to another day** — the session leaves the old day and shows on the new one. Empty, deleted, or a future day invents nothing. Open a session to **Copy to another day** — a new session lands on that day; the original stays. Empty, deleted, same day, or a future day invents nothing. Open a session to **Save this session** as a CSV (JSON is the same rows). Empty or deleted invents nothing. **Export this diary** saves the live diary as a file. Empty invents nothing. Deleted sessions stay out. **Import this diary** brings that file back after you confirm. Empty invents nothing. You can also delete that one finished session after confirm — Restore is on History → Deleted sessions. Other days stay.
...
Mobility and recovery flows — timed sequences, breathing, and light movement on rest days. You can also log an optional easy walk or easy session (minutes or distance if you have them). Rest days on Today stay empty — Move is not a second Start. After you log a session, Victory may offer **one** matching free flow for the muscles you just trained. Cross-pillar coach may also suggest Move when strain is high.
...
Weekly AI-planned sessions that adapt to your readiness, equipment, and history. See [mission-coach.md](mission-coach.md).

## Human coaching (not AI)

**Route:** `/coaching`

A lead form for 1:1 human coaching — separate from Mission Coach AI.
