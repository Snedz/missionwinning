# Mission Winning — Development Log Archive (2026-06 → 2026-07-20)

Rotated out of root [LOG.md](../../../LOG.md) on 2026-07-21. Newest first. Entries are historical record only — do not treat as current state. Links inside were written repo-root-relative; resolve them from the repository root.
...
- [docs/BETA_INVITE.md](docs/BETA_INVITE.md) points at panel; scripts remain copy source
...
- Env: `STRAVA_*` + `NEXT_PUBLIC_WEARABLES` in docs/ENV.md / check-env optional
...
- Applied `crypto_payment_intents` on prod Supabase (`YOUR_SUPABASE_REF`)
...
- Treasury pubkey: `YOUR_SOLANA_TREASURY_PUBKEY` (secret offline for founder — fund USDC ATA; one signed-in Lifetime Phantom pay still needed to confirm `enrollments.provider=phantom`)
...
## 2026-07-19 — Prod: Wave 7–8 migrations applied (`YOUR_SUPABASE_REF`)
