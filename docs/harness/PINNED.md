...
- Do not flip `PRIVATE_MODE`. Do not write excellence `status: pass`.
...
- Two consecutive generate-harvests at zero survivors → orchestration path (recipe 15): Horizon W while RESULT is not pass, then Horizon 0 agent streams. Path proven → stop. Not AU2. Not a craft walk.
